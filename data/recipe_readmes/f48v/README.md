# Voynich Speculative Herbal Ferment Recipe — f48v
...
- compound_counts: {'yeast fermentation': 65, 'main herb': 35, 'mix/transfer': 59, 'secondary herb': 8, 'aroma modifier': 3, 'sugars': 27, 'heat': 39, 'complex herbal compound': 6, 'liquid base': 3}
- dose_level: 3
...
- pcheodchy: add main plant (safe substitute) → mix / transfer → start fermentation (yeast) → duration level 1 → state: active extraction
...
- y: [unparsed]
- chedyty: apply heat/cooking → add main plant (safe substitute) → start fermentation (yeast) → duration level 1 → state: active extraction
...
- yotedy: apply heat/cooking → mix / transfer → start fermentation (yeast) → duration level 1 → state: active extraction
...
- tchddy: apply heat/cooking → add main plant (safe substitute) → start fermentation (yeast)
...
- otedy: apply heat/cooking → mix / transfer → start fermentation (yeast) → duration level 1 → state: active extraction
- fcheodal: add main plant (safe substitute) → add aroma modifier → mix / transfer → start fermentation (yeast) → duration level 1 → state: active extraction
...
- chdy: add main plant (safe substitute) → start fermentation (yeast)
- ytchedal: apply heat/cooking → add main plant (safe substitute) → start fermentation (yeast) → duration level 1 → state: active extraction
...
- or: mix / transfer
- cheedls: add main plant (safe substitute) → start fermentation (yeast) → duration level 2 → state: active extraction
- s: [unparsed]
