# Voynich Speculative Procedural Protocol — f103v

IMPORTANT: this is NOT a real or validated translation of the Voynich Manuscript. It is a speculative/procedural model that interprets EVA using a user-defined grammar to generate experimental recipes using safe, known edible substitutes.

This file is generated automatically from IVTFF/EVA transliteration plus a user-defined procedural grammar.

![f103v](../../../images/q20/f103v_crd.jpg)

## Page / Folio
- currier: B
- folio: f103v
- page_number: 213

## EVA Text (Transliteration)
```text
pol dar olfchey qoky dy qokeey qokeey daiin okeedaky qote[iir:ar] shedy dal
daiin shey qokal shedy qokeedy qoteor shey qoty chckhy qotain cha[l:?]r
qo[k:t] or chedy qokey dar chcthy char qoty shdy okeedy qokeey qokain
y cheey qokeey okeey lkees ol qoteedy ykeedy
pcheor olkeey cheky qokshdy qokaiin okechdy qopchdy qotedy qokaiin oly
dain shey qokeedy cheol qoeeor lshor qoky shedy qokaiin chedy qokam
daiin shey chol chey oteey lkeeor okaiin shedy shedy qokaiin ol chedydy
sain shey olsheey dair chekoal okeey
pchal shal shorchdy okeor okaiin shedy pchedy qotchedy qotar ol lkar
or cheey qokeeshy o[k:t]eey loiin oithy otedy lor aiin sheor qotain olldy
qokeedy olkeeshy qoky qokal shed
sal sheal shedy okeedy qokeey l ol shedy pchor pchedy pol sheedy opalam
dain ch'eey olshy otey olshedy qotshdy okeey lr ain okan olshey
teeol sheol sho qokeedy shedy qokey oshedy oteedy qokain otar aiin otam
tchedal shey lcheey lchdy ch ar olchey lcheody tedy otain otain ota[?:l]y
daiin sheekchy okeeshy qol shedy otain ol kedy
qokeedy chedy qoteey oteedy lkedy shedy qokal ol char otal opchedy
yshear ol oqaiin chckhey lchedy chedy olaiin oteedy qokeedal larorol
daiin chey lkeey chalkar cheeey l chealainor
pol char otar okaiin shaikhy oteal okain qotal shedy qokeey lolain
tokain shal qokeed oteedy sheoky shaikhhy tar teor otam oll
olshey qokshy qotalsheey [o:a]loiin oleeedy qokain shedy qokey
ycheody l ar cheey or aiiin oteey otal otear [o:a]r ar keey qoty
ykeey lchey qokeey ror aiin olan otan otain otain ar y kain
sain olkeeey qokan oteedy qotain otal oty opar aram oteeam
yteey qokeey sheety oteey lshedy oteaiin
fcheody arar okeey okeey lchedy oteal lpar otedy qotar otaryly
daiin checkhy ykeey shckhy otealshey okain chey oteedy por aiin y
olcheeey chey lkchdy sho chcthy sal araiin qokeey
pchear okain opchedy pchol fchedy otedy poly lchedy fchedey rar
okeey l chey qokeey o qokeey chedy qekchy daiin chckhy sar olainy
qokeeey chey qotey chokaiin shal chedy olkam
yshey lkeey qokain okey okaiin cheody otey shdpchy opchey oly
daiin sheey ol chey qok shey qokaiin checkhy otedy lshey lchdy
qol shey ykeey okeey lshey sheckhy chtain oty okedy otaly
saiin shey qokeey oshey olshedy
olshey qokain ol sh[e:?]y qokeshe lsheok shdy qcphey chetydar
oteey l chees ol chey chey chol keechy
polshor keeolshey okey lcharar shol okeedar shes aiin oty lchdy
yteey sheal shey qoain ol keey qokaiin shckhy lchedy rain
daiin shey l shey lshey qoar sha[s:r] al otar shedy ithy lchdy rar
yshey sheykain chey rar arol chsaly
orain chckhey qokaiin shckhy shtal opchy lkeedy chdy lchedy
qkain shey ar ar oky rain chckhy shedy qokeory lteedy ro
okey ol cheey lcheey lkain shckhy sheckhy or ain otar oly
tain shol qokain chckhy rorol chdy raly osaiin chary
```

## Domain Context (Heuristic; Not a Translation)

This section summarizes recurring **basewords** in this IVTFF domain and shows simple substring evidence that the token markers used by the procedural grammar occur inside frequent words.

Any Italian anagram / English gloss is a best-effort lexicon match, not a decipherment.



### Associated basewords (non-generic; top by frequency in this domain)
- `daiin` (count=231) → Italian anagram `piani`; English: plans (arrangements)
- `qokaiin` (count=122) → Italian anagram `ciancio`; English: [n/a]
- `okaiin` (count=109) → Italian anagram `coniai`; English: [n/a]
- `qokain` (count=101) → Italian anagram `acconi`; English: [n/a]
- `okain` (count=69) → Italian anagram `acino`; English: a berry
- `otain` (count=53) → Italian anagram `anito`; English: [n/a]
- `qokar` (count=48) → Italian anagram `carco`; English: [n/a]
- `saiin` (count=46) → Italian anagram `asini`; English: [n/a]
- `qokal` (count=43) → Italian anagram `calco`; English: cast (of sculpture)
- `qotaiin` (count=40) → Italian anagram `cationi`; English: [n/a]
- `lkaiin` (count=39) → Italian anagram `ancili`; English: [n/a]
- `kaiin` (count=37) → Italian anagram `acini`; English: [n/a]
- `qokeol` (count=37) → Italian anagram `eccolo`; English: [n/a]
- `qotain` (count=34) → Italian anagram `antico`; English: ancient
- `qotar` (count=29) → Italian anagram `corta`; English: [n/a]

### Marker evidence (substring in frequent basewords)
- `qo`: 60 basewords; examples: `qokeey`, `qokeedy`, `qokaiin`, `qokain`, `qokedy`, `qokey`
- `q`: 61 basewords; examples: `qokeey`, `qokeedy`, `qokaiin`, `qokain`, `qokedy`, `qokey`
- `o`: 262 basewords; examples: `qokeey`, `ol`, `o`, `qokeedy`, `okeey`, `qokaiin`
- `k`: 147 basewords; examples: `qokeey`, `qokeedy`, `okeey`, `qokaiin`, `okaiin`, `qokain`
- `t`: 102 basewords; examples: `otaiin`, `oteey`, `otar`, `otedy`, `otal`, `oteedy`
- `p`: 17 basewords; examples: `opchedy`, `qopchedy`, `opchey`, `pchedy`, `qopchdy`, `opchdy`
- `ch`: 137 basewords; examples: `chedy`, `chey`, `chol`, `cheey`, `cheol`, `cheody`
- `sh`: 50 basewords; examples: `shedy`, `shey`, `sheey`, `sheol`, `shol`, `sheedy`
- `f`: 1 basewords; examples: `f`
- `cth`: 16 basewords; examples: `chcthy`, `cthey`, `shcthy`, `checthy`, `cthol`, `ctheey`
- `ckh`: 15 basewords; examples: `chckhy`, `shckhy`, `checkhy`, `chckhey`, `chockhy`, `sheckhy`
- `cph`: 2 basewords; examples: `cphol`, `cphy`
- `dy`: 84 basewords; examples: `chedy`, `qokeedy`, `shedy`, `otedy`, `oteedy`, `qokedy`
- `iin`: 39 basewords; examples: `aiin`, `daiin`, `qokaiin`, `okaiin`, `otaiin`, `saiin`
- `aiin`: 33 basewords; examples: `aiin`, `daiin`, `qokaiin`, `okaiin`, `otaiin`, `saiin`

## Recipes Index (This Page)
- [f103v.1,@P0](#f103v-1-f103v-1-p0)
- [f103v.2,+P0](#f103v-2-f103v-2-p0)
- [f103v.3,+P0](#f103v-3-f103v-3-p0)
- [f103v.4,+P0](#f103v-4-f103v-4-p0)
- [f103v.5,+P0](#f103v-5-f103v-5-p0)
- [f103v.6,+P0](#f103v-6-f103v-6-p0)
- [f103v.7,+P0](#f103v-7-f103v-7-p0)
- [f103v.8,+P0](#f103v-8-f103v-8-p0)
- [f103v.9,+P0](#f103v-9-f103v-9-p0)
- [f103v.10,+P0](#f103v-10-f103v-10-p0)
- [f103v.11,+P0](#f103v-11-f103v-11-p0)
- [f103v.12,+P0](#f103v-12-f103v-12-p0)
- [f103v.13,+P0](#f103v-13-f103v-13-p0)
- [f103v.14,+P0](#f103v-14-f103v-14-p0)
- [f103v.15,+P0](#f103v-15-f103v-15-p0)
- [f103v.16,+P0](#f103v-16-f103v-16-p0)
- [f103v.17,+P0](#f103v-17-f103v-17-p0)
- [f103v.18,+P0](#f103v-18-f103v-18-p0)
- [f103v.19,+P0](#f103v-19-f103v-19-p0)
- [f103v.20,+P0](#f103v-20-f103v-20-p0)
- [f103v.21,+P0](#f103v-21-f103v-21-p0)
- [f103v.22,+P0](#f103v-22-f103v-22-p0)
- [f103v.23,+P0](#f103v-23-f103v-23-p0)
- [f103v.24,+P0](#f103v-24-f103v-24-p0)
- [f103v.25,+P0](#f103v-25-f103v-25-p0)
- [f103v.26,+P0](#f103v-26-f103v-26-p0)
- [f103v.27,+P0](#f103v-27-f103v-27-p0)
- [f103v.28,+P0](#f103v-28-f103v-28-p0)
- [f103v.29,+P0](#f103v-29-f103v-29-p0)
- [f103v.30,+P0](#f103v-30-f103v-30-p0)
- [f103v.31,+P0](#f103v-31-f103v-31-p0)
- [f103v.32,+P0](#f103v-32-f103v-32-p0)
- [f103v.33,+P0](#f103v-33-f103v-33-p0)
- [f103v.34,+P0](#f103v-34-f103v-34-p0)
- [f103v.35,+P0](#f103v-35-f103v-35-p0)
- [f103v.36,+P0](#f103v-36-f103v-36-p0)
- [f103v.37,+P0](#f103v-37-f103v-37-p0)
- [f103v.38,+P0](#f103v-38-f103v-38-p0)
- [f103v.39,+P0](#f103v-39-f103v-39-p0)
- [f103v.40,+P0](#f103v-40-f103v-40-p0)
- [f103v.41,+P0](#f103v-41-f103v-41-p0)
- [f103v.42,+P0](#f103v-42-f103v-42-p0)
- [f103v.43,+P0](#f103v-43-f103v-43-p0)
- [f103v.44,+P0](#f103v-44-f103v-44-p0)
- [f103v.45,+P0](#f103v-45-f103v-45-p0)
- [f103v.46,+P0](#f103v-46-f103v-46-p0)

## Line Glosses (Procedural Gloss Only; Not a Translation)

<a id="f103v-1-f103v-1-p0"></a>

### f103v.1,@P0

EVA: pol dar olfchey qoky dy qokeey qokeey daiin okeedaky qote[iir:ar] shedy dal

Direct Gloss (Procedural, Not a Real Translation):
- pol: tokens: p o l → connectors: l
- dar: tokens: p a r → connectors: r → vowel_run: a (level 1; class a)
- olfchey: tokens: o l f ch e → connectors: l → vowel_run: e (level 1; class e)
- qoky: tokens: qo k
- dy: tokens: p
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- okeedaky: tokens: o k ee p a k → vowel_run: ee (level 2; class e)
- qote: tokens: qo t e → vowel_run: e (level 1; class e)
- iir: tokens: ii r → connectors: r → vowel_run: ii (level 2; class i)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- dal: tokens: p a l → connectors: l → vowel_run: a (level 1; class a)

<a id="f103v-2-f103v-2-p0"></a>

### f103v.2,+P0

EVA: daiin shey qokal shedy qokeedy qoteor shey qoty chckhy qotain cha[l:?]r

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- qokal: tokens: qo k a l → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qokal` → `calco`; cast (of sculpture))
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qoteor: tokens: qo t e o r → connectors: r → vowel_run: e (level 1; class e)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- qoty: tokens: qo t
- chckhy: tokens: ch ckh
- qotain: tokens: qo t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qotain` → `antico`; ancient)
- cha: tokens: ch a → vowel_run: a (level 1; class a)
- l: tokens: l → connectors: l
- r: tokens: r → connectors: r

<a id="f103v-3-f103v-3-p0"></a>

### f103v.3,+P0

EVA: qo[k:t] or chedy qokey dar chcthy char qoty shdy okeedy qokeey qokain

Direct Gloss (Procedural, Not a Real Translation):
- qo: tokens: qo
- k: tokens: k
- t: tokens: t
- or: tokens: o r → connectors: r
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)
- dar: tokens: p a r → connectors: r → vowel_run: a (level 1; class a)
- chcthy: tokens: ch cth
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- qoty: tokens: qo t
- shdy: tokens: sh p
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `acconi`; [n/a])

<a id="f103v-4-f103v-4-p0"></a>

### f103v.4,+P0

EVA: y cheey qokeey okeey lkees ol qoteedy ykeedy

Direct Gloss (Procedural, Not a Real Translation):
- y: [unparsed]
- cheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- lkees: tokens: l k ee s → connectors: l s → vowel_run: ee (level 2; class e)
- ol: tokens: o l → connectors: l
- qoteedy: tokens: qo t ee p → vowel_run: ee (level 2; class e)
- ykeedy: tokens: k ee p → vowel_run: ee (level 2; class e)

<a id="f103v-5-f103v-5-p0"></a>

### f103v.5,+P0

EVA: pcheor olkeey cheky qokshdy qokaiin okechdy qopchdy qotedy qokaiin oly

Direct Gloss (Procedural, Not a Real Translation):
- pcheor: tokens: p ch e o r → connectors: r → vowel_run: e (level 1; class e)
- olkeey: tokens: o l k ee → connectors: l → vowel_run: ee (level 2; class e)
- cheky: tokens: ch e k → vowel_run: e (level 1; class e)
- qokshdy: tokens: qo k sh p
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- okechdy: tokens: o k e ch p → vowel_run: e (level 1; class e)
- qopchdy: tokens: qo p ch p
- qotedy: tokens: qo t e p → vowel_run: e (level 1; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- oly: tokens: o l → connectors: l

<a id="f103v-6-f103v-6-p0"></a>

### f103v.6,+P0

EVA: dain shey qokeedy cheol qoeeor lshor qoky shedy qokaiin chedy qokam

Direct Gloss (Procedural, Not a Real Translation):
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- cheol: tokens: ch e o l → connectors: l → vowel_run: e (level 1; class e)
- qoeeor: tokens: qo ee o r → connectors: r → vowel_run: ee (level 2; class e)
- lshor: tokens: l sh o r → connectors: l r
- qoky: tokens: qo k
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokam: tokens: qo k a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f103v-7-f103v-7-p0"></a>

### f103v.7,+P0

EVA: daiin shey chol chey oteey lkeeor okaiin shedy shedy qokaiin ol chedydy

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- chol: tokens: ch o l → connectors: l
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- lkeeor: tokens: l k ee o r → connectors: l r → vowel_run: ee (level 2; class e)
- okaiin: tokens: o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- ol: tokens: o l → connectors: l
- chedydy: tokens: ch e p p → vowel_run: e (level 1; class e)

<a id="f103v-8-f103v-8-p0"></a>

### f103v.8,+P0

EVA: sain shey olsheey dair chekoal okeey

Direct Gloss (Procedural, Not a Real Translation):
- sain: tokens: s a i n → connectors: s n → vowel_run: a (level 1; class a)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- olsheey: tokens: o l sh ee → connectors: l → vowel_run: ee (level 2; class e)
- dair: tokens: p a i r → connectors: r → vowel_run: a (level 1; class a)
- chekoal: tokens: ch e k o a l → connectors: l → vowel_run: e (level 1; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)

<a id="f103v-9-f103v-9-p0"></a>

### f103v.9,+P0

EVA: pchal shal shorchdy okeor okaiin shedy pchedy qotchedy qotar ol lkar

Direct Gloss (Procedural, Not a Real Translation):
- pchal: tokens: p ch a l → connectors: l → vowel_run: a (level 1; class a)
- shal: tokens: sh a l → connectors: l → vowel_run: a (level 1; class a)
- shorchdy: tokens: sh o r ch p → connectors: r
- okeor: tokens: o k e o r → connectors: r → vowel_run: e (level 1; class e)
- okaiin: tokens: o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- pchedy: tokens: p ch e p → vowel_run: e (level 1; class e)
- qotchedy: tokens: qo t ch e p → vowel_run: e (level 1; class e)
- qotar: tokens: qo t a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qotar` → `corta`; [n/a])
- ol: tokens: o l → connectors: l
- lkar: tokens: l k a r → connectors: l r → vowel_run: a (level 1; class a)

<a id="f103v-10-f103v-10-p0"></a>

### f103v.10,+P0

EVA: or cheey qokeeshy o[k:t]eey loiin oithy otedy lor aiin sheor qotain olldy

Direct Gloss (Procedural, Not a Real Translation):
- or: tokens: o r → connectors: r
- cheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- qokeeshy: tokens: qo k ee sh → vowel_run: ee (level 2; class e)
- o: tokens: o
- k: tokens: k
- t: tokens: t
- eey: tokens: ee → vowel_run: ee (level 2; class e)
- loiin: tokens: l o iin → connectors: l → vowel_run: ii (level 2; class i) → suffix: iin
- oithy: tokens: o i t h → vowel_run: i (level 1; class i) → unmodeled_tokens: h
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- lor: tokens: l o r → connectors: l r
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- sheor: tokens: sh e o r → connectors: r → vowel_run: e (level 1; class e)
- qotain: tokens: qo t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qotain` → `antico`; ancient)
- olldy: tokens: o l l p → connectors: l l

<a id="f103v-11-f103v-11-p0"></a>

### f103v.11,+P0

EVA: qokeedy olkeeshy qoky qokal shed

Direct Gloss (Procedural, Not a Real Translation):
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- olkeeshy: tokens: o l k ee sh → connectors: l → vowel_run: ee (level 2; class e)
- qoky: tokens: qo k
- qokal: tokens: qo k a l → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qokal` → `calco`; cast (of sculpture))
- shed: tokens: sh e p → vowel_run: e (level 1; class e)

<a id="f103v-12-f103v-12-p0"></a>

### f103v.12,+P0

EVA: sal sheal shedy okeedy qokeey l ol shedy pchor pchedy pol sheedy opalam

Direct Gloss (Procedural, Not a Real Translation):
- sal: tokens: s a l → connectors: s l → vowel_run: a (level 1; class a)
- sheal: tokens: sh e a l → connectors: l → vowel_run: e (level 1; class e)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- l: tokens: l → connectors: l
- ol: tokens: o l → connectors: l
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- pchor: tokens: p ch o r → connectors: r
- pchedy: tokens: p ch e p → vowel_run: e (level 1; class e)
- pol: tokens: p o l → connectors: l
- sheedy: tokens: sh ee p → vowel_run: ee (level 2; class e)
- opalam: tokens: o p a l a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f103v-13-f103v-13-p0"></a>

### f103v.13,+P0

EVA: dain ch'eey olshy otey olshedy qotshdy okeey lr ain okan olshey

Direct Gloss (Procedural, Not a Real Translation):
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- ch: tokens: ch
- eey: tokens: ee → vowel_run: ee (level 2; class e)
- olshy: tokens: o l sh → connectors: l
- otey: tokens: o t e → vowel_run: e (level 1; class e)
- olshedy: tokens: o l sh e p → connectors: l → vowel_run: e (level 1; class e)
- qotshdy: tokens: qo t sh p
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- lr: tokens: l r → connectors: l r
- ain: tokens: a i n → connectors: n → vowel_run: a (level 1; class a)
- okan: tokens: o k a n → connectors: n → vowel_run: a (level 1; class a)
- olshey: tokens: o l sh e → connectors: l → vowel_run: e (level 1; class e)

<a id="f103v-14-f103v-14-p0"></a>

### f103v.14,+P0

EVA: teeol sheol sho qokeedy shedy qokey oshedy oteedy qokain otar aiin otam

Direct Gloss (Procedural, Not a Real Translation):
- teeol: tokens: t ee o l → connectors: l → vowel_run: ee (level 2; class e)
- sheol: tokens: sh e o l → connectors: l → vowel_run: e (level 1; class e)
- sho: tokens: sh o
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)
- oshedy: tokens: o sh e p → vowel_run: e (level 1; class e)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `acconi`; [n/a])
- otar: tokens: o t a r → connectors: r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- otam: tokens: o t a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f103v-15-f103v-15-p0"></a>

### f103v.15,+P0

EVA: tchedal shey lcheey lchdy ch ar olchey lcheody tedy otain otain ota[?:l]y

Direct Gloss (Procedural, Not a Real Translation):
- tchedal: tokens: t ch e p a l → connectors: l → vowel_run: e (level 1; class e)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- lcheey: tokens: l ch ee → connectors: l → vowel_run: ee (level 2; class e)
- lchdy: tokens: l ch p → connectors: l
- ch: tokens: ch
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- olchey: tokens: o l ch e → connectors: l → vowel_run: e (level 1; class e)
- lcheody: tokens: l ch e o p → connectors: l → vowel_run: e (level 1; class e)
- tedy: tokens: t e p → vowel_run: e (level 1; class e)
- otain: tokens: o t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `otain` → `anito`; [n/a])
- otain: tokens: o t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `otain` → `anito`; [n/a])
- ota: tokens: o t a → vowel_run: a (level 1; class a)
- l: tokens: l → connectors: l
- y: [unparsed]

<a id="f103v-16-f103v-16-p0"></a>

### f103v.16,+P0

EVA: daiin sheekchy okeeshy qol shedy otain ol kedy

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- sheekchy: tokens: sh ee k ch → vowel_run: ee (level 2; class e)
- okeeshy: tokens: o k ee sh → vowel_run: ee (level 2; class e)
- qol: tokens: qo l → connectors: l
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- otain: tokens: o t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `otain` → `anito`; [n/a])
- ol: tokens: o l → connectors: l
- kedy: tokens: k e p → vowel_run: e (level 1; class e)

<a id="f103v-17-f103v-17-p0"></a>

### f103v.17,+P0

EVA: qokeedy chedy qoteey oteedy lkedy shedy qokal ol char otal opchedy

Direct Gloss (Procedural, Not a Real Translation):
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qoteey: tokens: qo t ee → vowel_run: ee (level 2; class e)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- lkedy: tokens: l k e p → connectors: l → vowel_run: e (level 1; class e)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokal: tokens: qo k a l → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qokal` → `calco`; cast (of sculpture))
- ol: tokens: o l → connectors: l
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- opchedy: tokens: o p ch e p → vowel_run: e (level 1; class e)

<a id="f103v-18-f103v-18-p0"></a>

### f103v.18,+P0

EVA: yshear ol oqaiin chckhey lchedy chedy olaiin oteedy qokeedal larorol

Direct Gloss (Procedural, Not a Real Translation):
- yshear: tokens: sh e a r → connectors: r → vowel_run: e (level 1; class e)
- ol: tokens: o l → connectors: l
- oqaiin: tokens: o q aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chckhey: tokens: ch ckh e → vowel_run: e (level 1; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- olaiin: tokens: o l aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `olaiin` → `ialino`; hyaline, glassy)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- qokeedal: tokens: qo k ee p a l → connectors: l → vowel_run: ee (level 2; class e)
- larorol: tokens: l a r o r o l → connectors: l r r l → vowel_run: a (level 1; class a)

<a id="f103v-19-f103v-19-p0"></a>

### f103v.19,+P0

EVA: daiin chey lkeey chalkar cheeey l chealainor

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- chalkar: tokens: ch a l k a r → connectors: l r → vowel_run: a (level 1; class a)
- cheeey: tokens: ch eee → vowel_run: eee (level 3; class e)
- l: tokens: l → connectors: l
- chealainor: tokens: ch e a l a i n o r → connectors: l n r → vowel_run: e (level 1; class e)

<a id="f103v-20-f103v-20-p0"></a>

### f103v.20,+P0

EVA: pol char otar okaiin shaikhy oteal okain qotal shedy qokeey lolain

Direct Gloss (Procedural, Not a Real Translation):
- pol: tokens: p o l → connectors: l
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- otar: tokens: o t a r → connectors: r → vowel_run: a (level 1; class a)
- okaiin: tokens: o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- shaikhy: tokens: sh a i k h → vowel_run: a (level 1; class a) → unmodeled_tokens: h
- oteal: tokens: o t e a l → connectors: l → vowel_run: e (level 1; class e)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `acino`; a berry)
- qotal: tokens: qo t a l → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qotal` → `colta`; [n/a])
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- lolain: tokens: l o l a i n → connectors: l l n → vowel_run: a (level 1; class a)

<a id="f103v-21-f103v-21-p0"></a>

### f103v.21,+P0

EVA: tokain shal qokeed oteedy sheoky shaikhhy tar teor otam oll

Direct Gloss (Procedural, Not a Real Translation):
- tokain: tokens: t o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `acino`; a berry)
- shal: tokens: sh a l → connectors: l → vowel_run: a (level 1; class a)
- qokeed: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- sheoky: tokens: sh e o k → vowel_run: e (level 1; class e)
- shaikhhy: tokens: sh a i k h h → vowel_run: a (level 1; class a) → unmodeled_tokens: h
- tar: tokens: t a r → connectors: r → vowel_run: a (level 1; class a)
- teor: tokens: t e o r → connectors: r → vowel_run: e (level 1; class e)
- otam: tokens: o t a m → connectors: m → vowel_run: a (level 1; class a)
- oll: tokens: o l l → connectors: l l

<a id="f103v-22-f103v-22-p0"></a>

### f103v.22,+P0

EVA: olshey qokshy qotalsheey [o:a]loiin oleeedy qokain shedy qokey

Direct Gloss (Procedural, Not a Real Translation):
- olshey: tokens: o l sh e → connectors: l → vowel_run: e (level 1; class e)
- qokshy: tokens: qo k sh
- qotalsheey: tokens: qo t a l sh ee → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qotal` → `colta`; [n/a])
- o: tokens: o
- a: tokens: a → vowel_run: a (level 1; class a)
- loiin: tokens: l o iin → connectors: l → vowel_run: ii (level 2; class i) → suffix: iin
- oleeedy: tokens: o l eee p → connectors: l → vowel_run: eee (level 3; class e)
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `acconi`; [n/a])
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)

<a id="f103v-23-f103v-23-p0"></a>

### f103v.23,+P0

EVA: ycheody l ar cheey or aiiin oteey otal otear [o:a]r ar keey qoty

Direct Gloss (Procedural, Not a Real Translation):
- ycheody: tokens: ch e o p → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- cheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- or: tokens: o r → connectors: r
- aiiin: tokens: a iii n → connectors: n → vowel_run: a (level 1; class a) → suffix: iin
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- otear: tokens: o t e a r → connectors: r → vowel_run: e (level 1; class e)
- o: tokens: o
- a: tokens: a → vowel_run: a (level 1; class a)
- r: tokens: r → connectors: r
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- keey: tokens: k ee → vowel_run: ee (level 2; class e)
- qoty: tokens: qo t

<a id="f103v-24-f103v-24-p0"></a>

### f103v.24,+P0

EVA: ykeey lchey qokeey ror aiin olan otan otain otain ar y kain

Direct Gloss (Procedural, Not a Real Translation):
- ykeey: tokens: k ee → vowel_run: ee (level 2; class e)
- lchey: tokens: l ch e → connectors: l → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- ror: tokens: r o r → connectors: r r
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- olan: tokens: o l a n → connectors: l n → vowel_run: a (level 1; class a)
- otan: tokens: o t a n → connectors: n → vowel_run: a (level 1; class a)
- otain: tokens: o t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `otain` → `anito`; [n/a])
- otain: tokens: o t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `otain` → `anito`; [n/a])
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- y: [unparsed]
- kain: tokens: k a i n → connectors: n → vowel_run: a (level 1; class a)

<a id="f103v-25-f103v-25-p0"></a>

### f103v.25,+P0

EVA: sain olkeeey qokan oteedy qotain otal oty opar aram oteeam

Direct Gloss (Procedural, Not a Real Translation):
- sain: tokens: s a i n → connectors: s n → vowel_run: a (level 1; class a)
- olkeeey: tokens: o l k eee → connectors: l → vowel_run: eee (level 3; class e)
- qokan: tokens: qo k a n → connectors: n → vowel_run: a (level 1; class a)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- qotain: tokens: qo t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qotain` → `antico`; ancient)
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- oty: tokens: o t
- opar: tokens: o p a r → connectors: r → vowel_run: a (level 1; class a)
- aram: tokens: a r a m → connectors: r m → vowel_run: a (level 1; class a)
- oteeam: tokens: o t ee a m → connectors: m → vowel_run: ee (level 2; class e)

<a id="f103v-26-f103v-26-p0"></a>

### f103v.26,+P0

EVA: yteey qokeey sheety oteey lshedy oteaiin

Direct Gloss (Procedural, Not a Real Translation):
- yteey: tokens: t ee → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- sheety: tokens: sh ee t → vowel_run: ee (level 2; class e)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- lshedy: tokens: l sh e p → connectors: l → vowel_run: e (level 1; class e)
- oteaiin: tokens: o t e aiin → vowel_run: e (level 1; class e) → suffix: aiin

<a id="f103v-27-f103v-27-p0"></a>

### f103v.27,+P0

EVA: fcheody arar okeey okeey lchedy oteal lpar otedy qotar otaryly

Direct Gloss (Procedural, Not a Real Translation):
- fcheody: tokens: f ch e o p → vowel_run: e (level 1; class e)
- arar: tokens: a r a r → connectors: r r → vowel_run: a (level 1; class a)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- oteal: tokens: o t e a l → connectors: l → vowel_run: e (level 1; class e)
- lpar: tokens: l p a r → connectors: l r → vowel_run: a (level 1; class a)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- qotar: tokens: qo t a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qotar` → `corta`; [n/a])
- otaryly: tokens: o t a r l → connectors: r l → vowel_run: a (level 1; class a)

<a id="f103v-28-f103v-28-p0"></a>

### f103v.28,+P0

EVA: daiin checkhy ykeey shckhy otealshey okain chey oteedy por aiin y

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- checkhy: tokens: ch e ckh → vowel_run: e (level 1; class e)
- ykeey: tokens: k ee → vowel_run: ee (level 2; class e)
- shckhy: tokens: sh ckh
- otealshey: tokens: o t e a l sh e → connectors: l → vowel_run: e (level 1; class e)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `acino`; a berry)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- por: tokens: p o r → connectors: r
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- y: [unparsed]

<a id="f103v-29-f103v-29-p0"></a>

### f103v.29,+P0

EVA: olcheeey chey lkchdy sho chcthy sal araiin qokeey

Direct Gloss (Procedural, Not a Real Translation):
- olcheeey: tokens: o l ch eee → connectors: l → vowel_run: eee (level 3; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- lkchdy: tokens: l k ch p → connectors: l
- sho: tokens: sh o
- chcthy: tokens: ch cth
- sal: tokens: s a l → connectors: s l → vowel_run: a (level 1; class a)
- araiin: tokens: a r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)

<a id="f103v-30-f103v-30-p0"></a>

### f103v.30,+P0

EVA: pchear okain opchedy pchol fchedy otedy poly lchedy fchedey rar

Direct Gloss (Procedural, Not a Real Translation):
- pchear: tokens: p ch e a r → connectors: r → vowel_run: e (level 1; class e)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `acino`; a berry)
- opchedy: tokens: o p ch e p → vowel_run: e (level 1; class e)
- pchol: tokens: p ch o l → connectors: l
- fchedy: tokens: f ch e p → vowel_run: e (level 1; class e)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- poly: tokens: p o l → connectors: l
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- fchedey: tokens: f ch e p e → vowel_run: e (level 1; class e)
- rar: tokens: r a r → connectors: r r → vowel_run: a (level 1; class a)

<a id="f103v-31-f103v-31-p0"></a>

### f103v.31,+P0

EVA: okeey l chey qokeey o qokeey chedy qekchy daiin chckhy sar olainy

Direct Gloss (Procedural, Not a Real Translation):
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- l: tokens: l → connectors: l
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- o: tokens: o
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qekchy: tokens: q e k ch → vowel_run: e (level 1; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- chckhy: tokens: ch ckh
- sar: tokens: s a r → connectors: s r → vowel_run: a (level 1; class a)
- olainy: tokens: o l a i n → connectors: l n → vowel_run: a (level 1; class a)

<a id="f103v-32-f103v-32-p0"></a>

### f103v.32,+P0

EVA: qokeeey chey qotey chokaiin shal chedy olkam

Direct Gloss (Procedural, Not a Real Translation):
- qokeeey: tokens: qo k eee → vowel_run: eee (level 3; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- qotey: tokens: qo t e → vowel_run: e (level 1; class e)
- chokaiin: tokens: ch o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- shal: tokens: sh a l → connectors: l → vowel_run: a (level 1; class a)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- olkam: tokens: o l k a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f103v-33-f103v-33-p0"></a>

### f103v.33,+P0

EVA: yshey lkeey qokain okey okaiin cheody otey shdpchy opchey oly

Direct Gloss (Procedural, Not a Real Translation):
- yshey: tokens: sh e → vowel_run: e (level 1; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `acconi`; [n/a])
- okey: tokens: o k e → vowel_run: e (level 1; class e)
- okaiin: tokens: o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- cheody: tokens: ch e o p → vowel_run: e (level 1; class e)
- otey: tokens: o t e → vowel_run: e (level 1; class e)
- shdpchy: tokens: sh p p ch
- opchey: tokens: o p ch e → vowel_run: e (level 1; class e)
- oly: tokens: o l → connectors: l

<a id="f103v-34-f103v-34-p0"></a>

### f103v.34,+P0

EVA: daiin sheey ol chey qok shey qokaiin checkhy otedy lshey lchdy

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- sheey: tokens: sh ee → vowel_run: ee (level 2; class e)
- ol: tokens: o l → connectors: l
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- qok: tokens: qo k
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- checkhy: tokens: ch e ckh → vowel_run: e (level 1; class e)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- lshey: tokens: l sh e → connectors: l → vowel_run: e (level 1; class e)
- lchdy: tokens: l ch p → connectors: l

<a id="f103v-35-f103v-35-p0"></a>

### f103v.35,+P0

EVA: qol shey ykeey okeey lshey sheckhy chtain oty okedy otaly

Direct Gloss (Procedural, Not a Real Translation):
- qol: tokens: qo l → connectors: l
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- ykeey: tokens: k ee → vowel_run: ee (level 2; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- lshey: tokens: l sh e → connectors: l → vowel_run: e (level 1; class e)
- sheckhy: tokens: sh e ckh → vowel_run: e (level 1; class e)
- chtain: tokens: ch t a i n → connectors: n → vowel_run: a (level 1; class a)
- oty: tokens: o t
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- otaly: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)

<a id="f103v-36-f103v-36-p0"></a>

### f103v.36,+P0

EVA: saiin shey qokeey oshey olshedy

Direct Gloss (Procedural, Not a Real Translation):
- saiin: tokens: s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `saiin` → `asini`; [n/a])
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- oshey: tokens: o sh e → vowel_run: e (level 1; class e)
- olshedy: tokens: o l sh e p → connectors: l → vowel_run: e (level 1; class e)

<a id="f103v-37-f103v-37-p0"></a>

### f103v.37,+P0

EVA: olshey qokain ol sh[e:?]y qokeshe lsheok shdy qcphey chetydar

Direct Gloss (Procedural, Not a Real Translation):
- olshey: tokens: o l sh e → connectors: l → vowel_run: e (level 1; class e)
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `acconi`; [n/a])
- ol: tokens: o l → connectors: l
- sh: tokens: sh
- e: tokens: e → vowel_run: e (level 1; class e)
- y: [unparsed]
- qokeshe: tokens: qo k e sh e → vowel_run: e (level 1; class e)
- lsheok: tokens: l sh e o k → connectors: l → vowel_run: e (level 1; class e)
- shdy: tokens: sh p
- qcphey: tokens: q cph e → vowel_run: e (level 1; class e)
- chetydar: tokens: ch e t p a r → connectors: r → vowel_run: e (level 1; class e)

<a id="f103v-38-f103v-38-p0"></a>

### f103v.38,+P0

EVA: oteey l chees ol chey chey chol keechy

Direct Gloss (Procedural, Not a Real Translation):
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- l: tokens: l → connectors: l
- chees: tokens: ch ee s → connectors: s → vowel_run: ee (level 2; class e)
- ol: tokens: o l → connectors: l
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- chol: tokens: ch o l → connectors: l
- keechy: tokens: k ee ch → vowel_run: ee (level 2; class e)

<a id="f103v-39-f103v-39-p0"></a>

### f103v.39,+P0

EVA: polshor keeolshey okey lcharar shol okeedar shes aiin oty lchdy

Direct Gloss (Procedural, Not a Real Translation):
- polshor: tokens: p o l sh o r → connectors: l r
- keeolshey: tokens: k ee o l sh e → connectors: l → vowel_run: ee (level 2; class e)
- okey: tokens: o k e → vowel_run: e (level 1; class e)
- lcharar: tokens: l ch a r a r → connectors: l r r → vowel_run: a (level 1; class a)
- shol: tokens: sh o l → connectors: l
- okeedar: tokens: o k ee p a r → connectors: r → vowel_run: ee (level 2; class e)
- shes: tokens: sh e s → connectors: s → vowel_run: e (level 1; class e)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- oty: tokens: o t
- lchdy: tokens: l ch p → connectors: l

<a id="f103v-40-f103v-40-p0"></a>

### f103v.40,+P0

EVA: yteey sheal shey qoain ol keey qokaiin shckhy lchedy rain

Direct Gloss (Procedural, Not a Real Translation):
- yteey: tokens: t ee → vowel_run: ee (level 2; class e)
- sheal: tokens: sh e a l → connectors: l → vowel_run: e (level 1; class e)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- qoain: tokens: qo a i n → connectors: n → vowel_run: a (level 1; class a)
- ol: tokens: o l → connectors: l
- keey: tokens: k ee → vowel_run: ee (level 2; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- shckhy: tokens: sh ckh
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- rain: tokens: r a i n → connectors: r n → vowel_run: a (level 1; class a)

<a id="f103v-41-f103v-41-p0"></a>

### f103v.41,+P0

EVA: daiin shey l shey lshey qoar sha[s:r] al otar shedy ithy lchdy rar

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- lshey: tokens: l sh e → connectors: l → vowel_run: e (level 1; class e)
- qoar: tokens: qo a r → connectors: r → vowel_run: a (level 1; class a)
- sha: tokens: sh a → vowel_run: a (level 1; class a)
- s: tokens: s → connectors: s
- r: tokens: r → connectors: r
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- otar: tokens: o t a r → connectors: r → vowel_run: a (level 1; class a)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- ithy: tokens: i t h → vowel_run: i (level 1; class i) → unmodeled_tokens: h
- lchdy: tokens: l ch p → connectors: l
- rar: tokens: r a r → connectors: r r → vowel_run: a (level 1; class a)

<a id="f103v-42-f103v-42-p0"></a>

### f103v.42,+P0

EVA: yshey sheykain chey rar arol chsaly

Direct Gloss (Procedural, Not a Real Translation):
- yshey: tokens: sh e → vowel_run: e (level 1; class e)
- sheykain: tokens: sh e k a i n → connectors: n → vowel_run: e (level 1; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- rar: tokens: r a r → connectors: r r → vowel_run: a (level 1; class a)
- arol: tokens: a r o l → connectors: r l → vowel_run: a (level 1; class a)
- chsaly: tokens: ch s a l → connectors: s l → vowel_run: a (level 1; class a)

<a id="f103v-43-f103v-43-p0"></a>

### f103v.43,+P0

EVA: orain chckhey qokaiin shckhy shtal opchy lkeedy chdy lchedy

Direct Gloss (Procedural, Not a Real Translation):
- orain: tokens: o r a i n → connectors: r n → vowel_run: a (level 1; class a)
- chckhey: tokens: ch ckh e → vowel_run: e (level 1; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- shckhy: tokens: sh ckh
- shtal: tokens: sh t a l → connectors: l → vowel_run: a (level 1; class a)
- opchy: tokens: o p ch
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- chdy: tokens: ch p
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)

<a id="f103v-44-f103v-44-p0"></a>

### f103v.44,+P0

EVA: qkain shey ar ar oky rain chckhy shedy qokeory lteedy ro

Direct Gloss (Procedural, Not a Real Translation):
- qkain: tokens: q k a i n → connectors: n → vowel_run: a (level 1; class a)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- oky: tokens: o k
- rain: tokens: r a i n → connectors: r n → vowel_run: a (level 1; class a)
- chckhy: tokens: ch ckh
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokeory: tokens: qo k e o r → connectors: r → vowel_run: e (level 1; class e)
- lteedy: tokens: l t ee p → connectors: l → vowel_run: ee (level 2; class e)
- ro: tokens: r o → connectors: r

<a id="f103v-45-f103v-45-p0"></a>

### f103v.45,+P0

EVA: okey ol cheey lcheey lkain shckhy sheckhy or ain otar oly

Direct Gloss (Procedural, Not a Real Translation):
- okey: tokens: o k e → vowel_run: e (level 1; class e)
- ol: tokens: o l → connectors: l
- cheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- lcheey: tokens: l ch ee → connectors: l → vowel_run: ee (level 2; class e)
- lkain: tokens: l k a i n → connectors: l n → vowel_run: a (level 1; class a) (lexicon-context: `lkain` → `lanci`; [n/a])
- shckhy: tokens: sh ckh
- sheckhy: tokens: sh e ckh → vowel_run: e (level 1; class e)
- or: tokens: o r → connectors: r
- ain: tokens: a i n → connectors: n → vowel_run: a (level 1; class a)
- otar: tokens: o t a r → connectors: r → vowel_run: a (level 1; class a)
- oly: tokens: o l → connectors: l

<a id="f103v-46-f103v-46-p0"></a>

### f103v.46,+P0

EVA: tain shol qokain chckhy rorol chdy raly osaiin chary

Direct Gloss (Procedural, Not a Real Translation):
- tain: tokens: t a i n → connectors: n → vowel_run: a (level 1; class a)
- shol: tokens: sh o l → connectors: l
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `acconi`; [n/a])
- chckhy: tokens: ch ckh
- rorol: tokens: r o r o l → connectors: r r l
- chdy: tokens: ch p
- raly: tokens: r a l → connectors: r l → vowel_run: a (level 1; class a)
- osaiin: tokens: o s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `saiin` → `asini`; [n/a])
- chary: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
