# Voynich Speculative Procedural Protocol — f108r

IMPORTANT: this is NOT a real or validated translation of the Voynich Manuscript. It is a speculative/procedural model that interprets EVA using a user-defined grammar to generate experimental recipes using safe, known edible substitutes.

This file is generated automatically from IVTFF/EVA transliteration plus a user-defined procedural grammar.

![f108r](../../../images/q20/f108r_crd.jpg)

## Page / Folio
- currier: B
- folio: f108r
- page_number: 222

## EVA Text (Transliteration)
```text
teodeeor qopchety okeeoly okchdpchy todai'hx opar yo lpair ody otedy kedydy
qokeedy chokedy chockhed qokeor shedy otal otaiin otedy qokechy ofchdy qody
cheodchy qote[ci:a]r okedy oteal lkedal okedy otedy okedal chckhedy chokedal
ychedaiin shky chdy oteeody chedor sheeky
tchokedy chey oteey okeey lkedch'edy okchecf y pchofar cheo pchedy qotedy otol
ol cheol qo qokeey qokeey qokeedy sheoky otedy qotey qokchey chdar aiin y
ok[che:eee]y okedy qokchedy chedy qokedy okar chdy okar char chkaiin chsy
pochedy shy shokeedar al kedal chokchy qokar ch okeey qaiin okeedy qetam
cthey chedain chedy okedy lkedy raraiin qokar otal kedy ar air ockhedy
taiin shkeedy qoteey oteey ykeol ykeedy chekar qokeeodaiin okeyty ody
te[o:a]deey ar aiin okeey qetchdy oteeody okaiin ykeeo dain otalaiin oly
tshey okchey dain aloiin ocheey qokaiin chlam okaiin okal kain alam
ykeey raiin ar ail odaiin
polchal shol qokar shedy pcholy qokal opchdy ofal shor qokaiin otalod
orain cheey chey qokeey chedy okchedy qoeedy qockhey d iirain okain
saithy olkaiin chckhy daiin dal daiin
fcheokair okedaiin chedy qokeed okain chdy laiin ofar chedy tedam
okeedy lkal daiin ykchedy qokol chedy qokedy lkedy okalo l chedl y
dchedy okeedar shchy okol kedy okeedy chal raiin otedy chtal om
dain chey qokeedy cholcheey dalkar okedy
qolshy qoeedy lkealshedy shokal keedy qotedy qopchedy otal dkedam
dcheol shol dal qokaiin otal ol shedy qokey chey lor aiin okeeam
ykeol chey okain charain
folaiin c'hkhy qokal lkedy qotedy qoked qotedy okal chdar al char aiin
y cheeor aiin okeey cheol chedy opchsd opchedy alkedy al lkeam ol rain al
dair okal chedy qokeedy oteor al qokedy qokedy otaiin otedar chalkeedy
ychedain orcheory qoaiin okeey qokeey chdal okedy qokedy okedam chdy
sain arolkeey qoal olkedy okaiin
polchedy okedy qopchedy okeolkey teedy qotaiin otedy okeedy otedam
ykeol chedal chokedy o qokedy okchedy saiin alkan
keol sheody qokedy qokeedy qokoy qoiiedy otedy qokedy qokedy qokal
ykedar chedy qokey lkeedy otedy [o:y]kedy otedy otal shol alodar or alold
solkedy okchd qokedy okedy okeol chdy okar okedy okedy chlar
ycheol chckhy qokedy okain
pychory opchey lkeol chdar shedy qofchdy opalal cheedy qokalchdy opchdy
okechy okeol okeedy cheedar okedy qotal chdy chedal chedy
tchedy qokey okeey lkeey chedy cheokeey qokeey chedl al lkair alkeedy ram
dcheeody cheedy qokeedy otedy qeeey chey
poro l tedo lchdain qokar otedy otarar yty sho qotedy oteedy okedy
okor aiin okeedy qokeedy okeeom cheykeeed chol qekeey or aiin chckhom
sheedain qokeedy chokeedy qoteody keesho daiin chedy keedal as keodky
okeey qokeey qokeedy qokeey chedal chedy qokeey okeedain otain oolals
olke?y okey lokeey qokedy ol kechy chedar olal oedy qckhy cheam
ychey chey okar charam otam cheody
dain sheckhy okeey keey lchedy okeey qokedy chedy dalkedy qokedy dal
yteedy qokeed y qokey otedy okedy qokaiin cheodar cheey qokedy al
yched chockhey dain sheol okedy lkeedy oteedy qokeedy ched al keam
pcheedy qokeo l okeol keol dain ar otedy qokedy qokeedy qokeedy lchdy
oteedy lchedy qokeedy qokeedy qokeo lchedy qokey qokeey lkedy lkedy
qokeedy lchedy lkaiin chdy qokey okedy okaram chdar ar okam
```

## Domain Context (Heuristic; Not a Translation)

This section summarizes recurring **basewords** in this IVTFF domain and shows simple substring evidence that the token markers used by the procedural grammar occur inside frequent words.

Any Italian anagram / English gloss is a best-effort lexicon match, not a decipherment.



### Associated basewords (non-generic; top by frequency in this domain)
- `daiin` (count=231) → Italian anagram `piani`; English: plans (arrangements)
- `qokaiin` (count=122) → Italian anagram `ciancio`; English: [n/a]
- `okaiin` (count=109) → Italian anagram `coniai`; English: [n/a]
- `qokain` (count=101) → Italian anagram `acconi`; English: [n/a]
- `okain` (count=69) → Italian anagram `acino`; English: a berry
- `otain` (count=53) → Italian anagram `anito`; English: [n/a]
- `qokar` (count=48) → Italian anagram `carco`; English: [n/a]
- `saiin` (count=46) → Italian anagram `asini`; English: [n/a]
- `qokal` (count=43) → Italian anagram `calco`; English: cast (of sculpture)
- `qotaiin` (count=40) → Italian anagram `cationi`; English: [n/a]
- `lkaiin` (count=39) → Italian anagram `ancili`; English: [n/a]
- `kaiin` (count=37) → Italian anagram `acini`; English: [n/a]
- `qokeol` (count=37) → Italian anagram `eccolo`; English: [n/a]
- `qotain` (count=34) → Italian anagram `antico`; English: ancient
- `qotar` (count=29) → Italian anagram `corta`; English: [n/a]

### Marker evidence (substring in frequent basewords)
- `qo`: 60 basewords; examples: `qokeey`, `qokeedy`, `qokaiin`, `qokain`, `qokedy`, `qokey`
- `q`: 61 basewords; examples: `qokeey`, `qokeedy`, `qokaiin`, `qokain`, `qokedy`, `qokey`
- `o`: 262 basewords; examples: `qokeey`, `ol`, `o`, `qokeedy`, `okeey`, `qokaiin`
- `k`: 147 basewords; examples: `qokeey`, `qokeedy`, `okeey`, `qokaiin`, `okaiin`, `qokain`
- `t`: 102 basewords; examples: `otaiin`, `oteey`, `otar`, `otedy`, `otal`, `oteedy`
- `p`: 17 basewords; examples: `opchedy`, `qopchedy`, `opchey`, `pchedy`, `qopchdy`, `opchdy`
- `ch`: 137 basewords; examples: `chedy`, `chey`, `chol`, `cheey`, `cheol`, `cheody`
- `sh`: 50 basewords; examples: `shedy`, `shey`, `sheey`, `sheol`, `shol`, `sheedy`
- `f`: 1 basewords; examples: `f`
- `cth`: 16 basewords; examples: `chcthy`, `cthey`, `shcthy`, `checthy`, `cthol`, `ctheey`
- `ckh`: 15 basewords; examples: `chckhy`, `shckhy`, `checkhy`, `chckhey`, `chockhy`, `sheckhy`
- `cph`: 2 basewords; examples: `cphol`, `cphy`
- `dy`: 84 basewords; examples: `chedy`, `qokeedy`, `shedy`, `otedy`, `oteedy`, `qokedy`
- `iin`: 39 basewords; examples: `aiin`, `daiin`, `qokaiin`, `okaiin`, `otaiin`, `saiin`
- `aiin`: 33 basewords; examples: `aiin`, `daiin`, `qokaiin`, `okaiin`, `otaiin`, `saiin`

## Recipes Index (This Page)
- [f108r.1,@P0](#f108r-1-f108r-1-p0)
- [f108r.2,+P0](#f108r-2-f108r-2-p0)
- [f108r.3,+P0](#f108r-3-f108r-3-p0)
- [f108r.4,+P0](#f108r-4-f108r-4-p0)
- [f108r.5,+P0](#f108r-5-f108r-5-p0)
- [f108r.6,+P0](#f108r-6-f108r-6-p0)
- [f108r.7,+P0](#f108r-7-f108r-7-p0)
- [f108r.8,+P0](#f108r-8-f108r-8-p0)
- [f108r.9,+P0](#f108r-9-f108r-9-p0)
- [f108r.10,+P0](#f108r-10-f108r-10-p0)
- [f108r.11,+P0](#f108r-11-f108r-11-p0)
- [f108r.12,+P0](#f108r-12-f108r-12-p0)
- [f108r.13,+P0](#f108r-13-f108r-13-p0)
- [f108r.14,+P0](#f108r-14-f108r-14-p0)
- [f108r.15,+P0](#f108r-15-f108r-15-p0)
- [f108r.16,+P0](#f108r-16-f108r-16-p0)
- [f108r.17,+P0](#f108r-17-f108r-17-p0)
- [f108r.18,+P0](#f108r-18-f108r-18-p0)
- [f108r.19,+P0](#f108r-19-f108r-19-p0)
- [f108r.20,+P0](#f108r-20-f108r-20-p0)
- [f108r.21,+P0](#f108r-21-f108r-21-p0)
- [f108r.22,+P0](#f108r-22-f108r-22-p0)
- [f108r.23,+P0](#f108r-23-f108r-23-p0)
- [f108r.24,+P0](#f108r-24-f108r-24-p0)
- [f108r.25,+P0](#f108r-25-f108r-25-p0)
- [f108r.26,+P0](#f108r-26-f108r-26-p0)
- [f108r.27,+P0](#f108r-27-f108r-27-p0)
- [f108r.28,+P0](#f108r-28-f108r-28-p0)
- [f108r.29,+P0](#f108r-29-f108r-29-p0)
- [f108r.30,+P0](#f108r-30-f108r-30-p0)
- [f108r.31,+P0](#f108r-31-f108r-31-p0)
- [f108r.32,+P0](#f108r-32-f108r-32-p0)
- [f108r.33,+P0](#f108r-33-f108r-33-p0)
- [f108r.34,+P0](#f108r-34-f108r-34-p0)
- [f108r.35,+P0](#f108r-35-f108r-35-p0)
- [f108r.36,+P0](#f108r-36-f108r-36-p0)
- [f108r.37,+P0](#f108r-37-f108r-37-p0)
- [f108r.38,+P0](#f108r-38-f108r-38-p0)
- [f108r.39,+P0](#f108r-39-f108r-39-p0)
- [f108r.40,+P0](#f108r-40-f108r-40-p0)
- [f108r.41,+P0](#f108r-41-f108r-41-p0)
- [f108r.42,+P0](#f108r-42-f108r-42-p0)
- [f108r.43,+P0](#f108r-43-f108r-43-p0)
- [f108r.44,+P0](#f108r-44-f108r-44-p0)
- [f108r.45,+P0](#f108r-45-f108r-45-p0)
- [f108r.46,+P0](#f108r-46-f108r-46-p0)
- [f108r.47,+P0](#f108r-47-f108r-47-p0)
- [f108r.48,+P0](#f108r-48-f108r-48-p0)
- [f108r.49,+P0](#f108r-49-f108r-49-p0)
- [f108r.50,+P0](#f108r-50-f108r-50-p0)

## Line Glosses (Procedural Gloss Only; Not a Translation)

<a id="f108r-1-f108r-1-p0"></a>

### f108r.1,@P0

EVA: teodeeor qopchety okeeoly okchdpchy todai'hx opar yo lpair ody otedy kedydy

Direct Gloss (Procedural, Not a Real Translation):
- teodeeor: tokens: t e o p ee o r → connectors: r → vowel_run: e (level 1; class e)
- qopchety: tokens: qo p ch e t → vowel_run: e (level 1; class e)
- okeeoly: tokens: o k ee o l → connectors: l → vowel_run: ee (level 2; class e)
- okchdpchy: tokens: o k ch p p ch
- todai: tokens: t o p a i → vowel_run: a (level 1; class a)
- hx: tokens: h x → unmodeled_tokens: h
- opar: tokens: o p a r → connectors: r → vowel_run: a (level 1; class a)
- yo: tokens: o
- lpair: tokens: l p a i r → connectors: l r → vowel_run: a (level 1; class a)
- ody: tokens: o p
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- kedydy: tokens: k e p p → vowel_run: e (level 1; class e)

<a id="f108r-2-f108r-2-p0"></a>

### f108r.2,+P0

EVA: qokeedy chokedy chockhed qokeor shedy otal otaiin otedy qokechy ofchdy qody

Direct Gloss (Procedural, Not a Real Translation):
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- chokedy: tokens: ch o k e p → vowel_run: e (level 1; class e)
- chockhed: tokens: ch o ckh e p → vowel_run: e (level 1; class e)
- qokeor: tokens: qo k e o r → connectors: r → vowel_run: e (level 1; class e)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- qokechy: tokens: qo k e ch → vowel_run: e (level 1; class e)
- ofchdy: tokens: o f ch p
- qody: tokens: qo p

<a id="f108r-3-f108r-3-p0"></a>

### f108r.3,+P0

EVA: cheodchy qote[ci:a]r okedy oteal lkedal okedy otedy okedal chckhedy chokedal

Direct Gloss (Procedural, Not a Real Translation):
- cheodchy: tokens: ch e o p ch → vowel_run: e (level 1; class e)
- qote: tokens: qo t e → vowel_run: e (level 1; class e)
- ci: tokens: c i → vowel_run: i (level 1; class i)
- a: tokens: a → vowel_run: a (level 1; class a)
- r: tokens: r → connectors: r
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- oteal: tokens: o t e a l → connectors: l → vowel_run: e (level 1; class e)
- lkedal: tokens: l k e p a l → connectors: l l → vowel_run: e (level 1; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- okedal: tokens: o k e p a l → connectors: l → vowel_run: e (level 1; class e)
- chckhedy: tokens: ch ckh e p → vowel_run: e (level 1; class e)
- chokedal: tokens: ch o k e p a l → connectors: l → vowel_run: e (level 1; class e)

<a id="f108r-4-f108r-4-p0"></a>

### f108r.4,+P0

EVA: ychedaiin shky chdy oteeody chedor sheeky

Direct Gloss (Procedural, Not a Real Translation):
- ychedaiin: tokens: ch e p aiin → vowel_run: e (level 1; class e) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- shky: tokens: sh k
- chdy: tokens: ch p
- oteeody: tokens: o t ee o p → vowel_run: ee (level 2; class e)
- chedor: tokens: ch e p o r → connectors: r → vowel_run: e (level 1; class e)
- sheeky: tokens: sh ee k → vowel_run: ee (level 2; class e)

<a id="f108r-5-f108r-5-p0"></a>

### f108r.5,+P0

EVA: tchokedy chey oteey okeey lkedch'edy okchecf y pchofar cheo pchedy qotedy otol

Direct Gloss (Procedural, Not a Real Translation):
- tchokedy: tokens: t ch o k e p → vowel_run: e (level 1; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- lkedch: tokens: l k e p ch → connectors: l → vowel_run: e (level 1; class e)
- edy: tokens: e p → vowel_run: e (level 1; class e)
- okchecf: tokens: o k ch e c f → vowel_run: e (level 1; class e)
- y: [unparsed]
- pchofar: tokens: p ch o f a r → connectors: r → vowel_run: a (level 1; class a)
- cheo: tokens: ch e o → vowel_run: e (level 1; class e)
- pchedy: tokens: p ch e p → vowel_run: e (level 1; class e)
- qotedy: tokens: qo t e p → vowel_run: e (level 1; class e)
- otol: tokens: o t o l → connectors: l

<a id="f108r-6-f108r-6-p0"></a>

### f108r.6,+P0

EVA: ol cheol qo qokeey qokeey qokeedy sheoky otedy qotey qokchey chdar aiin y

Direct Gloss (Procedural, Not a Real Translation):
- ol: tokens: o l → connectors: l
- cheol: tokens: ch e o l → connectors: l → vowel_run: e (level 1; class e)
- qo: tokens: qo
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- sheoky: tokens: sh e o k → vowel_run: e (level 1; class e)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- qotey: tokens: qo t e → vowel_run: e (level 1; class e)
- qokchey: tokens: qo k ch e → vowel_run: e (level 1; class e) (lexicon-context: `qokchey` → `cecco`; [n/a])
- chdar: tokens: ch p a r → connectors: r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- y: [unparsed]

<a id="f108r-7-f108r-7-p0"></a>

### f108r.7,+P0

EVA: ok[che:eee]y okedy qokchedy chedy qokedy okar chdy okar char chkaiin chsy

Direct Gloss (Procedural, Not a Real Translation):
- ok: tokens: o k
- che: tokens: ch e → vowel_run: e (level 1; class e)
- eee: tokens: eee → vowel_run: eee (level 3; class e)
- y: [unparsed]
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- qokchedy: tokens: qo k ch e p → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- okar: tokens: o k a r → connectors: r → vowel_run: a (level 1; class a)
- chdy: tokens: ch p
- okar: tokens: o k a r → connectors: r → vowel_run: a (level 1; class a)
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- chkaiin: tokens: ch k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `kaiin` → `acini`; [n/a])
- chsy: tokens: ch s → connectors: s

<a id="f108r-8-f108r-8-p0"></a>

### f108r.8,+P0

EVA: pochedy shy shokeedar al kedal chokchy qokar ch okeey qaiin okeedy qetam

Direct Gloss (Procedural, Not a Real Translation):
- pochedy: tokens: p o ch e p → vowel_run: e (level 1; class e)
- shy: tokens: sh
- shokeedar: tokens: sh o k ee p a r → connectors: r → vowel_run: ee (level 2; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- kedal: tokens: k e p a l → connectors: l → vowel_run: e (level 1; class e)
- chokchy: tokens: ch o k ch
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qokar` → `carco`; [n/a])
- ch: tokens: ch
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- qaiin: tokens: q aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- qetam: tokens: q e t a m → connectors: m → vowel_run: e (level 1; class e)

<a id="f108r-9-f108r-9-p0"></a>

### f108r.9,+P0

EVA: cthey chedain chedy okedy lkedy raraiin qokar otal kedy ar air ockhedy

Direct Gloss (Procedural, Not a Real Translation):
- cthey: tokens: cth e → vowel_run: e (level 1; class e)
- chedain: tokens: ch e p a i n → connectors: n → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- lkedy: tokens: l k e p → connectors: l → vowel_run: e (level 1; class e)
- raraiin: tokens: r a r aiin → connectors: r r → vowel_run: a (level 1; class a) → suffix: aiin
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qokar` → `carco`; [n/a])
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- kedy: tokens: k e p → vowel_run: e (level 1; class e)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- air: tokens: a i r → connectors: r → vowel_run: a (level 1; class a)
- ockhedy: tokens: o ckh e p → vowel_run: e (level 1; class e)

<a id="f108r-10-f108r-10-p0"></a>

### f108r.10,+P0

EVA: taiin shkeedy qoteey oteey ykeol ykeedy chekar qokeeodaiin okeyty ody

Direct Gloss (Procedural, Not a Real Translation):
- taiin: tokens: t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- shkeedy: tokens: sh k ee p → vowel_run: ee (level 2; class e)
- qoteey: tokens: qo t ee → vowel_run: ee (level 2; class e)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- ykeol: tokens: k e o l → connectors: l → vowel_run: e (level 1; class e)
- ykeedy: tokens: k ee p → vowel_run: ee (level 2; class e)
- chekar: tokens: ch e k a r → connectors: r → vowel_run: e (level 1; class e)
- qokeeodaiin: tokens: qo k ee o p aiin → vowel_run: ee (level 2; class e) → suffix: aiin (lexicon-context: `odaiin` → `inopia`; poverty)
- okeyty: tokens: o k e t → vowel_run: e (level 1; class e)
- ody: tokens: o p

<a id="f108r-11-f108r-11-p0"></a>

### f108r.11,+P0

EVA: te[o:a]deey ar aiin okeey qetchdy oteeody okaiin ykeeo dain otalaiin oly

Direct Gloss (Procedural, Not a Real Translation):
- te: tokens: t e → vowel_run: e (level 1; class e)
- o: tokens: o
- a: tokens: a → vowel_run: a (level 1; class a)
- deey: tokens: p ee → vowel_run: ee (level 2; class e)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- qetchdy: tokens: q e t ch p → vowel_run: e (level 1; class e)
- oteeody: tokens: o t ee o p → vowel_run: ee (level 2; class e)
- okaiin: tokens: o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- ykeeo: tokens: k ee o → vowel_run: ee (level 2; class e)
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- otalaiin: tokens: o t a l aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin
- oly: tokens: o l → connectors: l

<a id="f108r-12-f108r-12-p0"></a>

### f108r.12,+P0

EVA: tshey okchey dain aloiin ocheey qokaiin chlam okaiin okal kain alam

Direct Gloss (Procedural, Not a Real Translation):
- tshey: tokens: t sh e → vowel_run: e (level 1; class e)
- okchey: tokens: o k ch e → vowel_run: e (level 1; class e)
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- aloiin: tokens: a l o iin → connectors: l → vowel_run: a (level 1; class a) → suffix: iin
- ocheey: tokens: o ch ee → vowel_run: ee (level 2; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- chlam: tokens: ch l a m → connectors: l m → vowel_run: a (level 1; class a)
- okaiin: tokens: o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- okal: tokens: o k a l → connectors: l → vowel_run: a (level 1; class a)
- kain: tokens: k a i n → connectors: n → vowel_run: a (level 1; class a)
- alam: tokens: a l a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f108r-13-f108r-13-p0"></a>

### f108r.13,+P0

EVA: ykeey raiin ar ail odaiin

Direct Gloss (Procedural, Not a Real Translation):
- ykeey: tokens: k ee → vowel_run: ee (level 2; class e)
- raiin: tokens: r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- ail: tokens: a i l → connectors: l → vowel_run: a (level 1; class a)
- odaiin: tokens: o p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `odaiin` → `inopia`; poverty)

<a id="f108r-14-f108r-14-p0"></a>

### f108r.14,+P0

EVA: polchal shol qokar shedy pcholy qokal opchdy ofal shor qokaiin otalod

Direct Gloss (Procedural, Not a Real Translation):
- polchal: tokens: p o l ch a l → connectors: l l → vowel_run: a (level 1; class a)
- shol: tokens: sh o l → connectors: l
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qokar` → `carco`; [n/a])
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- pcholy: tokens: p ch o l → connectors: l
- qokal: tokens: qo k a l → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qokal` → `calco`; cast (of sculpture))
- opchdy: tokens: o p ch p
- ofal: tokens: o f a l → connectors: l → vowel_run: a (level 1; class a)
- shor: tokens: sh o r → connectors: r
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- otalod: tokens: o t a l o p → connectors: l → vowel_run: a (level 1; class a)

<a id="f108r-15-f108r-15-p0"></a>

### f108r.15,+P0

EVA: orain cheey chey qokeey chedy okchedy qoeedy qockhey d iirain okain

Direct Gloss (Procedural, Not a Real Translation):
- orain: tokens: o r a i n → connectors: r n → vowel_run: a (level 1; class a)
- cheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- okchedy: tokens: o k ch e p → vowel_run: e (level 1; class e)
- qoeedy: tokens: qo ee p → vowel_run: ee (level 2; class e)
- qockhey: tokens: qo ckh e → vowel_run: e (level 1; class e)
- d: tokens: p
- iirain: tokens: ii r a i n → connectors: r n → vowel_run: ii (level 2; class i)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `acino`; a berry)

<a id="f108r-16-f108r-16-p0"></a>

### f108r.16,+P0

EVA: saithy olkaiin chckhy daiin dal daiin

Direct Gloss (Procedural, Not a Real Translation):
- saithy: tokens: s a i t h → connectors: s → vowel_run: a (level 1; class a) → unmodeled_tokens: h
- olkaiin: tokens: o l k aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `olkaiin` → `caolini`; [n/a])
- chckhy: tokens: ch ckh
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- dal: tokens: p a l → connectors: l → vowel_run: a (level 1; class a)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))

<a id="f108r-17-f108r-17-p0"></a>

### f108r.17,+P0

EVA: fcheokair okedaiin chedy qokeed okain chdy laiin ofar chedy tedam

Direct Gloss (Procedural, Not a Real Translation):
- fcheokair: tokens: f ch e o k a i r → connectors: r → vowel_run: e (level 1; class e)
- okedaiin: tokens: o k e p aiin → vowel_run: e (level 1; class e) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokeed: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `acino`; a berry)
- chdy: tokens: ch p
- laiin: tokens: l aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin
- ofar: tokens: o f a r → connectors: r → vowel_run: a (level 1; class a)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- tedam: tokens: t e p a m → connectors: m → vowel_run: e (level 1; class e)

<a id="f108r-18-f108r-18-p0"></a>

### f108r.18,+P0

EVA: okeedy lkal daiin ykchedy qokol chedy qokedy lkedy okalo l chedl y

Direct Gloss (Procedural, Not a Real Translation):
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- lkal: tokens: l k a l → connectors: l l → vowel_run: a (level 1; class a)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- ykchedy: tokens: k ch e p → vowel_run: e (level 1; class e)
- qokol: tokens: qo k o l → connectors: l
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- lkedy: tokens: l k e p → connectors: l → vowel_run: e (level 1; class e)
- okalo: tokens: o k a l o → connectors: l → vowel_run: a (level 1; class a)
- l: tokens: l → connectors: l
- chedl: tokens: ch e p l → connectors: l → vowel_run: e (level 1; class e)
- y: [unparsed]

<a id="f108r-19-f108r-19-p0"></a>

### f108r.19,+P0

EVA: dchedy okeedar shchy okol kedy okeedy chal raiin otedy chtal om

Direct Gloss (Procedural, Not a Real Translation):
- dchedy: tokens: p ch e p → vowel_run: e (level 1; class e)
- okeedar: tokens: o k ee p a r → connectors: r → vowel_run: ee (level 2; class e)
- shchy: tokens: sh ch
- okol: tokens: o k o l → connectors: l
- kedy: tokens: k e p → vowel_run: e (level 1; class e)
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- chal: tokens: ch a l → connectors: l → vowel_run: a (level 1; class a)
- raiin: tokens: r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- chtal: tokens: ch t a l → connectors: l → vowel_run: a (level 1; class a)
- om: tokens: o m → connectors: m

<a id="f108r-20-f108r-20-p0"></a>

### f108r.20,+P0

EVA: dain chey qokeedy cholcheey dalkar okedy

Direct Gloss (Procedural, Not a Real Translation):
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- cholcheey: tokens: ch o l ch ee → connectors: l → vowel_run: ee (level 2; class e)
- dalkar: tokens: p a l k a r → connectors: l r → vowel_run: a (level 1; class a)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)

<a id="f108r-21-f108r-21-p0"></a>

### f108r.21,+P0

EVA: qolshy qoeedy lkealshedy shokal keedy qotedy qopchedy otal dkedam

Direct Gloss (Procedural, Not a Real Translation):
- qolshy: tokens: qo l sh → connectors: l
- qoeedy: tokens: qo ee p → vowel_run: ee (level 2; class e)
- lkealshedy: tokens: l k e a l sh e p → connectors: l l → vowel_run: e (level 1; class e)
- shokal: tokens: sh o k a l → connectors: l → vowel_run: a (level 1; class a)
- keedy: tokens: k ee p → vowel_run: ee (level 2; class e)
- qotedy: tokens: qo t e p → vowel_run: e (level 1; class e)
- qopchedy: tokens: qo p ch e p → vowel_run: e (level 1; class e)
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- dkedam: tokens: p k e p a m → connectors: m → vowel_run: e (level 1; class e)

<a id="f108r-22-f108r-22-p0"></a>

### f108r.22,+P0

EVA: dcheol shol dal qokaiin otal ol shedy qokey chey lor aiin okeeam

Direct Gloss (Procedural, Not a Real Translation):
- dcheol: tokens: p ch e o l → connectors: l → vowel_run: e (level 1; class e)
- shol: tokens: sh o l → connectors: l
- dal: tokens: p a l → connectors: l → vowel_run: a (level 1; class a)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- ol: tokens: o l → connectors: l
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- lor: tokens: l o r → connectors: l r
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okeeam: tokens: o k ee a m → connectors: m → vowel_run: ee (level 2; class e)

<a id="f108r-23-f108r-23-p0"></a>

### f108r.23,+P0

EVA: ykeol chey okain charain

Direct Gloss (Procedural, Not a Real Translation):
- ykeol: tokens: k e o l → connectors: l → vowel_run: e (level 1; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `acino`; a berry)
- charain: tokens: ch a r a i n → connectors: r n → vowel_run: a (level 1; class a)

<a id="f108r-24-f108r-24-p0"></a>

### f108r.24,+P0

EVA: folaiin c'hkhy qokal lkedy qotedy qoked qotedy okal chdar al char aiin

Direct Gloss (Procedural, Not a Real Translation):
- folaiin: tokens: f o l aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `olaiin` → `ialino`; hyaline, glassy)
- c: tokens: c
- hkhy: tokens: h k h → unmodeled_tokens: h
- qokal: tokens: qo k a l → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qokal` → `calco`; cast (of sculpture))
- lkedy: tokens: l k e p → connectors: l → vowel_run: e (level 1; class e)
- qotedy: tokens: qo t e p → vowel_run: e (level 1; class e)
- qoked: tokens: qo k e p → vowel_run: e (level 1; class e)
- qotedy: tokens: qo t e p → vowel_run: e (level 1; class e)
- okal: tokens: o k a l → connectors: l → vowel_run: a (level 1; class a)
- chdar: tokens: ch p a r → connectors: r → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin

<a id="f108r-25-f108r-25-p0"></a>

### f108r.25,+P0

EVA: y cheeor aiin okeey cheol chedy opchsd opchedy alkedy al lkeam ol rain al

Direct Gloss (Procedural, Not a Real Translation):
- y: [unparsed]
- cheeor: tokens: ch ee o r → connectors: r → vowel_run: ee (level 2; class e)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- cheol: tokens: ch e o l → connectors: l → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- opchsd: tokens: o p ch s p → connectors: s
- opchedy: tokens: o p ch e p → vowel_run: e (level 1; class e)
- alkedy: tokens: a l k e p → connectors: l → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- lkeam: tokens: l k e a m → connectors: l m → vowel_run: e (level 1; class e)
- ol: tokens: o l → connectors: l
- rain: tokens: r a i n → connectors: r n → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)

<a id="f108r-26-f108r-26-p0"></a>

### f108r.26,+P0

EVA: dair okal chedy qokeedy oteor al qokedy qokedy otaiin otedar chalkeedy

Direct Gloss (Procedural, Not a Real Translation):
- dair: tokens: p a i r → connectors: r → vowel_run: a (level 1; class a)
- okal: tokens: o k a l → connectors: l → vowel_run: a (level 1; class a)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- oteor: tokens: o t e o r → connectors: r → vowel_run: e (level 1; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- otedar: tokens: o t e p a r → connectors: r → vowel_run: e (level 1; class e)
- chalkeedy: tokens: ch a l k ee p → connectors: l → vowel_run: a (level 1; class a)

<a id="f108r-27-f108r-27-p0"></a>

### f108r.27,+P0

EVA: ychedain orcheory qoaiin okeey qokeey chdal okedy qokedy okedam chdy

Direct Gloss (Procedural, Not a Real Translation):
- ychedain: tokens: ch e p a i n → connectors: n → vowel_run: e (level 1; class e)
- orcheory: tokens: o r ch e o r → connectors: r r → vowel_run: e (level 1; class e)
- qoaiin: tokens: qo aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- chdal: tokens: ch p a l → connectors: l → vowel_run: a (level 1; class a)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- okedam: tokens: o k e p a m → connectors: m → vowel_run: e (level 1; class e)
- chdy: tokens: ch p

<a id="f108r-28-f108r-28-p0"></a>

### f108r.28,+P0

EVA: sain arolkeey qoal olkedy okaiin

Direct Gloss (Procedural, Not a Real Translation):
- sain: tokens: s a i n → connectors: s n → vowel_run: a (level 1; class a)
- arolkeey: tokens: a r o l k ee → connectors: r l → vowel_run: a (level 1; class a)
- qoal: tokens: qo a l → connectors: l → vowel_run: a (level 1; class a)
- olkedy: tokens: o l k e p → connectors: l → vowel_run: e (level 1; class e)
- okaiin: tokens: o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])

<a id="f108r-29-f108r-29-p0"></a>

### f108r.29,+P0

EVA: polchedy okedy qopchedy okeolkey teedy qotaiin otedy okeedy otedam

Direct Gloss (Procedural, Not a Real Translation):
- polchedy: tokens: p o l ch e p → connectors: l → vowel_run: e (level 1; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- qopchedy: tokens: qo p ch e p → vowel_run: e (level 1; class e)
- okeolkey: tokens: o k e o l k e → connectors: l → vowel_run: e (level 1; class e)
- teedy: tokens: t ee p → vowel_run: ee (level 2; class e)
- qotaiin: tokens: qo t aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qotaiin` → `cationi`; [n/a])
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- otedam: tokens: o t e p a m → connectors: m → vowel_run: e (level 1; class e)

<a id="f108r-30-f108r-30-p0"></a>

### f108r.30,+P0

EVA: ykeol chedal chokedy o qokedy okchedy saiin alkan

Direct Gloss (Procedural, Not a Real Translation):
- ykeol: tokens: k e o l → connectors: l → vowel_run: e (level 1; class e)
- chedal: tokens: ch e p a l → connectors: l → vowel_run: e (level 1; class e)
- chokedy: tokens: ch o k e p → vowel_run: e (level 1; class e)
- o: tokens: o
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- okchedy: tokens: o k ch e p → vowel_run: e (level 1; class e)
- saiin: tokens: s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `saiin` → `asini`; [n/a])
- alkan: tokens: a l k a n → connectors: l n → vowel_run: a (level 1; class a)

<a id="f108r-31-f108r-31-p0"></a>

### f108r.31,+P0

EVA: keol sheody qokedy qokeedy qokoy qoiiedy otedy qokedy qokedy qokal

Direct Gloss (Procedural, Not a Real Translation):
- keol: tokens: k e o l → connectors: l → vowel_run: e (level 1; class e)
- sheody: tokens: sh e o p → vowel_run: e (level 1; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qokoy: tokens: qo k o
- qoiiedy: tokens: qo ii e p → vowel_run: ii (level 2; class i)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- qokal: tokens: qo k a l → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qokal` → `calco`; cast (of sculpture))

<a id="f108r-32-f108r-32-p0"></a>

### f108r.32,+P0

EVA: ykedar chedy qokey lkeedy otedy [o:y]kedy otedy otal shol alodar or alold

Direct Gloss (Procedural, Not a Real Translation):
- ykedar: tokens: k e p a r → connectors: r → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- o: tokens: o
- y: [unparsed]
- kedy: tokens: k e p → vowel_run: e (level 1; class e)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- shol: tokens: sh o l → connectors: l
- alodar: tokens: a l o p a r → connectors: l r → vowel_run: a (level 1; class a)
- or: tokens: o r → connectors: r
- alold: tokens: a l o l p → connectors: l l → vowel_run: a (level 1; class a)

<a id="f108r-33-f108r-33-p0"></a>

### f108r.33,+P0

EVA: solkedy okchd qokedy okedy okeol chdy okar okedy okedy chlar

Direct Gloss (Procedural, Not a Real Translation):
- solkedy: tokens: s o l k e p → connectors: s l → vowel_run: e (level 1; class e)
- okchd: tokens: o k ch p
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- okeol: tokens: o k e o l → connectors: l → vowel_run: e (level 1; class e)
- chdy: tokens: ch p
- okar: tokens: o k a r → connectors: r → vowel_run: a (level 1; class a)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- chlar: tokens: ch l a r → connectors: l r → vowel_run: a (level 1; class a)

<a id="f108r-34-f108r-34-p0"></a>

### f108r.34,+P0

EVA: ycheol chckhy qokedy okain

Direct Gloss (Procedural, Not a Real Translation):
- ycheol: tokens: ch e o l → connectors: l → vowel_run: e (level 1; class e)
- chckhy: tokens: ch ckh
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `acino`; a berry)

<a id="f108r-35-f108r-35-p0"></a>

### f108r.35,+P0

EVA: pychory opchey lkeol chdar shedy qofchdy opalal cheedy qokalchdy opchdy

Direct Gloss (Procedural, Not a Real Translation):
- pychory: tokens: p ch o r → connectors: r
- opchey: tokens: o p ch e → vowel_run: e (level 1; class e)
- lkeol: tokens: l k e o l → connectors: l l → vowel_run: e (level 1; class e)
- chdar: tokens: ch p a r → connectors: r → vowel_run: a (level 1; class a)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qofchdy: tokens: qo f ch p
- opalal: tokens: o p a l a l → connectors: l l → vowel_run: a (level 1; class a)
- cheedy: tokens: ch ee p → vowel_run: ee (level 2; class e)
- qokalchdy: tokens: qo k a l ch p → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qokal` → `calco`; cast (of sculpture))
- opchdy: tokens: o p ch p

<a id="f108r-36-f108r-36-p0"></a>

### f108r.36,+P0

EVA: okechy okeol okeedy cheedar okedy qotal chdy chedal chedy

Direct Gloss (Procedural, Not a Real Translation):
- okechy: tokens: o k e ch → vowel_run: e (level 1; class e)
- okeol: tokens: o k e o l → connectors: l → vowel_run: e (level 1; class e)
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- cheedar: tokens: ch ee p a r → connectors: r → vowel_run: ee (level 2; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- qotal: tokens: qo t a l → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qotal` → `colta`; [n/a])
- chdy: tokens: ch p
- chedal: tokens: ch e p a l → connectors: l → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)

<a id="f108r-37-f108r-37-p0"></a>

### f108r.37,+P0

EVA: tchedy qokey okeey lkeey chedy cheokeey qokeey chedl al lkair alkeedy ram

Direct Gloss (Procedural, Not a Real Translation):
- tchedy: tokens: t ch e p → vowel_run: e (level 1; class e)
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- cheokeey: tokens: ch e o k ee → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- chedl: tokens: ch e p l → connectors: l → vowel_run: e (level 1; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- lkair: tokens: l k a i r → connectors: l r → vowel_run: a (level 1; class a)
- alkeedy: tokens: a l k ee p → connectors: l → vowel_run: a (level 1; class a)
- ram: tokens: r a m → connectors: r m → vowel_run: a (level 1; class a)

<a id="f108r-38-f108r-38-p0"></a>

### f108r.38,+P0

EVA: dcheeody cheedy qokeedy otedy qeeey chey

Direct Gloss (Procedural, Not a Real Translation):
- dcheeody: tokens: p ch ee o p → vowel_run: ee (level 2; class e)
- cheedy: tokens: ch ee p → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- qeeey: tokens: q eee → vowel_run: eee (level 3; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)

<a id="f108r-39-f108r-39-p0"></a>

### f108r.39,+P0

EVA: poro l tedo lchdain qokar otedy otarar yty sho qotedy oteedy okedy

Direct Gloss (Procedural, Not a Real Translation):
- poro: tokens: p o r o → connectors: r
- l: tokens: l → connectors: l
- tedo: tokens: t e p o → vowel_run: e (level 1; class e)
- lchdain: tokens: l ch p a i n → connectors: l n → vowel_run: a (level 1; class a)
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qokar` → `carco`; [n/a])
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- otarar: tokens: o t a r a r → connectors: r r → vowel_run: a (level 1; class a)
- yty: tokens: t
- sho: tokens: sh o
- qotedy: tokens: qo t e p → vowel_run: e (level 1; class e)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)

<a id="f108r-40-f108r-40-p0"></a>

### f108r.40,+P0

EVA: okor aiin okeedy qokeedy okeeom cheykeeed chol qekeey or aiin chckhom

Direct Gloss (Procedural, Not a Real Translation):
- okor: tokens: o k o r → connectors: r
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- okeeom: tokens: o k ee o m → connectors: m → vowel_run: ee (level 2; class e)
- cheykeeed: tokens: ch e k eee p → vowel_run: e (level 1; class e)
- chol: tokens: ch o l → connectors: l
- qekeey: tokens: q e k ee → vowel_run: e (level 1; class e)
- or: tokens: o r → connectors: r
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chckhom: tokens: ch ckh o m → connectors: m

<a id="f108r-41-f108r-41-p0"></a>

### f108r.41,+P0

EVA: sheedain qokeedy chokeedy qoteody keesho daiin chedy keedal as keodky

Direct Gloss (Procedural, Not a Real Translation):
- sheedain: tokens: sh ee p a i n → connectors: n → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- chokeedy: tokens: ch o k ee p → vowel_run: ee (level 2; class e)
- qoteody: tokens: qo t e o p → vowel_run: e (level 1; class e)
- keesho: tokens: k ee sh o → vowel_run: ee (level 2; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `daiin` → `piani`; plans (arrangements))
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- keedal: tokens: k ee p a l → connectors: l → vowel_run: ee (level 2; class e)
- as: tokens: a s → connectors: s → vowel_run: a (level 1; class a)
- keodky: tokens: k e o p k → vowel_run: e (level 1; class e)

<a id="f108r-42-f108r-42-p0"></a>

### f108r.42,+P0

EVA: okeey qokeey qokeedy qokeey chedal chedy qokeey okeedain otain oolals

Direct Gloss (Procedural, Not a Real Translation):
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- chedal: tokens: ch e p a l → connectors: l → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- okeedain: tokens: o k ee p a i n → connectors: n → vowel_run: ee (level 2; class e)
- otain: tokens: o t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `otain` → `anito`; [n/a])
- oolals: tokens: o o l a l s → connectors: l l s → vowel_run: a (level 1; class a)

<a id="f108r-43-f108r-43-p0"></a>

### f108r.43,+P0

EVA: olke?y okey lokeey qokedy ol kechy chedar olal oedy qckhy cheam

Direct Gloss (Procedural, Not a Real Translation):
- olke: tokens: o l k e → connectors: l → vowel_run: e (level 1; class e)
- y: [unparsed]
- okey: tokens: o k e → vowel_run: e (level 1; class e)
- lokeey: tokens: l o k ee → connectors: l → vowel_run: ee (level 2; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- ol: tokens: o l → connectors: l
- kechy: tokens: k e ch → vowel_run: e (level 1; class e)
- chedar: tokens: ch e p a r → connectors: r → vowel_run: e (level 1; class e) (lexicon-context: `chedar` → `capre`; [n/a])
- olal: tokens: o l a l → connectors: l l → vowel_run: a (level 1; class a)
- oedy: tokens: o e p → vowel_run: e (level 1; class e)
- qckhy: tokens: q ckh
- cheam: tokens: ch e a m → connectors: m → vowel_run: e (level 1; class e)

<a id="f108r-44-f108r-44-p0"></a>

### f108r.44,+P0

EVA: ychey chey okar charam otam cheody

Direct Gloss (Procedural, Not a Real Translation):
- ychey: tokens: ch e → vowel_run: e (level 1; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- okar: tokens: o k a r → connectors: r → vowel_run: a (level 1; class a)
- charam: tokens: ch a r a m → connectors: r m → vowel_run: a (level 1; class a)
- otam: tokens: o t a m → connectors: m → vowel_run: a (level 1; class a)
- cheody: tokens: ch e o p → vowel_run: e (level 1; class e)

<a id="f108r-45-f108r-45-p0"></a>

### f108r.45,+P0

EVA: dain sheckhy okeey keey lchedy okeey qokedy chedy dalkedy qokedy dal

Direct Gloss (Procedural, Not a Real Translation):
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- sheckhy: tokens: sh e ckh → vowel_run: e (level 1; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- keey: tokens: k ee → vowel_run: ee (level 2; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- dalkedy: tokens: p a l k e p → connectors: l → vowel_run: a (level 1; class a)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- dal: tokens: p a l → connectors: l → vowel_run: a (level 1; class a)

<a id="f108r-46-f108r-46-p0"></a>

### f108r.46,+P0

EVA: yteedy qokeed y qokey otedy okedy qokaiin cheodar cheey qokedy al

Direct Gloss (Procedural, Not a Real Translation):
- yteedy: tokens: t ee p → vowel_run: ee (level 2; class e)
- qokeed: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- y: [unparsed]
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `ciancio`; [n/a])
- cheodar: tokens: ch e o p a r → connectors: r → vowel_run: e (level 1; class e)
- cheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)

<a id="f108r-47-f108r-47-p0"></a>

### f108r.47,+P0

EVA: yched chockhey dain sheol okedy lkeedy oteedy qokeedy ched al keam

Direct Gloss (Procedural, Not a Real Translation):
- yched: tokens: ch e p → vowel_run: e (level 1; class e)
- chockhey: tokens: ch o ckh e → vowel_run: e (level 1; class e)
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- sheol: tokens: sh e o l → connectors: l → vowel_run: e (level 1; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- ched: tokens: ch e p → vowel_run: e (level 1; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- keam: tokens: k e a m → connectors: m → vowel_run: e (level 1; class e)

<a id="f108r-48-f108r-48-p0"></a>

### f108r.48,+P0

EVA: pcheedy qokeo l okeol keol dain ar otedy qokedy qokeedy qokeedy lchdy

Direct Gloss (Procedural, Not a Real Translation):
- pcheedy: tokens: p ch ee p → vowel_run: ee (level 2; class e)
- qokeo: tokens: qo k e o → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- okeol: tokens: o k e o l → connectors: l → vowel_run: e (level 1; class e)
- keol: tokens: k e o l → connectors: l → vowel_run: e (level 1; class e)
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- lchdy: tokens: l ch p → connectors: l

<a id="f108r-49-f108r-49-p0"></a>

### f108r.49,+P0

EVA: oteedy lchedy qokeedy qokeedy qokeo lchedy qokey qokeey lkedy lkedy

Direct Gloss (Procedural, Not a Real Translation):
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qokeo: tokens: qo k e o → vowel_run: e (level 1; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- lkedy: tokens: l k e p → connectors: l → vowel_run: e (level 1; class e)
- lkedy: tokens: l k e p → connectors: l → vowel_run: e (level 1; class e)

<a id="f108r-50-f108r-50-p0"></a>

### f108r.50,+P0

EVA: qokeedy lchedy lkaiin chdy qokey okedy okaram chdar ar okam

Direct Gloss (Procedural, Not a Real Translation):
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- lkaiin: tokens: l k aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `lkaiin` → `ancili`; [n/a])
- chdy: tokens: ch p
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- okaram: tokens: o k a r a m → connectors: r m → vowel_run: a (level 1; class a)
- chdar: tokens: ch p a r → connectors: r → vowel_run: a (level 1; class a)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- okam: tokens: o k a m → connectors: m → vowel_run: a (level 1; class a)
