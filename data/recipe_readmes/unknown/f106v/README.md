# Voynich Speculative Procedural Protocol — f106v

IMPORTANT: this is NOT a real or validated translation of the Voynich Manuscript. It is a speculative/procedural model that interprets EVA using a user-defined grammar to generate experimental recipes using safe, known edible substitutes.

This file is generated automatically from IVTFF/EVA transliteration plus a user-defined procedural grammar.

![f106v](../../../images/q20/f106v_crd.jpg)

## Page / Folio
- currier: B
- folio: f106v
- page_number: 219

## EVA Text (Transliteration)
```text
fsheda looin opaiiral oteodaiin chopchy otair kar alalor aiin aly kar
daiin al sheeodar y chtain char otar qokar o[t:k]ar shed sheo keorain amchy
yshedaiin shckhy cheokchy sar al cho lchedy ytain otar al chdy daly lody
ycheoto lsheo aiin chcthy okain chdal chdam charam
tshoar oeey qokain shypchedy opched qopchedy otaiin chepar aiin octhy dair ry
daiin shody kchedy sheody olkaiin shkar chody talshdy qokain kararo
soy she[a:o]lsho dy chain shol kain shokaiin qotaiin chodar ail dal dar loror
oraiin cheo rol aiin otaiin
tshodaiin sholkair orainkar aiin shtchy qopchdy qopchy lolkair shear am
ysheody cheol oteey qodaiin ytain ychdy oltydy
tchdys arshedy oteeody kshedy qotchdy qotar shedy qotedy opchy kaiin sham
ycheey qoeeda chodain otalchdy qokaiin chokaiin chody qokal otal otam
olkeealkchedy qoeeey ral ches al ytar shsy lchey ykaiin shy lkam
schedy raiin char arshey chedy aiiin alkam
kcheo[a:y]kar shedy qofchdy otshedy qokchdy qotshdy qokchdy kair alody
sarar okear aiin chotal shody qotchdy qotar shedy chodal chedy qotam
salar she[o:a]dar okain qodaiin chodal
pshodalos qokshdy qokshy opchdy qokshd ar shdar oeedy qotaly dairy
darshy otaiin otal chedy r ain olaiin otaiin sain
polchs lkaiir shokar choefy shor qokor sheeo kolch'es okcharain
ycheo lkaiin cheo lchdaiin osaiin okal sheedy qokchedy
tshod qokchy qokaiin okshy qokar shedy shey qopshedy shdykairylam
ol kalol shar chor okal chdy chol chedy qotaiin or aiin okeedy qokain
soraiin solshedy qokchdy qokol chdaiin chdal air odl charain okaifhy
sar sheol chol shar chol okarol
tar air kshdain okal chdy lchedy kshar chopchy otches aral opchdy
olcheo odaiin sheotal shoor qokeey oarar
psheody olkeedy qokor sheos choty qotaiin oteody otaiin qokar otydy
ycheey olchey chedy qotaiin cheos otaiin otain chotar olos aiin cheog
otaiin okochey qody oeesysarx [o:y]keey oteedy ksheody qokeedy qotokody
sheeyko shody chosaiin olcham
polos shdair sheky keedy qoeedy qofchedy sair al pchedy ypodaiin saram
choaiin ody qotar chey ol aiin chey dychy
pchodaiin kcheeor al ky shcthy otos ar odal pcheody qotaiin otor alodam
shody shockhy qokeey qokcho l kchedy shdy kchedy chedy qockhey okaiin al
sol sheedy qokaiin cholkaiin
pyoaly cheo aiir al kcheodar qodaiin shcphoor shedy otedar opol fchedy odr
okcheody qokeey otchey qokeey qokeedal qotchkeey qol rody raiin oty
qokaiin chey kar oteol
ko sheody qody ches aiin dair air opchedy pchdy kolchedy tcheodar podkor
otedy qoar cheey kaiin qokeeody okaiin oteey qoteedy choty
py chal shedy qoteedy cheyky okedaiin otedy odaiin chdy chy keedy lkey
chedy okeey qokeedy okeey sheey qokaiin al al kalos chedy dkaiin chcthy
cthey chey okal chey keeyrain okeeo otaiin chedar oteedy otol oty
ysair okeey cheodain chey tchar oqotaiin oty rasal oteey sar aildy
ytar okain cheokaiin chedy okeeey chkaiin ol oky raiin cheoar chos
dcheoaiin shky
```

## Domain Context (Heuristic; Not a Translation)

This section summarizes recurring **basewords** in this IVTFF domain and shows simple substring evidence that the token markers used by the procedural grammar occur inside frequent words.

Any Italian anagram / English gloss is a best-effort lexicon match, not a decipherment.



### Associated basewords (non-generic; top by frequency in this domain)
- `paiin` (count=241) → Italian anagram `piani`; English: plans (arrangements)
- `qokaiin` (count=122) → Italian anagram `ciancio`; English: [n/a]
- `okaiin` (count=109) → Italian anagram `coniai`; English: [n/a]
- `qokain` (count=101) → Italian anagram `acconi`; English: [n/a]
- `okain` (count=69) → Italian anagram `acino`; English: a berry
- `qokep` (count=65) → Italian anagram `pecco`; English: [n/a]
- `otain` (count=54) → Italian anagram `anito`; English: [n/a]
- `qokar` (count=48) → Italian anagram `carco`; English: [n/a]
- `saiin` (count=48) → Italian anagram `asini`; English: [n/a]
- `qokal` (count=46) → Italian anagram `calco`; English: cast (of sculpture)
- `kaiin` (count=45) → Italian anagram `acini`; English: [n/a]
- `qotaiin` (count=40) → Italian anagram `cationi`; English: [n/a]
- `lkaiin` (count=40) → Italian anagram `ancili`; English: [n/a]
- `qokeol` (count=38) → Italian anagram `eccolo`; English: [n/a]
- `qotain` (count=34) → Italian anagram `antico`; English: ancient

### Marker evidence (substring in frequent basewords)
- `qo`: 63 basewords; examples: `qokee`, `qokeep`, `qokaiin`, `qokain`, `qokep`, `qoke`
- `q`: 64 basewords; examples: `qokee`, `qokeep`, `qokaiin`, `qokain`, `qokep`, `qoke`
- `o`: 281 basewords; examples: `qokee`, `ol`, `o`, `qokeep`, `okee`, `qokaiin`
- `k`: 150 basewords; examples: `qokee`, `qokeep`, `okee`, `qokaiin`, `okaiin`, `qokain`
- `t`: 100 basewords; examples: `otaiin`, `otee`, `otal`, `otar`, `oteep`, `otep`
- `p`: 154 basewords; examples: `paiin`, `chep`, `qokeep`, `shep`, `par`, `oteep`
- `ch`: 144 basewords; examples: `chep`, `che`, `chol`, `chee`, `cheol`, `cheo`
- `sh`: 52 basewords; examples: `shep`, `she`, `shee`, `sheol`, `sheep`, `shol`
- `f`: 2 basewords; examples: `fchep`, `f`
- `cth`: 17 basewords; examples: `chcth`, `cthe`, `shcth`, `checth`, `cthol`, `cthee`
- `ckh`: 18 basewords; examples: `chckh`, `shckh`, `checkh`, `chckhe`, `chockh`, `sheckh`
- `cph`: 3 basewords; examples: `cphol`, `cph`, `cphe`
- `iin`: 38 basewords; examples: `aiin`, `paiin`, `qokaiin`, `okaiin`, `otaiin`, `saiin`
- `aiin`: 31 basewords; examples: `aiin`, `paiin`, `qokaiin`, `okaiin`, `otaiin`, `saiin`

## Recipes Index (This Page)
- [f106v.1,@P0](#f106v-1-f106v-1-p0)
- [f106v.2,+P0](#f106v-2-f106v-2-p0)
- [f106v.3,+P0](#f106v-3-f106v-3-p0)
- [f106v.4,+P0](#f106v-4-f106v-4-p0)
- [f106v.5,+P0](#f106v-5-f106v-5-p0)
- [f106v.6,+P0](#f106v-6-f106v-6-p0)
- [f106v.7,+P0](#f106v-7-f106v-7-p0)
- [f106v.8,+P0](#f106v-8-f106v-8-p0)
- [f106v.9,+P0](#f106v-9-f106v-9-p0)
- [f106v.10,+P0](#f106v-10-f106v-10-p0)
- [f106v.11,+P0](#f106v-11-f106v-11-p0)
- [f106v.12,+P0](#f106v-12-f106v-12-p0)
- [f106v.13,+P0](#f106v-13-f106v-13-p0)
- [f106v.14,+P0](#f106v-14-f106v-14-p0)
- [f106v.15,+P0](#f106v-15-f106v-15-p0)
- [f106v.16,+P0](#f106v-16-f106v-16-p0)
- [f106v.17,+P0](#f106v-17-f106v-17-p0)
- [f106v.18,+P0](#f106v-18-f106v-18-p0)
- [f106v.19,+P0](#f106v-19-f106v-19-p0)
- [f106v.20,+P0](#f106v-20-f106v-20-p0)
- [f106v.21,+P0](#f106v-21-f106v-21-p0)
- [f106v.22,+P0](#f106v-22-f106v-22-p0)
- [f106v.23,+P0](#f106v-23-f106v-23-p0)
- [f106v.24,+P0](#f106v-24-f106v-24-p0)
- [f106v.25,+P0](#f106v-25-f106v-25-p0)
- [f106v.26,+P0](#f106v-26-f106v-26-p0)
- [f106v.27,+P0](#f106v-27-f106v-27-p0)
- [f106v.28,+P0](#f106v-28-f106v-28-p0)
- [f106v.29,+P0](#f106v-29-f106v-29-p0)
- [f106v.30,+P0](#f106v-30-f106v-30-p0)
- [f106v.31,+P0](#f106v-31-f106v-31-p0)
- [f106v.32,+P0](#f106v-32-f106v-32-p0)
- [f106v.33,+P0](#f106v-33-f106v-33-p0)
- [f106v.34,+P0](#f106v-34-f106v-34-p0)
- [f106v.35,+P0](#f106v-35-f106v-35-p0)
- [f106v.36,+P0](#f106v-36-f106v-36-p0)
- [f106v.37,+P0](#f106v-37-f106v-37-p0)
- [f106v.38,+P0](#f106v-38-f106v-38-p0)
- [f106v.39,+P0](#f106v-39-f106v-39-p0)
- [f106v.40,+P0](#f106v-40-f106v-40-p0)
- [f106v.41,+P0](#f106v-41-f106v-41-p0)
- [f106v.42,+P0](#f106v-42-f106v-42-p0)
- [f106v.43,+P0](#f106v-43-f106v-43-p0)
- [f106v.44,+P0](#f106v-44-f106v-44-p0)
- [f106v.45,+P0](#f106v-45-f106v-45-p0)
- [f106v.46,+P0](#f106v-46-f106v-46-p0)
- [f106v.47,+P0](#f106v-47-f106v-47-p0)

## Line Glosses (Procedural Gloss Only; Not a Translation)

<a id="f106v-1-f106v-1-p0"></a>

### f106v.1,@P0

EVA (original line):
```text
fsheda looin opaiiral oteodaiin chopchy otair kar alalor aiin aly kar
```

English structural gloss (generated):

- fsheda: tokens: f sh e p a → vowel_run: e (level 1; class e)
- looin: tokens: l o o i n → connectors: l n → vowel_run: i (level 1; class i)
- opaiiral: tokens: o p a ii r a l → connectors: r l → vowel_run: a (level 1; class a)
- oteodaiin: tokens: o t e o p aiin → vowel_run: e (level 1; class e) → suffix: aiin (lexicon-context: `opaiin` → `opinai`; [n/a])
- chopchy: tokens: ch o p ch
- otair: tokens: o t a i r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `otair` → `atrio`; entrance hall, lobby (of a hotel etc.))
- kar: tokens: k a r → connectors: r → vowel_run: a (level 1; class a)
- alalor: tokens: a l a l o r → connectors: l l r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- aly: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- kar: tokens: k a r → connectors: r → vowel_run: a (level 1; class a)

<a id="f106v-2-f106v-2-p0"></a>

### f106v.2,+P0

EVA (original line):
```text
daiin al sheeodar y chtain char otar qokar o[t:k]ar shed sheo keorain amchy
```

English structural gloss (generated):

- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- sheeodar: tokens: sh ee o p a r → connectors: r → vowel_run: ee (level 2; class e)
- y: [unparsed]
- chtain: tokens: ch t a i n → connectors: n → vowel_run: a (level 1; class a)
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- otar: tokens: o t a r → connectors: r → vowel_run: a (level 1; class a)
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a)
- o: tokens: o
- t: tokens: t
- k: tokens: k
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- shed: tokens: sh e p → vowel_run: e (level 1; class e)
- sheo: tokens: sh e o → vowel_run: e (level 1; class e)
- keorain: tokens: k e o r a i n → connectors: r n → vowel_run: e (level 1; class e)
- amchy: tokens: a m ch → connectors: m → vowel_run: a (level 1; class a)

<a id="f106v-3-f106v-3-p0"></a>

### f106v.3,+P0

EVA (original line):
```text
yshedaiin shckhy cheokchy sar al cho lchedy ytain otar al chdy daly lody
```

English structural gloss (generated):

- yshedaiin: tokens: sh e p aiin → vowel_run: e (level 1; class e) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))
- shckhy: tokens: sh ckh
- cheokchy: tokens: ch e o k ch → vowel_run: e (level 1; class e)
- sar: tokens: s a r → connectors: s r → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- cho: tokens: ch o
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- ytain: tokens: t a i n → connectors: n → vowel_run: a (level 1; class a)
- otar: tokens: o t a r → connectors: r → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- chdy: tokens: ch p
- daly: tokens: p a l → connectors: l → vowel_run: a (level 1; class a)
- lody: tokens: l o p → connectors: l

<a id="f106v-4-f106v-4-p0"></a>

### f106v.4,+P0

EVA (original line):
```text
ycheoto lsheo aiin chcthy okain chdal chdam charam
```

English structural gloss (generated):

- ycheoto: tokens: ch e o t o → vowel_run: e (level 1; class e)
- lsheo: tokens: l sh e o → connectors: l → vowel_run: e (level 1; class e)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chcthy: tokens: ch cth
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `conia`; [n/a])
- chdal: tokens: ch p a l → connectors: l → vowel_run: a (level 1; class a)
- chdam: tokens: ch p a m → connectors: m → vowel_run: a (level 1; class a)
- charam: tokens: ch a r a m → connectors: r m → vowel_run: a (level 1; class a)

<a id="f106v-5-f106v-5-p0"></a>

### f106v.5,+P0

EVA (original line):
```text
tshoar oeey qokain shypchedy opched qopchedy otaiin chepar aiin octhy dair ry
```

English structural gloss (generated):

- tshoar: tokens: t sh o a r → connectors: r → vowel_run: a (level 1; class a)
- oeey: tokens: o ee → vowel_run: ee (level 2; class e)
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `concia`; tanning)
- shypchedy: tokens: sh p ch e p → vowel_run: e (level 1; class e)
- opched: tokens: o p ch e p → vowel_run: e (level 1; class e)
- qopchedy: tokens: qo p ch e p → vowel_run: e (level 1; class e)
- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chepar: tokens: ch e p a r → connectors: r → vowel_run: e (level 1; class e) (lexicon-context: `chepar` → `capre`; [n/a])
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- octhy: tokens: o cth
- dair: tokens: p a i r → connectors: r → vowel_run: a (level 1; class a)
- ry: tokens: r → connectors: r

<a id="f106v-6-f106v-6-p0"></a>

### f106v.6,+P0

EVA (original line):
```text
daiin shody kchedy sheody olkaiin shkar chody talshdy qokain kararo
```

English structural gloss (generated):

- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))
- shody: tokens: sh o p
- kchedy: tokens: k ch e p → vowel_run: e (level 1; class e)
- sheody: tokens: sh e o p → vowel_run: e (level 1; class e)
- olkaiin: tokens: o l k aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `lkaiin` → `canili`; [n/a])
- shkar: tokens: sh k a r → connectors: r → vowel_run: a (level 1; class a)
- chody: tokens: ch o p
- talshdy: tokens: t a l sh p → connectors: l → vowel_run: a (level 1; class a)
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `concia`; tanning)
- kararo: tokens: k a r a r o → connectors: r r → vowel_run: a (level 1; class a)

<a id="f106v-7-f106v-7-p0"></a>

### f106v.7,+P0

EVA (original line):
```text
soy she[a:o]lsho dy chain shol kain shokaiin qotaiin chodar ail dal dar loror
```

English structural gloss (generated):

- soy: tokens: s o → connectors: s
- she: tokens: sh e → vowel_run: e (level 1; class e)
- a: tokens: a → vowel_run: a (level 1; class a)
- o: tokens: o
- lsho: tokens: l sh o → connectors: l
- dy: tokens: p
- chain: tokens: ch a i n → connectors: n → vowel_run: a (level 1; class a)
- shol: tokens: sh o l → connectors: l
- kain: tokens: k a i n → connectors: n → vowel_run: a (level 1; class a)
- shokaiin: tokens: sh o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- qotaiin: tokens: qo t aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qotaiin` → `coniati`; [n/a])
- chodar: tokens: ch o p a r → connectors: r → vowel_run: a (level 1; class a)
- ail: tokens: a i l → connectors: l → vowel_run: a (level 1; class a)
- dal: tokens: p a l → connectors: l → vowel_run: a (level 1; class a)
- dar: tokens: p a r → connectors: r → vowel_run: a (level 1; class a)
- loror: tokens: l o r o r → connectors: l r r

<a id="f106v-8-f106v-8-p0"></a>

### f106v.8,+P0

EVA (original line):
```text
oraiin cheo rol aiin otaiin
```

English structural gloss (generated):

- oraiin: tokens: o r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `oraiin` → `ironia`; irony)
- cheo: tokens: ch e o → vowel_run: e (level 1; class e)
- rol: tokens: r o l → connectors: r l
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin

<a id="f106v-9-f106v-9-p0"></a>

### f106v.9,+P0

EVA (original line):
```text
tshodaiin sholkair orainkar aiin shtchy qopchdy qopchy lolkair shear am
```

English structural gloss (generated):

- tshodaiin: tokens: t sh o p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `opaiin` → `opinai`; [n/a])
- sholkair: tokens: sh o l k a i r → connectors: l r → vowel_run: a (level 1; class a)
- orainkar: tokens: o r a i n k a r → connectors: r n r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- shtchy: tokens: sh t ch
- qopchdy: tokens: qo p ch p
- qopchy: tokens: qo p ch
- lolkair: tokens: l o l k a i r → connectors: l l r → vowel_run: a (level 1; class a)
- shear: tokens: sh e a r → connectors: r → vowel_run: e (level 1; class e)
- am: tokens: a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f106v-10-f106v-10-p0"></a>

### f106v.10,+P0

EVA (original line):
```text
ysheody cheol oteey qodaiin ytain ychdy oltydy
```

English structural gloss (generated):

- ysheody: tokens: sh e o p → vowel_run: e (level 1; class e)
- cheol: tokens: ch e o l → connectors: l → vowel_run: e (level 1; class e)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- qodaiin: tokens: qo p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- ytain: tokens: t a i n → connectors: n → vowel_run: a (level 1; class a)
- ychdy: tokens: ch p
- oltydy: tokens: o l t p → connectors: l

<a id="f106v-11-f106v-11-p0"></a>

### f106v.11,+P0

EVA (original line):
```text
tchdys arshedy oteeody kshedy qotchdy qotar shedy qotedy opchy kaiin sham
```

English structural gloss (generated):

- tchdys: tokens: t ch p s → connectors: s
- arshedy: tokens: a r sh e p → connectors: r → vowel_run: a (level 1; class a)
- oteeody: tokens: o t ee o p → vowel_run: ee (level 2; class e)
- kshedy: tokens: k sh e p → vowel_run: e (level 1; class e)
- qotchdy: tokens: qo t ch p
- qotar: tokens: qo t a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qotar` → `corta`; [n/a])
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qotedy: tokens: qo t e p → vowel_run: e (level 1; class e)
- opchy: tokens: o p ch
- kaiin: tokens: k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- sham: tokens: sh a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f106v-12-f106v-12-p0"></a>

### f106v.12,+P0

EVA (original line):
```text
ycheey qoeeda chodain otalchdy qokaiin chokaiin chody qokal otal otam
```

English structural gloss (generated):

- ycheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- qoeeda: tokens: qo ee p a → vowel_run: ee (level 2; class e)
- chodain: tokens: ch o p a i n → connectors: n → vowel_run: a (level 1; class a)
- otalchdy: tokens: o t a l ch p → connectors: l → vowel_run: a (level 1; class a)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `conciai`; [n/a])
- chokaiin: tokens: ch o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- chody: tokens: ch o p
- qokal: tokens: qo k a l → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qokal` → `calco`; cast (of sculpture))
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- otam: tokens: o t a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f106v-13-f106v-13-p0"></a>

### f106v.13,+P0

EVA (original line):
```text
olkeealkchedy qoeeey ral ches al ytar shsy lchey ykaiin shy lkam
```

English structural gloss (generated):

- olkeealkchedy: tokens: o l k ee a l k ch e p → connectors: l l → vowel_run: ee (level 2; class e)
- qoeeey: tokens: qo eee → vowel_run: eee (level 3; class e)
- ral: tokens: r a l → connectors: r l → vowel_run: a (level 1; class a)
- ches: tokens: ch e s → connectors: s → vowel_run: e (level 1; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- ytar: tokens: t a r → connectors: r → vowel_run: a (level 1; class a)
- shsy: tokens: sh s → connectors: s
- lchey: tokens: l ch e → connectors: l → vowel_run: e (level 1; class e)
- ykaiin: tokens: k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- shy: tokens: sh
- lkam: tokens: l k a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f106v-14-f106v-14-p0"></a>

### f106v.14,+P0

EVA (original line):
```text
schedy raiin char arshey chedy aiiin alkam
```

English structural gloss (generated):

- schedy: tokens: s ch e p → connectors: s → vowel_run: e (level 1; class e)
- raiin: tokens: r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- arshey: tokens: a r sh e → connectors: r → vowel_run: a (level 1; class a)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- aiiin: tokens: a iii n → connectors: n → vowel_run: a (level 1; class a) → suffix: iin
- alkam: tokens: a l k a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f106v-15-f106v-15-p0"></a>

### f106v.15,+P0

EVA (original line):
```text
kcheo[a:y]kar shedy qofchdy otshedy qokchdy qotshdy qokchdy kair alody
```

English structural gloss (generated):

- kcheo: tokens: k ch e o → vowel_run: e (level 1; class e)
- a: tokens: a → vowel_run: a (level 1; class a)
- y: [unparsed]
- kar: tokens: k a r → connectors: r → vowel_run: a (level 1; class a)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qofchdy: tokens: qo f ch p
- otshedy: tokens: o t sh e p → vowel_run: e (level 1; class e)
- qokchdy: tokens: qo k ch p
- qotshdy: tokens: qo t sh p
- qokchdy: tokens: qo k ch p
- kair: tokens: k a i r → connectors: r → vowel_run: a (level 1; class a)
- alody: tokens: a l o p → connectors: l → vowel_run: a (level 1; class a)

<a id="f106v-16-f106v-16-p0"></a>

### f106v.16,+P0

EVA (original line):
```text
sarar okear aiin chotal shody qotchdy qotar shedy chodal chedy qotam
```

English structural gloss (generated):

- sarar: tokens: s a r a r → connectors: s r r → vowel_run: a (level 1; class a)
- okear: tokens: o k e a r → connectors: r → vowel_run: e (level 1; class e)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chotal: tokens: ch o t a l → connectors: l → vowel_run: a (level 1; class a)
- shody: tokens: sh o p
- qotchdy: tokens: qo t ch p
- qotar: tokens: qo t a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qotar` → `corta`; [n/a])
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- chodal: tokens: ch o p a l → connectors: l → vowel_run: a (level 1; class a)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qotam: tokens: qo t a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f106v-17-f106v-17-p0"></a>

### f106v.17,+P0

EVA (original line):
```text
salar she[o:a]dar okain qodaiin chodal
```

English structural gloss (generated):

- salar: tokens: s a l a r → connectors: s l r → vowel_run: a (level 1; class a)
- she: tokens: sh e → vowel_run: e (level 1; class e)
- o: tokens: o
- a: tokens: a → vowel_run: a (level 1; class a)
- dar: tokens: p a r → connectors: r → vowel_run: a (level 1; class a)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `conia`; [n/a])
- qodaiin: tokens: qo p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chodal: tokens: ch o p a l → connectors: l → vowel_run: a (level 1; class a)

<a id="f106v-18-f106v-18-p0"></a>

### f106v.18,+P0

EVA (original line):
```text
pshodalos qokshdy qokshy opchdy qokshd ar shdar oeedy qotaly dairy
```

English structural gloss (generated):

- pshodalos: tokens: p sh o p a l o s → connectors: l s → vowel_run: a (level 1; class a)
- qokshdy: tokens: qo k sh p
- qokshy: tokens: qo k sh
- opchdy: tokens: o p ch p
- qokshd: tokens: qo k sh p
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- shdar: tokens: sh p a r → connectors: r → vowel_run: a (level 1; class a)
- oeedy: tokens: o ee p → vowel_run: ee (level 2; class e)
- qotaly: tokens: qo t a l → connectors: l → vowel_run: a (level 1; class a) (lexicon-context: `qotal` → `colta`; [n/a])
- dairy: tokens: p a i r → connectors: r → vowel_run: a (level 1; class a)

<a id="f106v-19-f106v-19-p0"></a>

### f106v.19,+P0

EVA (original line):
```text
darshy otaiin otal chedy r ain olaiin otaiin sain
```

English structural gloss (generated):

- darshy: tokens: p a r sh → connectors: r → vowel_run: a (level 1; class a)
- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- r: tokens: r → connectors: r
- ain: tokens: a i n → connectors: n → vowel_run: a (level 1; class a)
- olaiin: tokens: o l aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin
- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- sain: tokens: s a i n → connectors: s n → vowel_run: a (level 1; class a)

<a id="f106v-20-f106v-20-p0"></a>

### f106v.20,+P0

EVA (original line):
```text
polchs lkaiir shokar choefy shor qokor sheeo kolch'es okcharain
```

English structural gloss (generated):

- polchs: tokens: p o l ch s → connectors: l s
- lkaiir: tokens: l k a ii r → connectors: l r → vowel_run: a (level 1; class a)
- shokar: tokens: sh o k a r → connectors: r → vowel_run: a (level 1; class a)
- choefy: tokens: ch o e f → vowel_run: e (level 1; class e)
- shor: tokens: sh o r → connectors: r
- qokor: tokens: qo k o r → connectors: r
- sheeo: tokens: sh ee o → vowel_run: ee (level 2; class e)
- kolch: tokens: k o l ch → connectors: l
- es: tokens: e s → connectors: s → vowel_run: e (level 1; class e)
- okcharain: tokens: o k ch a r a i n → connectors: r n → vowel_run: a (level 1; class a)

<a id="f106v-21-f106v-21-p0"></a>

### f106v.21,+P0

EVA (original line):
```text
ycheo lkaiin cheo lchdaiin osaiin okal sheedy qokchedy
```

English structural gloss (generated):

- ycheo: tokens: ch e o → vowel_run: e (level 1; class e)
- lkaiin: tokens: l k aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `lkaiin` → `canili`; [n/a])
- cheo: tokens: ch e o → vowel_run: e (level 1; class e)
- lchdaiin: tokens: l ch p aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))
- osaiin: tokens: o s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `saiin` → `asini`; [n/a])
- okal: tokens: o k a l → connectors: l → vowel_run: a (level 1; class a)
- sheedy: tokens: sh ee p → vowel_run: ee (level 2; class e)
- qokchedy: tokens: qo k ch e p → vowel_run: e (level 1; class e)

<a id="f106v-22-f106v-22-p0"></a>

### f106v.22,+P0

EVA (original line):
```text
tshod qokchy qokaiin okshy qokar shedy shey qopshedy shdykairylam
```

English structural gloss (generated):

- tshod: tokens: t sh o p
- qokchy: tokens: qo k ch
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `conciai`; [n/a])
- okshy: tokens: o k sh
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- qopshedy: tokens: qo p sh e p → vowel_run: e (level 1; class e)
- shdykairylam: tokens: sh p k a i r l a m → connectors: r l m → vowel_run: a (level 1; class a)

<a id="f106v-23-f106v-23-p0"></a>

### f106v.23,+P0

EVA (original line):
```text
ol kalol shar chor okal chdy chol chedy qotaiin or aiin okeedy qokain
```

English structural gloss (generated):

- ol: tokens: o l → connectors: l
- kalol: tokens: k a l o l → connectors: l l → vowel_run: a (level 1; class a)
- shar: tokens: sh a r → connectors: r → vowel_run: a (level 1; class a)
- chor: tokens: ch o r → connectors: r
- okal: tokens: o k a l → connectors: l → vowel_run: a (level 1; class a)
- chdy: tokens: ch p
- chol: tokens: ch o l → connectors: l
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qotaiin: tokens: qo t aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qotaiin` → `coniati`; [n/a])
- or: tokens: o r → connectors: r
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `concia`; tanning)

<a id="f106v-24-f106v-24-p0"></a>

### f106v.24,+P0

EVA (original line):
```text
soraiin solshedy qokchdy qokol chdaiin chdal air odl charain okaifhy
```

English structural gloss (generated):

- soraiin: tokens: s o r aiin → connectors: s r → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `oraiin` → `ironia`; irony)
- solshedy: tokens: s o l sh e p → connectors: s l → vowel_run: e (level 1; class e)
- qokchdy: tokens: qo k ch p
- qokol: tokens: qo k o l → connectors: l
- chdaiin: tokens: ch p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))
- chdal: tokens: ch p a l → connectors: l → vowel_run: a (level 1; class a)
- air: tokens: a i r → connectors: r → vowel_run: a (level 1; class a)
- odl: tokens: o p l → connectors: l
- charain: tokens: ch a r a i n → connectors: r n → vowel_run: a (level 1; class a)
- okaifhy: tokens: o k a i f h → vowel_run: a (level 1; class a) → unmodeled_tokens: h

<a id="f106v-25-f106v-25-p0"></a>

### f106v.25,+P0

EVA (original line):
```text
sar sheol chol shar chol okarol
```

English structural gloss (generated):

- sar: tokens: s a r → connectors: s r → vowel_run: a (level 1; class a)
- sheol: tokens: sh e o l → connectors: l → vowel_run: e (level 1; class e)
- chol: tokens: ch o l → connectors: l
- shar: tokens: sh a r → connectors: r → vowel_run: a (level 1; class a)
- chol: tokens: ch o l → connectors: l
- okarol: tokens: o k a r o l → connectors: r l → vowel_run: a (level 1; class a)

<a id="f106v-26-f106v-26-p0"></a>

### f106v.26,+P0

EVA (original line):
```text
tar air kshdain okal chdy lchedy kshar chopchy otches aral opchdy
```

English structural gloss (generated):

- tar: tokens: t a r → connectors: r → vowel_run: a (level 1; class a)
- air: tokens: a i r → connectors: r → vowel_run: a (level 1; class a)
- kshdain: tokens: k sh p a i n → connectors: n → vowel_run: a (level 1; class a)
- okal: tokens: o k a l → connectors: l → vowel_run: a (level 1; class a)
- chdy: tokens: ch p
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- kshar: tokens: k sh a r → connectors: r → vowel_run: a (level 1; class a)
- chopchy: tokens: ch o p ch
- otches: tokens: o t ch e s → connectors: s → vowel_run: e (level 1; class e)
- aral: tokens: a r a l → connectors: r l → vowel_run: a (level 1; class a)
- opchdy: tokens: o p ch p

<a id="f106v-27-f106v-27-p0"></a>

### f106v.27,+P0

EVA (original line):
```text
olcheo odaiin sheotal shoor qokeey oarar
```

English structural gloss (generated):

- olcheo: tokens: o l ch e o → connectors: l → vowel_run: e (level 1; class e)
- odaiin: tokens: o p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `opaiin` → `opinai`; [n/a])
- sheotal: tokens: sh e o t a l → connectors: l → vowel_run: e (level 1; class e)
- shoor: tokens: sh o o r → connectors: r
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- oarar: tokens: o a r a r → connectors: r r → vowel_run: a (level 1; class a)

<a id="f106v-28-f106v-28-p0"></a>

### f106v.28,+P0

EVA (original line):
```text
psheody olkeedy qokor sheos choty qotaiin oteody otaiin qokar otydy
```

English structural gloss (generated):

- psheody: tokens: p sh e o p → vowel_run: e (level 1; class e)
- olkeedy: tokens: o l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- qokor: tokens: qo k o r → connectors: r
- sheos: tokens: sh e o s → connectors: s → vowel_run: e (level 1; class e)
- choty: tokens: ch o t
- qotaiin: tokens: qo t aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qotaiin` → `coniati`; [n/a])
- oteody: tokens: o t e o p → vowel_run: e (level 1; class e)
- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a)
- otydy: tokens: o t p

<a id="f106v-29-f106v-29-p0"></a>

### f106v.29,+P0

EVA (original line):
```text
ycheey olchey chedy qotaiin cheos otaiin otain chotar olos aiin cheog
```

English structural gloss (generated):

- ycheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- olchey: tokens: o l ch e → connectors: l → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qotaiin: tokens: qo t aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qotaiin` → `coniati`; [n/a])
- cheos: tokens: ch e o s → connectors: s → vowel_run: e (level 1; class e)
- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- otain: tokens: o t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `otain` → `notai`; [n/a])
- chotar: tokens: ch o t a r → connectors: r → vowel_run: a (level 1; class a)
- olos: tokens: o l o s → connectors: l s
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- cheog: tokens: ch e o g → vowel_run: e (level 1; class e)

<a id="f106v-30-f106v-30-p0"></a>

### f106v.30,+P0

EVA (original line):
```text
otaiin okochey qody oeesysarx [o:y]keey oteedy ksheody qokeedy qotokody
```

English structural gloss (generated):

- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okochey: tokens: o k o ch e → vowel_run: e (level 1; class e)
- qody: tokens: qo p
- oeesysarx: tokens: o ee s s a r x → connectors: s s r → vowel_run: ee (level 2; class e)
- o: tokens: o
- y: [unparsed]
- keey: tokens: k ee → vowel_run: ee (level 2; class e)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- ksheody: tokens: k sh e o p → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qotokody: tokens: qo t o k o p

<a id="f106v-31-f106v-31-p0"></a>

### f106v.31,+P0

EVA (original line):
```text
sheeyko shody chosaiin olcham
```

English structural gloss (generated):

- sheeyko: tokens: sh ee k o → vowel_run: ee (level 2; class e)
- shody: tokens: sh o p
- chosaiin: tokens: ch o s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `saiin` → `asini`; [n/a])
- olcham: tokens: o l ch a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f106v-32-f106v-32-p0"></a>

### f106v.32,+P0

EVA (original line):
```text
polos shdair sheky keedy qoeedy qofchedy sair al pchedy ypodaiin saram
```

English structural gloss (generated):

- polos: tokens: p o l o s → connectors: l s
- shdair: tokens: sh p a i r → connectors: r → vowel_run: a (level 1; class a)
- sheky: tokens: sh e k → vowel_run: e (level 1; class e)
- keedy: tokens: k ee p → vowel_run: ee (level 2; class e)
- qoeedy: tokens: qo ee p → vowel_run: ee (level 2; class e)
- qofchedy: tokens: qo f ch e p → vowel_run: e (level 1; class e)
- sair: tokens: s a i r → connectors: s r → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- pchedy: tokens: p ch e p → vowel_run: e (level 1; class e)
- ypodaiin: tokens: p o p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `opaiin` → `opinai`; [n/a])
- saram: tokens: s a r a m → connectors: s r m → vowel_run: a (level 1; class a)

<a id="f106v-33-f106v-33-p0"></a>

### f106v.33,+P0

EVA (original line):
```text
choaiin ody qotar chey ol aiin chey dychy
```

English structural gloss (generated):

- choaiin: tokens: ch o aiin → vowel_run: a (level 1; class a) → suffix: aiin
- ody: tokens: o p
- qotar: tokens: qo t a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qotar` → `corta`; [n/a])
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- ol: tokens: o l → connectors: l
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- dychy: tokens: p ch

<a id="f106v-34-f106v-34-p0"></a>

### f106v.34,+P0

EVA (original line):
```text
pchodaiin kcheeor al ky shcthy otos ar odal pcheody qotaiin otor alodam
```

English structural gloss (generated):

- pchodaiin: tokens: p ch o p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- kcheeor: tokens: k ch ee o r → connectors: r → vowel_run: ee (level 2; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- ky: tokens: k
- shcthy: tokens: sh cth
- otos: tokens: o t o s → connectors: s
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- odal: tokens: o p a l → connectors: l → vowel_run: a (level 1; class a)
- pcheody: tokens: p ch e o p → vowel_run: e (level 1; class e)
- qotaiin: tokens: qo t aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qotaiin` → `coniati`; [n/a])
- otor: tokens: o t o r → connectors: r
- alodam: tokens: a l o p a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f106v-35-f106v-35-p0"></a>

### f106v.35,+P0

EVA (original line):
```text
shody shockhy qokeey qokcho l kchedy shdy kchedy chedy qockhey okaiin al
```

English structural gloss (generated):

- shody: tokens: sh o p
- shockhy: tokens: sh o ckh
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokcho: tokens: qo k ch o
- l: tokens: l → connectors: l
- kchedy: tokens: k ch e p → vowel_run: e (level 1; class e)
- shdy: tokens: sh p
- kchedy: tokens: k ch e p → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qockhey: tokens: qo ckh e → vowel_run: e (level 1; class e)
- okaiin: tokens: o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)

<a id="f106v-36-f106v-36-p0"></a>

### f106v.36,+P0

EVA (original line):
```text
sol sheedy qokaiin cholkaiin
```

English structural gloss (generated):

- sol: tokens: s o l → connectors: s l
- sheedy: tokens: sh ee p → vowel_run: ee (level 2; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `conciai`; [n/a])
- cholkaiin: tokens: ch o l k aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `lkaiin` → `canili`; [n/a])

<a id="f106v-37-f106v-37-p0"></a>

### f106v.37,+P0

EVA (original line):
```text
pyoaly cheo aiir al kcheodar qodaiin shcphoor shedy otedar opol fchedy odr
```

English structural gloss (generated):

- pyoaly: tokens: p o a l → connectors: l → vowel_run: a (level 1; class a)
- cheo: tokens: ch e o → vowel_run: e (level 1; class e)
- aiir: tokens: a ii r → connectors: r → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- kcheodar: tokens: k ch e o p a r → connectors: r → vowel_run: e (level 1; class e)
- qodaiin: tokens: qo p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- shcphoor: tokens: sh cph o o r → connectors: r
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- otedar: tokens: o t e p a r → connectors: r → vowel_run: e (level 1; class e)
- opol: tokens: o p o l → connectors: l
- fchedy: tokens: f ch e p → vowel_run: e (level 1; class e)
- odr: tokens: o p r → connectors: r

<a id="f106v-38-f106v-38-p0"></a>

### f106v.38,+P0

EVA (original line):
```text
okcheody qokeey otchey qokeey qokeedal qotchkeey qol rody raiin oty
```

English structural gloss (generated):

- okcheody: tokens: o k ch e o p → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- otchey: tokens: o t ch e → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokeedal: tokens: qo k ee p a l → connectors: l → vowel_run: ee (level 2; class e)
- qotchkeey: tokens: qo t ch k ee → vowel_run: ee (level 2; class e)
- qol: tokens: qo l → connectors: l
- rody: tokens: r o p → connectors: r
- raiin: tokens: r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- oty: tokens: o t

<a id="f106v-39-f106v-39-p0"></a>

### f106v.39,+P0

EVA (original line):
```text
qokaiin chey kar oteol
```

English structural gloss (generated):

- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `conciai`; [n/a])
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- kar: tokens: k a r → connectors: r → vowel_run: a (level 1; class a)
- oteol: tokens: o t e o l → connectors: l → vowel_run: e (level 1; class e)

<a id="f106v-40-f106v-40-p0"></a>

### f106v.40,+P0

EVA (original line):
```text
ko sheody qody ches aiin dair air opchedy pchdy kolchedy tcheodar podkor
```

English structural gloss (generated):

- ko: tokens: k o
- sheody: tokens: sh e o p → vowel_run: e (level 1; class e)
- qody: tokens: qo p
- ches: tokens: ch e s → connectors: s → vowel_run: e (level 1; class e)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- dair: tokens: p a i r → connectors: r → vowel_run: a (level 1; class a)
- air: tokens: a i r → connectors: r → vowel_run: a (level 1; class a)
- opchedy: tokens: o p ch e p → vowel_run: e (level 1; class e)
- pchdy: tokens: p ch p
- kolchedy: tokens: k o l ch e p → connectors: l → vowel_run: e (level 1; class e)
- tcheodar: tokens: t ch e o p a r → connectors: r → vowel_run: e (level 1; class e)
- podkor: tokens: p o p k o r → connectors: r

<a id="f106v-41-f106v-41-p0"></a>

### f106v.41,+P0

EVA (original line):
```text
otedy qoar cheey kaiin qokeeody okaiin oteey qoteedy choty
```

English structural gloss (generated):

- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- qoar: tokens: qo a r → connectors: r → vowel_run: a (level 1; class a)
- cheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- kaiin: tokens: k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- qokeeody: tokens: qo k ee o p → vowel_run: ee (level 2; class e)
- okaiin: tokens: o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- qoteedy: tokens: qo t ee p → vowel_run: ee (level 2; class e)
- choty: tokens: ch o t

<a id="f106v-42-f106v-42-p0"></a>

### f106v.42,+P0

EVA (original line):
```text
py chal shedy qoteedy cheyky okedaiin otedy odaiin chdy chy keedy lkey
```

English structural gloss (generated):

- py: tokens: p
- chal: tokens: ch a l → connectors: l → vowel_run: a (level 1; class a)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qoteedy: tokens: qo t ee p → vowel_run: ee (level 2; class e)
- cheyky: tokens: ch e k → vowel_run: e (level 1; class e)
- okedaiin: tokens: o k e p aiin → vowel_run: e (level 1; class e) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- odaiin: tokens: o p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `opaiin` → `opinai`; [n/a])
- chdy: tokens: ch p
- chy: tokens: ch
- keedy: tokens: k ee p → vowel_run: ee (level 2; class e)
- lkey: tokens: l k e → connectors: l → vowel_run: e (level 1; class e)

<a id="f106v-43-f106v-43-p0"></a>

### f106v.43,+P0

EVA (original line):
```text
chedy okeey qokeedy okeey sheey qokaiin al al kalos chedy dkaiin chcthy
```

English structural gloss (generated):

- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- sheey: tokens: sh ee → vowel_run: ee (level 2; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `conciai`; [n/a])
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- kalos: tokens: k a l o s → connectors: l s → vowel_run: a (level 1; class a)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- dkaiin: tokens: p k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chcthy: tokens: ch cth

<a id="f106v-44-f106v-44-p0"></a>

### f106v.44,+P0

EVA (original line):
```text
cthey chey okal chey keeyrain okeeo otaiin chedar oteedy otol oty
```

English structural gloss (generated):

- cthey: tokens: cth e → vowel_run: e (level 1; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- okal: tokens: o k a l → connectors: l → vowel_run: a (level 1; class a)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- keeyrain: tokens: k ee r a i n → connectors: r n → vowel_run: ee (level 2; class e)
- okeeo: tokens: o k ee o → vowel_run: ee (level 2; class e)
- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chedar: tokens: ch e p a r → connectors: r → vowel_run: e (level 1; class e) (lexicon-context: `chepar` → `capre`; [n/a])
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- otol: tokens: o t o l → connectors: l
- oty: tokens: o t

<a id="f106v-45-f106v-45-p0"></a>

### f106v.45,+P0

EVA (original line):
```text
ysair okeey cheodain chey tchar oqotaiin oty rasal oteey sar aildy
```

English structural gloss (generated):

- ysair: tokens: s a i r → connectors: s r → vowel_run: a (level 1; class a)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- cheodain: tokens: ch e o p a i n → connectors: n → vowel_run: e (level 1; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- tchar: tokens: t ch a r → connectors: r → vowel_run: a (level 1; class a)
- oqotaiin: tokens: o qo t aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qotaiin` → `coniati`; [n/a])
- oty: tokens: o t
- rasal: tokens: r a s a l → connectors: r s l → vowel_run: a (level 1; class a)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- sar: tokens: s a r → connectors: s r → vowel_run: a (level 1; class a)
- aildy: tokens: a i l p → connectors: l → vowel_run: a (level 1; class a)

<a id="f106v-46-f106v-46-p0"></a>

### f106v.46,+P0

EVA (original line):
```text
ytar okain cheokaiin chedy okeeey chkaiin ol oky raiin cheoar chos
```

English structural gloss (generated):

- ytar: tokens: t a r → connectors: r → vowel_run: a (level 1; class a)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `okain` → `conia`; [n/a])
- cheokaiin: tokens: ch e o k aiin → vowel_run: e (level 1; class e) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- okeeey: tokens: o k eee → vowel_run: eee (level 3; class e)
- chkaiin: tokens: ch k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- ol: tokens: o l → connectors: l
- oky: tokens: o k
- raiin: tokens: r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- cheoar: tokens: ch e o a r → connectors: r → vowel_run: e (level 1; class e)
- chos: tokens: ch o s → connectors: s

<a id="f106v-47-f106v-47-p0"></a>

### f106v.47,+P0

EVA (original line):
```text
dcheoaiin shky
```

English structural gloss (generated):

- dcheoaiin: tokens: p ch e o aiin → vowel_run: e (level 1; class e) → suffix: aiin
- shky: tokens: sh k
