# Voynich Speculative Procedural Protocol — f115v

IMPORTANT: this is NOT a real or validated translation of the Voynich Manuscript. It is a speculative/procedural model that interprets EVA using a user-defined grammar to generate experimental recipes using safe, known edible substitutes.

This file is generated automatically from IVTFF/EVA transliteration plus a user-defined procedural grammar.

![f115v](../../../images/q20/f115v_crd.jpg)

## Page / Folio
- currier: B
- folio: f115v
- page_number: 233

## EVA Text (Transliteration)
```text
tchedor otchedy qotees ytain qoty qotar opolaiin qotdain ol raiin om
ykeeochody qokeedy yteedy qokar qor chodaiin qokchdy chdar okchdy qokam
dsheol qokchedy qokshedy qotchey otaiin qotedy lkedain dalchdy okchedam
ykcheodain lkchedy otechdy qotar or al ytchedy lched otchd chokeedy
dair cheky qoteey otar chl oleed
pchody odaiin chcphy qokchdain qotain qokor shed oteody solkaiin al
dchedy tedy qokeey roiin shedy okor air cheoor olkchedy chotam
tchoros sheol qotchs olchees otchdy qotol lpchedy okar lkechedy pchdam
ychees chdaiin chotain cholkeedy qotchy chody qotain lkchey lchey ror
saiin sho sheody okaiin dalchedy oteeo chedy
tchdor shorail chodaiin chkol chkchol qotched qotchey dpchedy q[?:o]tam
ysheed lchedy lkchedy soraiin
pchdarody pcheed rar tcheody polched lpchdy tol rchees cphor orair kol
okeeo kaiin lkeeey lkor sheedy chockhy qockheedy qokechy lkeey ldaiin
saiin chol qotain qokain chl lr chdain qoteey rcheey r ar rodam
dain ar oteey qoteol kar y sheo lkechdy qokar qokey lko rokeedy ld
okai[?:r] chey keey lcho racthy
pchodain chotain choky lchain lpchdain dalchedy qotolchedy qopchedy
yshedy qokeedy lxor xoiin choto keeody qoteody dain qokchedy ralom
ysheey qoteey lkeey raiin cheo lor otal otchedy
tshedy sheolkeedy lkeeshdy cheeo lor eees aiin okchedy qopcheddy lky
soiin shedain qokeedy chodain otedain qokeedy qokeedy qotedy [r:s]osy
lshes sheet chdy otedy shdy lchedy
pcheo cheeody qoteeotchy soin opchees chpcheod rchl cho pchdy qopcham
ykees aiin olkeeody qos ain cheodain chcthy tchedy qokeedy lkeedas
dcheedy kchedy lcheey ror al chokedy dol qokeeeos qolkeedy qokar ar
olain cheo lkain cheey qot cheody lor aiin oteed chkal kchetam
y chey lcho lor chedy chol chedy chdy
polor sheedy qoteedy qokechy lraly l shey sheot shedy chteey lky raram
ycheedaiin ol chlor lkchedy rchedar oteedal ar lky
pchdair opchedy qopor iirchal sheey qotain chety rodaiin opchepy shokchy
ykchedy okar chedar rolsheedy lkchdy chddy ch[cph:cth]edy cheey teeodaiin
ychedal qotchy chcthy chdal lchedy
taiin sheckeey lchckhy ldar shdar qotchdy qoky shedy qokar chckhy
shos shee oky cheo lkain cheeos al ches kcheo rain checthey lcham
ytar aiin qoteey lkchedy
tshar shor shckhhy olkeeo lkeedol ltchdy chkaidararal lkeedy
oiiin chees otairos loedy cheo keeo llchs o l r aiiin chkain sham
dairal chain ykeedy qokedy qokain lkaiin lkchey lkain lror
ycheeol kaiin shedain s cho r okas cheos qokchy
dcheodl sheo kolchey fchedol shedy qotoee tchy choiphy qopch[y:?]r
yair al sheey oteeol keody rcheey lkchdy qokchey lkar l[k:?]l rodam
ysheo[k:t]aiin qotchdal lkchdy lkshedy qokar cheos qokaiin aky
ototar sheey qokey qokchey qokchey ral rchos oty lchedy
yk chedaiin chody qokaiin dary cholaiim
```

## Domain Context (Heuristic; Not a Translation)

This section summarizes recurring **basewords** in this IVTFF domain and shows simple substring evidence that the token markers used by the procedural grammar occur inside frequent words.

Any Italian anagram / English gloss is a best-effort lexicon match, not a decipherment.



### Associated basewords (non-generic; top by frequency in this domain)
- `paiin` (count=241) → Italian anagram `piani`; English: plans (arrangements)
- `qokaiin` (count=122) → Italian anagram `ciancio`; English: [n/a]
- `okaiin` (count=109) → Italian anagram `coniai`; English: [n/a]
- `qokain` (count=101) → Italian anagram `acconi`; English: [n/a]
- `okain` (count=69) → Italian anagram `acino`; English: a berry
- `qokep` (count=65) → Italian anagram `pecco`; English: [n/a]
- `otain` (count=54) → Italian anagram `anito`; English: [n/a]
- `qokar` (count=48) → Italian anagram `carco`; English: [n/a]
- `saiin` (count=48) → Italian anagram `asini`; English: [n/a]
- `qokal` (count=46) → Italian anagram `calco`; English: cast (of sculpture)
- `kaiin` (count=45) → Italian anagram `acini`; English: [n/a]
- `qotaiin` (count=40) → Italian anagram `cationi`; English: [n/a]
- `lkaiin` (count=40) → Italian anagram `ancili`; English: [n/a]
- `qokeol` (count=38) → Italian anagram `eccolo`; English: [n/a]
- `qotain` (count=34) → Italian anagram `antico`; English: ancient

### Marker evidence (substring in frequent basewords)
- `qo`: 63 basewords; examples: `qokee`, `qokeep`, `qokaiin`, `qokain`, `qokep`, `qoke`
- `q`: 64 basewords; examples: `qokee`, `qokeep`, `qokaiin`, `qokain`, `qokep`, `qoke`
- `o`: 281 basewords; examples: `qokee`, `ol`, `o`, `qokeep`, `okee`, `qokaiin`
- `k`: 150 basewords; examples: `qokee`, `qokeep`, `okee`, `qokaiin`, `okaiin`, `qokain`
- `t`: 100 basewords; examples: `otaiin`, `otee`, `otal`, `otar`, `oteep`, `otep`
- `p`: 154 basewords; examples: `paiin`, `chep`, `qokeep`, `shep`, `par`, `oteep`
- `ch`: 144 basewords; examples: `chep`, `che`, `chol`, `chee`, `cheol`, `cheo`
- `sh`: 52 basewords; examples: `shep`, `she`, `shee`, `sheol`, `sheep`, `shol`
- `f`: 2 basewords; examples: `fchep`, `f`
- `cth`: 17 basewords; examples: `chcth`, `cthe`, `shcth`, `checth`, `cthol`, `cthee`
- `ckh`: 18 basewords; examples: `chckh`, `shckh`, `checkh`, `chckhe`, `chockh`, `sheckh`
- `cph`: 3 basewords; examples: `cphol`, `cph`, `cphe`
- `iin`: 38 basewords; examples: `aiin`, `paiin`, `qokaiin`, `okaiin`, `otaiin`, `saiin`
- `aiin`: 31 basewords; examples: `aiin`, `paiin`, `qokaiin`, `okaiin`, `otaiin`, `saiin`

## Recipes Index (This Page)
- [f115v.1,@P0](#f115v-1-f115v-1-p0)
- [f115v.2,+P0](#f115v-2-f115v-2-p0)
- [f115v.3,+P0](#f115v-3-f115v-3-p0)
- [f115v.4,+P0](#f115v-4-f115v-4-p0)
- [f115v.5,+P0](#f115v-5-f115v-5-p0)
- [f115v.6,+P0](#f115v-6-f115v-6-p0)
- [f115v.7,+P0](#f115v-7-f115v-7-p0)
- [f115v.8,+P0](#f115v-8-f115v-8-p0)
- [f115v.9,+P0](#f115v-9-f115v-9-p0)
- [f115v.10,+P0](#f115v-10-f115v-10-p0)
- [f115v.11,+P0](#f115v-11-f115v-11-p0)
- [f115v.12,+P0](#f115v-12-f115v-12-p0)
- [f115v.13,+P0](#f115v-13-f115v-13-p0)
- [f115v.14,+P0](#f115v-14-f115v-14-p0)
- [f115v.15,+P0](#f115v-15-f115v-15-p0)
- [f115v.16,+P0](#f115v-16-f115v-16-p0)
- [f115v.17,+P0](#f115v-17-f115v-17-p0)
- [f115v.18,+P0](#f115v-18-f115v-18-p0)
- [f115v.19,+P0](#f115v-19-f115v-19-p0)
- [f115v.20,+P0](#f115v-20-f115v-20-p0)
- [f115v.21,+P0](#f115v-21-f115v-21-p0)
- [f115v.22,+P0](#f115v-22-f115v-22-p0)
- [f115v.23,+P0](#f115v-23-f115v-23-p0)
- [f115v.24,+P0](#f115v-24-f115v-24-p0)
- [f115v.25,+P0](#f115v-25-f115v-25-p0)
- [f115v.26,+P0](#f115v-26-f115v-26-p0)
- [f115v.27,+P0](#f115v-27-f115v-27-p0)
- [f115v.28,+P0](#f115v-28-f115v-28-p0)
- [f115v.29,+P0](#f115v-29-f115v-29-p0)
- [f115v.30,+P0](#f115v-30-f115v-30-p0)
- [f115v.31,+P0](#f115v-31-f115v-31-p0)
- [f115v.32,+P0](#f115v-32-f115v-32-p0)
- [f115v.33,+P0](#f115v-33-f115v-33-p0)
- [f115v.34,+P0](#f115v-34-f115v-34-p0)
- [f115v.35,+P0](#f115v-35-f115v-35-p0)
- [f115v.36,+P0](#f115v-36-f115v-36-p0)
- [f115v.37,+P0](#f115v-37-f115v-37-p0)
- [f115v.38,+P0](#f115v-38-f115v-38-p0)
- [f115v.39,+P0](#f115v-39-f115v-39-p0)
- [f115v.40,+P0](#f115v-40-f115v-40-p0)
- [f115v.41,+P0](#f115v-41-f115v-41-p0)
- [f115v.42,+P0](#f115v-42-f115v-42-p0)
- [f115v.43,+P0](#f115v-43-f115v-43-p0)
- [f115v.44,+P0](#f115v-44-f115v-44-p0)
- [f115v.45,+P0](#f115v-45-f115v-45-p0)

## Line Glosses (Procedural Gloss Only; Not a Translation)

<a id="f115v-1-f115v-1-p0"></a>

### f115v.1,@P0

EVA: tchedor otchedy qotees ytain qoty qotar opolaiin qotdain ol raiin om

Direct Gloss (Procedural, Not a Real Translation):
- tchedor: tokens: t ch e p o r → connectors: r → vowel_run: e (level 1; class e)
- otchedy: tokens: o t ch e p → vowel_run: e (level 1; class e)
- qotees: tokens: qo t ee s → connectors: s → vowel_run: ee (level 2; class e)
- ytain: tokens: t a i n → connectors: n → vowel_run: a (level 1; class a)
- qoty: tokens: qo t
- qotar: tokens: qo t a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qotar` → `corta`; [n/a])
- opolaiin: tokens: o p o l aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin
- qotdain: tokens: qo t p a i n → connectors: n → vowel_run: a (level 1; class a)
- ol: tokens: o l → connectors: l
- raiin: tokens: r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- om: tokens: o m → connectors: m

<a id="f115v-2-f115v-2-p0"></a>

### f115v.2,+P0

EVA: ykeeochody qokeedy yteedy qokar qor chodaiin qokchdy chdar okchdy qokam

Direct Gloss (Procedural, Not a Real Translation):
- ykeeochody: tokens: k ee o ch o p → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- yteedy: tokens: t ee p → vowel_run: ee (level 2; class e)
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a)
- qor: tokens: qo r → connectors: r
- chodaiin: tokens: ch o p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- qokchdy: tokens: qo k ch p
- chdar: tokens: ch p a r → connectors: r → vowel_run: a (level 1; class a)
- okchdy: tokens: o k ch p
- qokam: tokens: qo k a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f115v-3-f115v-3-p0"></a>

### f115v.3,+P0

EVA: dsheol qokchedy qokshedy qotchey otaiin qotedy lkedain dalchdy okchedam

Direct Gloss (Procedural, Not a Real Translation):
- dsheol: tokens: p sh e o l → connectors: l → vowel_run: e (level 1; class e)
- qokchedy: tokens: qo k ch e p → vowel_run: e (level 1; class e)
- qokshedy: tokens: qo k sh e p → vowel_run: e (level 1; class e)
- qotchey: tokens: qo t ch e → vowel_run: e (level 1; class e)
- otaiin: tokens: o t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- qotedy: tokens: qo t e p → vowel_run: e (level 1; class e)
- lkedain: tokens: l k e p a i n → connectors: l n → vowel_run: e (level 1; class e)
- dalchdy: tokens: p a l ch p → connectors: l → vowel_run: a (level 1; class a)
- okchedam: tokens: o k ch e p a m → connectors: m → vowel_run: e (level 1; class e)

<a id="f115v-4-f115v-4-p0"></a>

### f115v.4,+P0

EVA: ykcheodain lkchedy otechdy qotar or al ytchedy lched otchd chokeedy

Direct Gloss (Procedural, Not a Real Translation):
- ykcheodain: tokens: k ch e o p a i n → connectors: n → vowel_run: e (level 1; class e)
- lkchedy: tokens: l k ch e p → connectors: l → vowel_run: e (level 1; class e)
- otechdy: tokens: o t e ch p → vowel_run: e (level 1; class e)
- qotar: tokens: qo t a r → connectors: r → vowel_run: a (level 1; class a) (lexicon-context: `qotar` → `corta`; [n/a])
- or: tokens: o r → connectors: r
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- ytchedy: tokens: t ch e p → vowel_run: e (level 1; class e)
- lched: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- otchd: tokens: o t ch p
- chokeedy: tokens: ch o k ee p → vowel_run: ee (level 2; class e)

<a id="f115v-5-f115v-5-p0"></a>

### f115v.5,+P0

EVA: dair cheky qoteey otar chl oleed

Direct Gloss (Procedural, Not a Real Translation):
- dair: tokens: p a i r → connectors: r → vowel_run: a (level 1; class a)
- cheky: tokens: ch e k → vowel_run: e (level 1; class e)
- qoteey: tokens: qo t ee → vowel_run: ee (level 2; class e)
- otar: tokens: o t a r → connectors: r → vowel_run: a (level 1; class a)
- chl: tokens: ch l → connectors: l
- oleed: tokens: o l ee p → connectors: l → vowel_run: ee (level 2; class e)

<a id="f115v-6-f115v-6-p0"></a>

### f115v.6,+P0

EVA: pchody odaiin chcphy qokchdain qotain qokor shed oteody solkaiin al

Direct Gloss (Procedural, Not a Real Translation):
- pchody: tokens: p ch o p
- odaiin: tokens: o p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `opaiin` → `opinai`; [n/a])
- chcphy: tokens: ch cph
- qokchdain: tokens: qo k ch p a i n → connectors: n → vowel_run: a (level 1; class a)
- qotain: tokens: qo t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qotain` → `antico`; ancient)
- qokor: tokens: qo k o r → connectors: r
- shed: tokens: sh e p → vowel_run: e (level 1; class e)
- oteody: tokens: o t e o p → vowel_run: e (level 1; class e)
- solkaiin: tokens: s o l k aiin → connectors: s l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `lkaiin` → `canili`; [n/a])
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)

<a id="f115v-7-f115v-7-p0"></a>

### f115v.7,+P0

EVA: dchedy tedy qokeey roiin shedy okor air cheoor olkchedy chotam

Direct Gloss (Procedural, Not a Real Translation):
- dchedy: tokens: p ch e p → vowel_run: e (level 1; class e)
- tedy: tokens: t e p → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- roiin: tokens: r o iin → connectors: r → vowel_run: ii (level 2; class i) → suffix: iin
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- okor: tokens: o k o r → connectors: r
- air: tokens: a i r → connectors: r → vowel_run: a (level 1; class a)
- cheoor: tokens: ch e o o r → connectors: r → vowel_run: e (level 1; class e)
- olkchedy: tokens: o l k ch e p → connectors: l → vowel_run: e (level 1; class e)
- chotam: tokens: ch o t a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f115v-8-f115v-8-p0"></a>

### f115v.8,+P0

EVA: tchoros sheol qotchs olchees otchdy qotol lpchedy okar lkechedy pchdam

Direct Gloss (Procedural, Not a Real Translation):
- tchoros: tokens: t ch o r o s → connectors: r s
- sheol: tokens: sh e o l → connectors: l → vowel_run: e (level 1; class e)
- qotchs: tokens: qo t ch s → connectors: s
- olchees: tokens: o l ch ee s → connectors: l s → vowel_run: ee (level 2; class e)
- otchdy: tokens: o t ch p
- qotol: tokens: qo t o l → connectors: l
- lpchedy: tokens: l p ch e p → connectors: l → vowel_run: e (level 1; class e)
- okar: tokens: o k a r → connectors: r → vowel_run: a (level 1; class a)
- lkechedy: tokens: l k e ch e p → connectors: l → vowel_run: e (level 1; class e)
- pchdam: tokens: p ch p a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f115v-9-f115v-9-p0"></a>

### f115v.9,+P0

EVA: ychees chdaiin chotain cholkeedy qotchy chody qotain lkchey lchey ror

Direct Gloss (Procedural, Not a Real Translation):
- ychees: tokens: ch ee s → connectors: s → vowel_run: ee (level 2; class e)
- chdaiin: tokens: ch p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))
- chotain: tokens: ch o t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `otain` → `notai`; [n/a])
- cholkeedy: tokens: ch o l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- qotchy: tokens: qo t ch
- chody: tokens: ch o p
- qotain: tokens: qo t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qotain` → `antico`; ancient)
- lkchey: tokens: l k ch e → connectors: l → vowel_run: e (level 1; class e)
- lchey: tokens: l ch e → connectors: l → vowel_run: e (level 1; class e)
- ror: tokens: r o r → connectors: r r

<a id="f115v-10-f115v-10-p0"></a>

### f115v.10,+P0

EVA: saiin sho sheody okaiin dalchedy oteeo chedy

Direct Gloss (Procedural, Not a Real Translation):
- saiin: tokens: s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `saiin` → `asini`; [n/a])
- sho: tokens: sh o
- sheody: tokens: sh e o p → vowel_run: e (level 1; class e)
- okaiin: tokens: o k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `okaiin` → `coniai`; [n/a])
- dalchedy: tokens: p a l ch e p → connectors: l → vowel_run: a (level 1; class a)
- oteeo: tokens: o t ee o → vowel_run: ee (level 2; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)

<a id="f115v-11-f115v-11-p0"></a>

### f115v.11,+P0

EVA: tchdor shorail chodaiin chkol chkchol qotched qotchey dpchedy q[?:o]tam

Direct Gloss (Procedural, Not a Real Translation):
- tchdor: tokens: t ch p o r → connectors: r
- shorail: tokens: sh o r a i l → connectors: r l → vowel_run: a (level 1; class a)
- chodaiin: tokens: ch o p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chkol: tokens: ch k o l → connectors: l
- chkchol: tokens: ch k ch o l → connectors: l
- qotched: tokens: qo t ch e p → vowel_run: e (level 1; class e)
- qotchey: tokens: qo t ch e → vowel_run: e (level 1; class e)
- dpchedy: tokens: p p ch e p → vowel_run: e (level 1; class e)
- q: tokens: q
- o: tokens: o
- tam: tokens: t a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f115v-12-f115v-12-p0"></a>

### f115v.12,+P0

EVA: ysheed lchedy lkchedy soraiin

Direct Gloss (Procedural, Not a Real Translation):
- ysheed: tokens: sh ee p → vowel_run: ee (level 2; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- lkchedy: tokens: l k ch e p → connectors: l → vowel_run: e (level 1; class e)
- soraiin: tokens: s o r aiin → connectors: s r → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `oraiin` → `ironia`; irony)

<a id="f115v-13-f115v-13-p0"></a>

### f115v.13,+P0

EVA: pchdarody pcheed rar tcheody polched lpchdy tol rchees cphor orair kol

Direct Gloss (Procedural, Not a Real Translation):
- pchdarody: tokens: p ch p a r o p → connectors: r → vowel_run: a (level 1; class a)
- pcheed: tokens: p ch ee p → vowel_run: ee (level 2; class e)
- rar: tokens: r a r → connectors: r r → vowel_run: a (level 1; class a)
- tcheody: tokens: t ch e o p → vowel_run: e (level 1; class e)
- polched: tokens: p o l ch e p → connectors: l → vowel_run: e (level 1; class e)
- lpchdy: tokens: l p ch p → connectors: l
- tol: tokens: t o l → connectors: l
- rchees: tokens: r ch ee s → connectors: r s → vowel_run: ee (level 2; class e)
- cphor: tokens: cph o r → connectors: r
- orair: tokens: o r a i r → connectors: r r → vowel_run: a (level 1; class a)
- kol: tokens: k o l → connectors: l

<a id="f115v-14-f115v-14-p0"></a>

### f115v.14,+P0

EVA: okeeo kaiin lkeeey lkor sheedy chockhy qockheedy qokechy lkeey ldaiin

Direct Gloss (Procedural, Not a Real Translation):
- okeeo: tokens: o k ee o → vowel_run: ee (level 2; class e)
- kaiin: tokens: k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- lkeeey: tokens: l k eee → connectors: l → vowel_run: eee (level 3; class e)
- lkor: tokens: l k o r → connectors: l r
- sheedy: tokens: sh ee p → vowel_run: ee (level 2; class e)
- chockhy: tokens: ch o ckh
- qockheedy: tokens: qo ckh ee p → vowel_run: ee (level 2; class e)
- qokechy: tokens: qo k e ch → vowel_run: e (level 1; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- ldaiin: tokens: l p aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))

<a id="f115v-15-f115v-15-p0"></a>

### f115v.15,+P0

EVA: saiin chol qotain qokain chl lr chdain qoteey rcheey r ar rodam

Direct Gloss (Procedural, Not a Real Translation):
- saiin: tokens: s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `saiin` → `asini`; [n/a])
- chol: tokens: ch o l → connectors: l
- qotain: tokens: qo t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qotain` → `antico`; ancient)
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `concia`; tanning)
- chl: tokens: ch l → connectors: l
- lr: tokens: l r → connectors: l r
- chdain: tokens: ch p a i n → connectors: n → vowel_run: a (level 1; class a)
- qoteey: tokens: qo t ee → vowel_run: ee (level 2; class e)
- rcheey: tokens: r ch ee → connectors: r → vowel_run: ee (level 2; class e)
- r: tokens: r → connectors: r
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- rodam: tokens: r o p a m → connectors: r m → vowel_run: a (level 1; class a)

<a id="f115v-16-f115v-16-p0"></a>

### f115v.16,+P0

EVA: dain ar oteey qoteol kar y sheo lkechdy qokar qokey lko rokeedy ld

Direct Gloss (Procedural, Not a Real Translation):
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- qoteol: tokens: qo t e o l → connectors: l → vowel_run: e (level 1; class e)
- kar: tokens: k a r → connectors: r → vowel_run: a (level 1; class a)
- y: [unparsed]
- sheo: tokens: sh e o → vowel_run: e (level 1; class e)
- lkechdy: tokens: l k e ch p → connectors: l → vowel_run: e (level 1; class e)
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a)
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)
- lko: tokens: l k o → connectors: l
- rokeedy: tokens: r o k ee p → connectors: r → vowel_run: ee (level 2; class e)
- ld: tokens: l p → connectors: l

<a id="f115v-17-f115v-17-p0"></a>

### f115v.17,+P0

EVA: okai[?:r] chey keey lcho racthy

Direct Gloss (Procedural, Not a Real Translation):
- okai: tokens: o k a i → vowel_run: a (level 1; class a)
- r: tokens: r → connectors: r
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- keey: tokens: k ee → vowel_run: ee (level 2; class e)
- lcho: tokens: l ch o → connectors: l
- racthy: tokens: r a cth → connectors: r → vowel_run: a (level 1; class a)

<a id="f115v-18-f115v-18-p0"></a>

### f115v.18,+P0

EVA: pchodain chotain choky lchain lpchdain dalchedy qotolchedy qopchedy

Direct Gloss (Procedural, Not a Real Translation):
- pchodain: tokens: p ch o p a i n → connectors: n → vowel_run: a (level 1; class a)
- chotain: tokens: ch o t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `otain` → `notai`; [n/a])
- choky: tokens: ch o k
- lchain: tokens: l ch a i n → connectors: l n → vowel_run: a (level 1; class a)
- lpchdain: tokens: l p ch p a i n → connectors: l n → vowel_run: a (level 1; class a)
- dalchedy: tokens: p a l ch e p → connectors: l → vowel_run: a (level 1; class a)
- qotolchedy: tokens: qo t o l ch e p → connectors: l → vowel_run: e (level 1; class e)
- qopchedy: tokens: qo p ch e p → vowel_run: e (level 1; class e)

<a id="f115v-19-f115v-19-p0"></a>

### f115v.19,+P0

EVA: yshedy qokeedy lxor xoiin choto keeody qoteody dain qokchedy ralom

Direct Gloss (Procedural, Not a Real Translation):
- yshedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- lxor: tokens: l x o r → connectors: l r
- xoiin: tokens: x o iin → vowel_run: ii (level 2; class i) → suffix: iin
- choto: tokens: ch o t o
- keeody: tokens: k ee o p → vowel_run: ee (level 2; class e)
- qoteody: tokens: qo t e o p → vowel_run: e (level 1; class e)
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- qokchedy: tokens: qo k ch e p → vowel_run: e (level 1; class e)
- ralom: tokens: r a l o m → connectors: r l m → vowel_run: a (level 1; class a)

<a id="f115v-20-f115v-20-p0"></a>

### f115v.20,+P0

EVA: ysheey qoteey lkeey raiin cheo lor otal otchedy

Direct Gloss (Procedural, Not a Real Translation):
- ysheey: tokens: sh ee → vowel_run: ee (level 2; class e)
- qoteey: tokens: qo t ee → vowel_run: ee (level 2; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- raiin: tokens: r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- cheo: tokens: ch e o → vowel_run: e (level 1; class e)
- lor: tokens: l o r → connectors: l r
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- otchedy: tokens: o t ch e p → vowel_run: e (level 1; class e)

<a id="f115v-21-f115v-21-p0"></a>

### f115v.21,+P0

EVA: tshedy sheolkeedy lkeeshdy cheeo lor eees aiin okchedy qopcheddy lky

Direct Gloss (Procedural, Not a Real Translation):
- tshedy: tokens: t sh e p → vowel_run: e (level 1; class e)
- sheolkeedy: tokens: sh e o l k ee p → connectors: l → vowel_run: e (level 1; class e)
- lkeeshdy: tokens: l k ee sh p → connectors: l → vowel_run: ee (level 2; class e)
- cheeo: tokens: ch ee o → vowel_run: ee (level 2; class e)
- lor: tokens: l o r → connectors: l r
- eees: tokens: eee s → connectors: s → vowel_run: eee (level 3; class e)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okchedy: tokens: o k ch e p → vowel_run: e (level 1; class e)
- qopcheddy: tokens: qo p ch e p p → vowel_run: e (level 1; class e)
- lky: tokens: l k → connectors: l

<a id="f115v-22-f115v-22-p0"></a>

### f115v.22,+P0

EVA: soiin shedain qokeedy chodain otedain qokeedy qokeedy qotedy [r:s]osy

Direct Gloss (Procedural, Not a Real Translation):
- soiin: tokens: s o iin → connectors: s → vowel_run: ii (level 2; class i) → suffix: iin
- shedain: tokens: sh e p a i n → connectors: n → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- chodain: tokens: ch o p a i n → connectors: n → vowel_run: a (level 1; class a)
- otedain: tokens: o t e p a i n → connectors: n → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qotedy: tokens: qo t e p → vowel_run: e (level 1; class e)
- r: tokens: r → connectors: r
- s: tokens: s → connectors: s
- osy: tokens: o s → connectors: s

<a id="f115v-23-f115v-23-p0"></a>

### f115v.23,+P0

EVA: lshes sheet chdy otedy shdy lchedy

Direct Gloss (Procedural, Not a Real Translation):
- lshes: tokens: l sh e s → connectors: l s → vowel_run: e (level 1; class e)
- sheet: tokens: sh ee t → vowel_run: ee (level 2; class e)
- chdy: tokens: ch p
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- shdy: tokens: sh p
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)

<a id="f115v-24-f115v-24-p0"></a>

### f115v.24,+P0

EVA: pcheo cheeody qoteeotchy soin opchees chpcheod rchl cho pchdy qopcham

Direct Gloss (Procedural, Not a Real Translation):
- pcheo: tokens: p ch e o → vowel_run: e (level 1; class e)
- cheeody: tokens: ch ee o p → vowel_run: ee (level 2; class e)
- qoteeotchy: tokens: qo t ee o t ch → vowel_run: ee (level 2; class e)
- soin: tokens: s o i n → connectors: s n → vowel_run: i (level 1; class i)
- opchees: tokens: o p ch ee s → connectors: s → vowel_run: ee (level 2; class e)
- chpcheod: tokens: ch p ch e o p → vowel_run: e (level 1; class e)
- rchl: tokens: r ch l → connectors: r l
- cho: tokens: ch o
- pchdy: tokens: p ch p
- qopcham: tokens: qo p ch a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f115v-25-f115v-25-p0"></a>

### f115v.25,+P0

EVA: ykees aiin olkeeody qos ain cheodain chcthy tchedy qokeedy lkeedas

Direct Gloss (Procedural, Not a Real Translation):
- ykees: tokens: k ee s → connectors: s → vowel_run: ee (level 2; class e)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- olkeeody: tokens: o l k ee o p → connectors: l → vowel_run: ee (level 2; class e)
- qos: tokens: qo s → connectors: s
- ain: tokens: a i n → connectors: n → vowel_run: a (level 1; class a)
- cheodain: tokens: ch e o p a i n → connectors: n → vowel_run: e (level 1; class e)
- chcthy: tokens: ch cth
- tchedy: tokens: t ch e p → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- lkeedas: tokens: l k ee p a s → connectors: l s → vowel_run: ee (level 2; class e)

<a id="f115v-26-f115v-26-p0"></a>

### f115v.26,+P0

EVA: dcheedy kchedy lcheey ror al chokedy dol qokeeeos qolkeedy qokar ar

Direct Gloss (Procedural, Not a Real Translation):
- dcheedy: tokens: p ch ee p → vowel_run: ee (level 2; class e)
- kchedy: tokens: k ch e p → vowel_run: e (level 1; class e)
- lcheey: tokens: l ch ee → connectors: l → vowel_run: ee (level 2; class e)
- ror: tokens: r o r → connectors: r r
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- chokedy: tokens: ch o k e p → vowel_run: e (level 1; class e)
- dol: tokens: p o l → connectors: l
- qokeeeos: tokens: qo k eee o s → connectors: s → vowel_run: eee (level 3; class e)
- qolkeedy: tokens: qo l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)

<a id="f115v-27-f115v-27-p0"></a>

### f115v.27,+P0

EVA: olain cheo lkain cheey qot cheody lor aiin oteed chkal kchetam

Direct Gloss (Procedural, Not a Real Translation):
- olain: tokens: o l a i n → connectors: l n → vowel_run: a (level 1; class a)
- cheo: tokens: ch e o → vowel_run: e (level 1; class e)
- lkain: tokens: l k a i n → connectors: l n → vowel_run: a (level 1; class a) (lexicon-context: `lkain` → `lanci`; [n/a])
- cheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- qot: tokens: qo t
- cheody: tokens: ch e o p → vowel_run: e (level 1; class e)
- lor: tokens: l o r → connectors: l r
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- oteed: tokens: o t ee p → vowel_run: ee (level 2; class e)
- chkal: tokens: ch k a l → connectors: l → vowel_run: a (level 1; class a)
- kchetam: tokens: k ch e t a m → connectors: m → vowel_run: e (level 1; class e)

<a id="f115v-28-f115v-28-p0"></a>

### f115v.28,+P0

EVA: y chey lcho lor chedy chol chedy chdy

Direct Gloss (Procedural, Not a Real Translation):
- y: [unparsed]
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- lcho: tokens: l ch o → connectors: l
- lor: tokens: l o r → connectors: l r
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- chol: tokens: ch o l → connectors: l
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- chdy: tokens: ch p

<a id="f115v-29-f115v-29-p0"></a>

### f115v.29,+P0

EVA: polor sheedy qoteedy qokechy lraly l shey sheot shedy chteey lky raram

Direct Gloss (Procedural, Not a Real Translation):
- polor: tokens: p o l o r → connectors: l r
- sheedy: tokens: sh ee p → vowel_run: ee (level 2; class e)
- qoteedy: tokens: qo t ee p → vowel_run: ee (level 2; class e)
- qokechy: tokens: qo k e ch → vowel_run: e (level 1; class e)
- lraly: tokens: l r a l → connectors: l r l → vowel_run: a (level 1; class a)
- l: tokens: l → connectors: l
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- sheot: tokens: sh e o t → vowel_run: e (level 1; class e)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- chteey: tokens: ch t ee → vowel_run: ee (level 2; class e)
- lky: tokens: l k → connectors: l
- raram: tokens: r a r a m → connectors: r r m → vowel_run: a (level 1; class a)

<a id="f115v-30-f115v-30-p0"></a>

### f115v.30,+P0

EVA: ycheedaiin ol chlor lkchedy rchedar oteedal ar lky

Direct Gloss (Procedural, Not a Real Translation):
- ycheedaiin: tokens: ch ee p aiin → vowel_run: ee (level 2; class e) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))
- ol: tokens: o l → connectors: l
- chlor: tokens: ch l o r → connectors: l r
- lkchedy: tokens: l k ch e p → connectors: l → vowel_run: e (level 1; class e)
- rchedar: tokens: r ch e p a r → connectors: r r → vowel_run: e (level 1; class e) (lexicon-context: `chepar` → `capre`; [n/a])
- oteedal: tokens: o t ee p a l → connectors: l → vowel_run: ee (level 2; class e)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- lky: tokens: l k → connectors: l

<a id="f115v-31-f115v-31-p0"></a>

### f115v.31,+P0

EVA: pchdair opchedy qopor iirchal sheey qotain chety rodaiin opchepy shokchy

Direct Gloss (Procedural, Not a Real Translation):
- pchdair: tokens: p ch p a i r → connectors: r → vowel_run: a (level 1; class a)
- opchedy: tokens: o p ch e p → vowel_run: e (level 1; class e)
- qopor: tokens: qo p o r → connectors: r
- iirchal: tokens: ii r ch a l → connectors: r l → vowel_run: ii (level 2; class i)
- sheey: tokens: sh ee → vowel_run: ee (level 2; class e)
- qotain: tokens: qo t a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qotain` → `antico`; ancient)
- chety: tokens: ch e t → vowel_run: e (level 1; class e)
- rodaiin: tokens: r o p aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `opaiin` → `opinai`; [n/a])
- opchepy: tokens: o p ch e p → vowel_run: e (level 1; class e)
- shokchy: tokens: sh o k ch

<a id="f115v-32-f115v-32-p0"></a>

### f115v.32,+P0

EVA: ykchedy okar chedar rolsheedy lkchdy chddy ch[cph:cth]edy cheey teeodaiin

Direct Gloss (Procedural, Not a Real Translation):
- ykchedy: tokens: k ch e p → vowel_run: e (level 1; class e)
- okar: tokens: o k a r → connectors: r → vowel_run: a (level 1; class a)
- chedar: tokens: ch e p a r → connectors: r → vowel_run: e (level 1; class e) (lexicon-context: `chepar` → `capre`; [n/a])
- rolsheedy: tokens: r o l sh ee p → connectors: r l → vowel_run: ee (level 2; class e)
- lkchdy: tokens: l k ch p → connectors: l
- chddy: tokens: ch p p
- ch: tokens: ch
- cph: tokens: cph
- cth: tokens: cth
- edy: tokens: e p → vowel_run: e (level 1; class e)
- cheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- teeodaiin: tokens: t ee o p aiin → vowel_run: ee (level 2; class e) → suffix: aiin (lexicon-context: `opaiin` → `opinai`; [n/a])

<a id="f115v-33-f115v-33-p0"></a>

### f115v.33,+P0

EVA: ychedal qotchy chcthy chdal lchedy

Direct Gloss (Procedural, Not a Real Translation):
- ychedal: tokens: ch e p a l → connectors: l → vowel_run: e (level 1; class e)
- qotchy: tokens: qo t ch
- chcthy: tokens: ch cth
- chdal: tokens: ch p a l → connectors: l → vowel_run: a (level 1; class a)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)

<a id="f115v-34-f115v-34-p0"></a>

### f115v.34,+P0

EVA: taiin sheckeey lchckhy ldar shdar qotchdy qoky shedy qokar chckhy

Direct Gloss (Procedural, Not a Real Translation):
- taiin: tokens: t aiin → vowel_run: a (level 1; class a) → suffix: aiin
- sheckeey: tokens: sh e c k ee → vowel_run: e (level 1; class e)
- lchckhy: tokens: l ch ckh → connectors: l
- ldar: tokens: l p a r → connectors: l r → vowel_run: a (level 1; class a)
- shdar: tokens: sh p a r → connectors: r → vowel_run: a (level 1; class a)
- qotchdy: tokens: qo t ch p
- qoky: tokens: qo k
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a)
- chckhy: tokens: ch ckh

<a id="f115v-35-f115v-35-p0"></a>

### f115v.35,+P0

EVA: shos shee oky cheo lkain cheeos al ches kcheo rain checthey lcham

Direct Gloss (Procedural, Not a Real Translation):
- shos: tokens: sh o s → connectors: s
- shee: tokens: sh ee → vowel_run: ee (level 2; class e)
- oky: tokens: o k
- cheo: tokens: ch e o → vowel_run: e (level 1; class e)
- lkain: tokens: l k a i n → connectors: l n → vowel_run: a (level 1; class a) (lexicon-context: `lkain` → `lanci`; [n/a])
- cheeos: tokens: ch ee o s → connectors: s → vowel_run: ee (level 2; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- ches: tokens: ch e s → connectors: s → vowel_run: e (level 1; class e)
- kcheo: tokens: k ch e o → vowel_run: e (level 1; class e)
- rain: tokens: r a i n → connectors: r n → vowel_run: a (level 1; class a)
- checthey: tokens: ch e cth e → vowel_run: e (level 1; class e)
- lcham: tokens: l ch a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f115v-36-f115v-36-p0"></a>

### f115v.36,+P0

EVA: ytar aiin qoteey lkchedy

Direct Gloss (Procedural, Not a Real Translation):
- ytar: tokens: t a r → connectors: r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- qoteey: tokens: qo t ee → vowel_run: ee (level 2; class e)
- lkchedy: tokens: l k ch e p → connectors: l → vowel_run: e (level 1; class e)

<a id="f115v-37-f115v-37-p0"></a>

### f115v.37,+P0

EVA: tshar shor shckhhy olkeeo lkeedol ltchdy chkaidararal lkeedy

Direct Gloss (Procedural, Not a Real Translation):
- tshar: tokens: t sh a r → connectors: r → vowel_run: a (level 1; class a)
- shor: tokens: sh o r → connectors: r
- shckhhy: tokens: sh ckh h → unmodeled_tokens: h
- olkeeo: tokens: o l k ee o → connectors: l → vowel_run: ee (level 2; class e)
- lkeedol: tokens: l k ee p o l → connectors: l l → vowel_run: ee (level 2; class e)
- ltchdy: tokens: l t ch p → connectors: l
- chkaidararal: tokens: ch k a i p a r a r a l → connectors: r r l → vowel_run: a (level 1; class a)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)

<a id="f115v-38-f115v-38-p0"></a>

### f115v.38,+P0

EVA: oiiin chees otairos loedy cheo keeo llchs o l r aiiin chkain sham

Direct Gloss (Procedural, Not a Real Translation):
- oiiin: tokens: o iii n → connectors: n → vowel_run: iii (level 3; class i) → suffix: iin
- chees: tokens: ch ee s → connectors: s → vowel_run: ee (level 2; class e)
- otairos: tokens: o t a i r o s → connectors: r s → vowel_run: a (level 1; class a) (lexicon-context: `otair` → `atrio`; entrance hall, lobby (of a hotel etc.))
- loedy: tokens: l o e p → connectors: l → vowel_run: e (level 1; class e)
- cheo: tokens: ch e o → vowel_run: e (level 1; class e)
- keeo: tokens: k ee o → vowel_run: ee (level 2; class e)
- llchs: tokens: l l ch s → connectors: l l s
- o: tokens: o
- l: tokens: l → connectors: l
- r: tokens: r → connectors: r
- aiiin: tokens: a iii n → connectors: n → vowel_run: a (level 1; class a) → suffix: iin
- chkain: tokens: ch k a i n → connectors: n → vowel_run: a (level 1; class a)
- sham: tokens: sh a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f115v-39-f115v-39-p0"></a>

### f115v.39,+P0

EVA: dairal chain ykeedy qokedy qokain lkaiin lkchey lkain lror

Direct Gloss (Procedural, Not a Real Translation):
- dairal: tokens: p a i r a l → connectors: r l → vowel_run: a (level 1; class a)
- chain: tokens: ch a i n → connectors: n → vowel_run: a (level 1; class a)
- ykeedy: tokens: k ee p → vowel_run: ee (level 2; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e) (lexicon-context: `qokep` → `pecco`; [n/a])
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a) (lexicon-context: `qokain` → `concia`; tanning)
- lkaiin: tokens: l k aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `lkaiin` → `canili`; [n/a])
- lkchey: tokens: l k ch e → connectors: l → vowel_run: e (level 1; class e)
- lkain: tokens: l k a i n → connectors: l n → vowel_run: a (level 1; class a) (lexicon-context: `lkain` → `lanci`; [n/a])
- lror: tokens: l r o r → connectors: l r r

<a id="f115v-40-f115v-40-p0"></a>

### f115v.40,+P0

EVA: ycheeol kaiin shedain s cho r okas cheos qokchy

Direct Gloss (Procedural, Not a Real Translation):
- ycheeol: tokens: ch ee o l → connectors: l → vowel_run: ee (level 2; class e)
- kaiin: tokens: k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- shedain: tokens: sh e p a i n → connectors: n → vowel_run: e (level 1; class e)
- s: tokens: s → connectors: s
- cho: tokens: ch o
- r: tokens: r → connectors: r
- okas: tokens: o k a s → connectors: s → vowel_run: a (level 1; class a)
- cheos: tokens: ch e o s → connectors: s → vowel_run: e (level 1; class e)
- qokchy: tokens: qo k ch

<a id="f115v-41-f115v-41-p0"></a>

### f115v.41,+P0

EVA: dcheodl sheo kolchey fchedol shedy qotoee tchy choiphy qopch[y:?]r

Direct Gloss (Procedural, Not a Real Translation):
- dcheodl: tokens: p ch e o p l → connectors: l → vowel_run: e (level 1; class e)
- sheo: tokens: sh e o → vowel_run: e (level 1; class e)
- kolchey: tokens: k o l ch e → connectors: l → vowel_run: e (level 1; class e)
- fchedol: tokens: f ch e p o l → connectors: l → vowel_run: e (level 1; class e)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qotoee: tokens: qo t o ee → vowel_run: ee (level 2; class e)
- tchy: tokens: t ch
- choiphy: tokens: ch o i p h → vowel_run: i (level 1; class i) → unmodeled_tokens: h
- qopch: tokens: qo p ch
- y: [unparsed]
- r: tokens: r → connectors: r

<a id="f115v-42-f115v-42-p0"></a>

### f115v.42,+P0

EVA: yair al sheey oteeol keody rcheey lkchdy qokchey lkar l[k:?]l rodam

Direct Gloss (Procedural, Not a Real Translation):
- yair: tokens: a i r → connectors: r → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- sheey: tokens: sh ee → vowel_run: ee (level 2; class e)
- oteeol: tokens: o t ee o l → connectors: l → vowel_run: ee (level 2; class e)
- keody: tokens: k e o p → vowel_run: e (level 1; class e)
- rcheey: tokens: r ch ee → connectors: r → vowel_run: ee (level 2; class e)
- lkchdy: tokens: l k ch p → connectors: l
- qokchey: tokens: qo k ch e → vowel_run: e (level 1; class e)
- lkar: tokens: l k a r → connectors: l r → vowel_run: a (level 1; class a)
- l: tokens: l → connectors: l
- k: tokens: k
- l: tokens: l → connectors: l
- rodam: tokens: r o p a m → connectors: r m → vowel_run: a (level 1; class a)

<a id="f115v-43-f115v-43-p0"></a>

### f115v.43,+P0

EVA: ysheo[k:t]aiin qotchdal lkchdy lkshedy qokar cheos qokaiin aky

Direct Gloss (Procedural, Not a Real Translation):
- ysheo: tokens: sh e o → vowel_run: e (level 1; class e)
- k: tokens: k
- t: tokens: t
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- qotchdal: tokens: qo t ch p a l → connectors: l → vowel_run: a (level 1; class a)
- lkchdy: tokens: l k ch p → connectors: l
- lkshedy: tokens: l k sh e p → connectors: l → vowel_run: e (level 1; class e)
- qokar: tokens: qo k a r → connectors: r → vowel_run: a (level 1; class a)
- cheos: tokens: ch e o s → connectors: s → vowel_run: e (level 1; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `conciai`; [n/a])
- aky: tokens: a k → vowel_run: a (level 1; class a)

<a id="f115v-44-f115v-44-p0"></a>

### f115v.44,+P0

EVA: ototar sheey qokey qokchey qokchey ral rchos oty lchedy

Direct Gloss (Procedural, Not a Real Translation):
- ototar: tokens: o t o t a r → connectors: r → vowel_run: a (level 1; class a)
- sheey: tokens: sh ee → vowel_run: ee (level 2; class e)
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)
- qokchey: tokens: qo k ch e → vowel_run: e (level 1; class e)
- qokchey: tokens: qo k ch e → vowel_run: e (level 1; class e)
- ral: tokens: r a l → connectors: r l → vowel_run: a (level 1; class a)
- rchos: tokens: r ch o s → connectors: r s
- oty: tokens: o t
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)

<a id="f115v-45-f115v-45-p0"></a>

### f115v.45,+P0

EVA: yk chedaiin chody qokaiin dary cholaiim

Direct Gloss (Procedural, Not a Real Translation):
- yk: tokens: k
- chedaiin: tokens: ch e p aiin → vowel_run: e (level 1; class e) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))
- chody: tokens: ch o p
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `qokaiin` → `conciai`; [n/a])
- dary: tokens: p a r → connectors: r → vowel_run: a (level 1; class a)
- cholaiim: tokens: ch o l a ii m → connectors: l m → vowel_run: a (level 1; class a)
