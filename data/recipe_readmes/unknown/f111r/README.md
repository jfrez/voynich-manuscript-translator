# Voynich Speculative Procedural Protocol — f111r

IMPORTANT: this is NOT a real or validated translation of the Voynich Manuscript. It is a speculative/procedural model that interprets EVA using a user-defined grammar to generate experimental recipes using safe, known edible substitutes.

This file is generated automatically from IVTFF/EVA transliteration plus a user-defined procedural grammar.

![f111r](../../../images/q20/f111r_crd.jpg)

## Page / Folio
- currier: B
- folio: f111r
- page_number: 224

## EVA Text (Transliteration)
```text
kcholchdar shar aiip chepchedy chetalshy sheek shear shey ror am shey
daiin sheeky okeey okeey qaal shedy okeey oteey shedy chcthy lcheol oteeam
dsheedy lkeedy chckhy lchedy qokeey qokear chal qokeear cheokedy sal lokam
saiin oteedy qokeey daiin okedal chedy qokedy l shedy chctchy okeey lor ar am
saiin sheekshy ol shedy chokchey lkey otain
dain shedy qoky chedy qok shed olchedy qokeedy teedy chdy keedy qotchedy dary
sshedy qokeey checthey oteedy lchedy chol sheeol qokeody raiin otedy otar aiin om
daiin o chedain daiin cheedy qokeey qokedy chckhhy otshedy lkeeol lkeey qotchdy
ycheeodain okeeo olchedy lchedy qokeey okeeedy okain chedy chedyteey dal lam
ykeedaiin shekain shedy qokear ochey reeey qokeey olaiiin chedy lkeeody oraiin oty
dshey lkeedy lkeedy cheedy oteedaiin sheedy ar aikhy shkaiin chey daiin daram
dsheeo qokeedy otchedey lshey lkeedy oteey qokeedy oteolair ar shedam cheam
qosheo lchdy lshedy olkeeedy lr al chr or dain shey arain chey teey cheodaiin
dchedar oteey lchey ykeeeos aiin shear oteedy tedam oteey pchedey chedy lchedy
sheeodar aiin sheey shey oteody oteedy chey daiin oteol otedar tar pchdam
dchedshey qokeedy okeodain shet sheota[?:im] oteal tey lkeey sheedy shtar keedy
otes lchey lshedy qoteedy olcheody qokeey chey kal chekeedy oteed keedy otam
daiin shecthey oteody qochey qokeey chey teey qokeedy qokeedy cheol lokeedy lched
saiin okeeol qokeedy lchey lcheoekam oaiin cheky qokeeodar checthdy qoted l loty
qokeey qokeey l[ch:a]edy checthy chedey cheol teedy okeedy qokeedy okeey cho ol l kai@175;
daiin okar ain teey oteey ox o r shey chey ky deedy cheo dain sheet ar al l cheotydy
ssheo l keedy dal tched checkhey okey qo ain qo chedy qokeor okeey keo cthedy qokey
dshey sheeckhy qokeshey keeshody qokeey qokedy okain shekal okechedy qokechedy
sain chckhey cheos lchal lkechey okeey lkechedy oteodain char lky lchr ainai@175;
so sheor okeedy oteeey qokeey qokeeo s chedar alal oikhy oxar aiin odaiin chody
soaiin shecthy oteodaiin chedy otedar air ykeey shedain l chedy qokeedy qoteedaiin
dar sheod qokeey qokeody otey lkeedy chl lkeey daiin chedar chedyteedain olkal
qokeeol shedy qokeod cheokeey sheokar shekeey qotees al dain chedy qotain alam
salkeedy lkeedy chckhy rky chey okeeo chr al ar qokeedy qokedy qoteedy qod
qockhedy qokeechy qotchey chtar ar chkeey lkar aiin chedy qokeey q[o:?]irain
sheotchedy shedain shkeyqokeaiin shkeody qokeey qokeedy chkal checthy
dsheeoteey qokeeo l keey tear qoteey qotain cho lkeeedy lchechdy qokechy
qoeeeo [s:d] ain ar ol keey lkair cheda chkal keeodal eees chody qokeedy dl
teo olkeeo al lkeeey olain ar akeom qokain al qokeeod lkedlkey teedar
y chedl ar aiin aiy dam
polal shedal pchedy qofchedy pykeor cheefy qopchey opchedal lfchedy pchal
dain chol l cheeol keeol lkeeedy qokeey l keedy l kedy okeal kain chckhy ltedy
dched daiin al okeey kechedy chcthedy chedain otedy qokeedy key l ltain
saiin chckhedy okain oteain shok qokechdy okechedy chedy qokeey cheamar
saiin al keey okedain otedy qokeokedy qokeedy oteeolkeey okeeo l lainal
ycheey raiin cheody qokechy qokeey okedy lchedy qokeeal daiin otaral
qokeol qok[a:ee]l qokeeol cho lkedy l chedy chedy chedyteokain chedy l kag am
okeedy lkeedy lkedar okeedy chedy
pchedar oteeol lkeedain okeam sheo teolkedain shal keey lteeal cheal oty
oain ain keeey teed checkhed iir okeey lkeeey okeeo lkeey lkeey lkeedy qoky
y cheeo lkeey oteed lkeedy okeeshey chey lkar al olchal or aiin che[o:?]m
sheedy qokeey sheey qoteedy qeear al chedy okeey chedy
poeokeey lkeedy tedain shecthy qokeedy chckhy lfchy cheal chepcham daim
dcheey keeey qockhey qokeey lchedy keeor okeey lkeedy lkeey qokaiin ol
okeey l chedy cheekain qokeeokain chckhy cheal k[ee:ei]l qokain chckhal lkal
polkeeo shey cheokeain chl kar r aiin char ?ain al lkeedy qokal okchy
dair al qokeey qokaiin sheal qokeain shckhy sain chckhy char aiin alom
yshe aiin okshdy shkeey kain chaiin alolshey qokaiin chcthydain
tair chckhaiin dair qokal otain okal
```

## Domain Context (Heuristic; Not a Translation)

This section summarizes recurring **basewords** in this IVTFF domain and shows simple substring evidence that the token markers used by the procedural grammar occur inside frequent words.

Any Italian anagram / English gloss is a best-effort lexicon match, not a decipherment.



### Associated basewords (non-generic; top by frequency in this domain)
- `daiin` (count=231) → Italian anagram `piani`; English: plans (arrangements)
- `qokaiin` (count=122) → Italian anagram `ciancio`; English: [n/a]
- `okaiin` (count=109) → Italian anagram `coniai`; English: [n/a]
- `qokain` (count=101) → Italian anagram `acconi`; English: [n/a]
- `okain` (count=69) → Italian anagram `acino`; English: a berry
- `otain` (count=53) → Italian anagram `anito`; English: [n/a]
- `qokar` (count=48) → Italian anagram `carco`; English: [n/a]
- `saiin` (count=46) → Italian anagram `asini`; English: [n/a]
- `qokal` (count=43) → Italian anagram `calco`; English: cast (of sculpture)
- `qotaiin` (count=40) → Italian anagram `cationi`; English: [n/a]
- `lkaiin` (count=39) → Italian anagram `ancili`; English: [n/a]
- `kaiin` (count=37) → Italian anagram `acini`; English: [n/a]
- `qokeol` (count=37) → Italian anagram `eccolo`; English: [n/a]
- `qotain` (count=34) → Italian anagram `antico`; English: ancient
- `qotar` (count=29) → Italian anagram `corta`; English: [n/a]

### Marker evidence (substring in frequent basewords)
- `qo`: 60 basewords; examples: `qokeey`, `qokeedy`, `qokaiin`, `qokain`, `qokedy`, `qokey`
- `q`: 61 basewords; examples: `qokeey`, `qokeedy`, `qokaiin`, `qokain`, `qokedy`, `qokey`
- `o`: 262 basewords; examples: `qokeey`, `ol`, `o`, `qokeedy`, `okeey`, `qokaiin`
- `k`: 147 basewords; examples: `qokeey`, `qokeedy`, `okeey`, `qokaiin`, `okaiin`, `qokain`
- `t`: 102 basewords; examples: `otaiin`, `oteey`, `otar`, `otedy`, `otal`, `oteedy`
- `p`: 17 basewords; examples: `opchedy`, `qopchedy`, `opchey`, `pchedy`, `qopchdy`, `opchdy`
- `ch`: 137 basewords; examples: `chedy`, `chey`, `chol`, `cheey`, `cheol`, `cheody`
- `sh`: 50 basewords; examples: `shedy`, `shey`, `sheey`, `sheol`, `shol`, `sheedy`
- `f`: 1 basewords; examples: `f`
- `cth`: 16 basewords; examples: `chcthy`, `cthey`, `shcthy`, `checthy`, `cthol`, `ctheey`
- `ckh`: 15 basewords; examples: `chckhy`, `shckhy`, `checkhy`, `chckhey`, `chockhy`, `sheckhy`
- `cph`: 2 basewords; examples: `cphol`, `cphy`
- `dy`: 84 basewords; examples: `chedy`, `qokeedy`, `shedy`, `otedy`, `oteedy`, `qokedy`
- `iin`: 39 basewords; examples: `aiin`, `daiin`, `qokaiin`, `okaiin`, `otaiin`, `saiin`
- `aiin`: 33 basewords; examples: `aiin`, `daiin`, `qokaiin`, `okaiin`, `otaiin`, `saiin`

## Recipes Index (This Page)
- [f111r.1,@P0](#f111r-1-f111r-1-p0)
- [f111r.2,+P0](#f111r-2-f111r-2-p0)
- [f111r.3,+P0](#f111r-3-f111r-3-p0)
- [f111r.4,+P0](#f111r-4-f111r-4-p0)
- [f111r.5,+P0](#f111r-5-f111r-5-p0)
- [f111r.6,+P0](#f111r-6-f111r-6-p0)
- [f111r.7,+P0](#f111r-7-f111r-7-p0)
- [f111r.8,+P0](#f111r-8-f111r-8-p0)
- [f111r.9,+P0](#f111r-9-f111r-9-p0)
- [f111r.10,+P0](#f111r-10-f111r-10-p0)
- [f111r.11,+P0](#f111r-11-f111r-11-p0)
- [f111r.12,+P0](#f111r-12-f111r-12-p0)
- [f111r.13,+P0](#f111r-13-f111r-13-p0)
- [f111r.14,+P0](#f111r-14-f111r-14-p0)
- [f111r.15,+P0](#f111r-15-f111r-15-p0)
- [f111r.16,+P0](#f111r-16-f111r-16-p0)
- [f111r.17,+P0](#f111r-17-f111r-17-p0)
- [f111r.18,+P0](#f111r-18-f111r-18-p0)
- [f111r.19,+P0](#f111r-19-f111r-19-p0)
- [f111r.20,+P0](#f111r-20-f111r-20-p0)
- [f111r.21,+P0](#f111r-21-f111r-21-p0)
- [f111r.22,+P0](#f111r-22-f111r-22-p0)
- [f111r.23,+P0](#f111r-23-f111r-23-p0)
- [f111r.24,+P0](#f111r-24-f111r-24-p0)
- [f111r.25,+P0](#f111r-25-f111r-25-p0)
- [f111r.26,+P0](#f111r-26-f111r-26-p0)
- [f111r.27,+P0](#f111r-27-f111r-27-p0)
- [f111r.28,+P0](#f111r-28-f111r-28-p0)
- [f111r.29,+P0](#f111r-29-f111r-29-p0)
- [f111r.30,+P0](#f111r-30-f111r-30-p0)
- [f111r.31,+P0](#f111r-31-f111r-31-p0)
- [f111r.32,+P0](#f111r-32-f111r-32-p0)
- [f111r.33,+P0](#f111r-33-f111r-33-p0)
- [f111r.34,+P0](#f111r-34-f111r-34-p0)
- [f111r.35,+P0](#f111r-35-f111r-35-p0)
- [f111r.36,+P0](#f111r-36-f111r-36-p0)
- [f111r.37,+P0](#f111r-37-f111r-37-p0)
- [f111r.38,+P0](#f111r-38-f111r-38-p0)
- [f111r.39,+P0](#f111r-39-f111r-39-p0)
- [f111r.40,+P0](#f111r-40-f111r-40-p0)
- [f111r.41,+P0](#f111r-41-f111r-41-p0)
- [f111r.42,+P0](#f111r-42-f111r-42-p0)
- [f111r.43,+P0](#f111r-43-f111r-43-p0)
- [f111r.44,+P0](#f111r-44-f111r-44-p0)
- [f111r.45,+P0](#f111r-45-f111r-45-p0)
- [f111r.46,+P0](#f111r-46-f111r-46-p0)
- [f111r.47,+P0](#f111r-47-f111r-47-p0)
- [f111r.48,+P0](#f111r-48-f111r-48-p0)
- [f111r.49,+P0](#f111r-49-f111r-49-p0)
- [f111r.50,+P0](#f111r-50-f111r-50-p0)
- [f111r.51,+P0](#f111r-51-f111r-51-p0)
- [f111r.52,+P0](#f111r-52-f111r-52-p0)
- [f111r.53,+P0](#f111r-53-f111r-53-p0)
- [f111r.54,+P0](#f111r-54-f111r-54-p0)

## Line Glosses (Procedural Gloss Only; Not a Translation)

<a id="f111r-1-f111r-1-p0"></a>

### f111r.1,@P0

EVA: kcholchdar shar aiip chepchedy chetalshy sheek shear shey ror am shey

Direct Gloss (Procedural, Not a Real Translation):
- kcholchdar: tokens: k ch o l ch p a r → connectors: l r → vowel_run: a (level 1; class a)
- shar: tokens: sh a r → connectors: r → vowel_run: a (level 1; class a)
- aiip: tokens: a ii p → vowel_run: a (level 1; class a)
- chepchedy: tokens: ch e p ch e p → vowel_run: e (level 1; class e)
- chetalshy: tokens: ch e t a l sh → connectors: l → vowel_run: e (level 1; class e)
- sheek: tokens: sh ee k → vowel_run: ee (level 2; class e)
- shear: tokens: sh e a r → connectors: r → vowel_run: e (level 1; class e)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- ror: tokens: r o r → connectors: r r
- am: tokens: a m → connectors: m → vowel_run: a (level 1; class a)
- shey: tokens: sh e → vowel_run: e (level 1; class e)

<a id="f111r-2-f111r-2-p0"></a>

### f111r.2,+P0

EVA: daiin sheeky okeey okeey qaal shedy okeey oteey shedy chcthy lcheol oteeam

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- sheeky: tokens: sh ee k → vowel_run: ee (level 2; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- qaal: tokens: q a a l → connectors: l → vowel_run: aa (level 2; class a)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- chcthy: tokens: ch cth
- lcheol: tokens: l ch e o l → connectors: l l → vowel_run: e (level 1; class e)
- oteeam: tokens: o t ee a m → connectors: m → vowel_run: ee (level 2; class e)

<a id="f111r-3-f111r-3-p0"></a>

### f111r.3,+P0

EVA: dsheedy lkeedy chckhy lchedy qokeey qokear chal qokeear cheokedy sal lokam

Direct Gloss (Procedural, Not a Real Translation):
- dsheedy: tokens: p sh ee p → vowel_run: ee (level 2; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- chckhy: tokens: ch ckh
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokear: tokens: qo k e a r → connectors: r → vowel_run: e (level 1; class e)
- chal: tokens: ch a l → connectors: l → vowel_run: a (level 1; class a)
- qokeear: tokens: qo k ee a r → connectors: r → vowel_run: ee (level 2; class e)
- cheokedy: tokens: ch e o k e p → vowel_run: e (level 1; class e)
- sal: tokens: s a l → connectors: s l → vowel_run: a (level 1; class a)
- lokam: tokens: l o k a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f111r-4-f111r-4-p0"></a>

### f111r.4,+P0

EVA: saiin oteedy qokeey daiin okedal chedy qokedy l shedy chctchy okeey lor ar am

Direct Gloss (Procedural, Not a Real Translation):
- saiin: tokens: s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okedal: tokens: o k e p a l → connectors: l → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- chctchy: tokens: ch c t ch
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- lor: tokens: l o r → connectors: l r
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- am: tokens: a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f111r-5-f111r-5-p0"></a>

### f111r.5,+P0

EVA: saiin sheekshy ol shedy chokchey lkey otain

Direct Gloss (Procedural, Not a Real Translation):
- saiin: tokens: s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin
- sheekshy: tokens: sh ee k sh → vowel_run: ee (level 2; class e)
- ol: tokens: o l → connectors: l
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- chokchey: tokens: ch o k ch e → vowel_run: e (level 1; class e)
- lkey: tokens: l k e → connectors: l → vowel_run: e (level 1; class e)
- otain: tokens: o t a i n → connectors: n → vowel_run: a (level 1; class a)

<a id="f111r-6-f111r-6-p0"></a>

### f111r.6,+P0

EVA: dain shedy qoky chedy qok shed olchedy qokeedy teedy chdy keedy qotchedy dary

Direct Gloss (Procedural, Not a Real Translation):
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qoky: tokens: qo k
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qok: tokens: qo k
- shed: tokens: sh e p → vowel_run: e (level 1; class e)
- olchedy: tokens: o l ch e p → connectors: l → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- teedy: tokens: t ee p → vowel_run: ee (level 2; class e)
- chdy: tokens: ch p
- keedy: tokens: k ee p → vowel_run: ee (level 2; class e)
- qotchedy: tokens: qo t ch e p → vowel_run: e (level 1; class e)
- dary: tokens: p a r → connectors: r → vowel_run: a (level 1; class a)

<a id="f111r-7-f111r-7-p0"></a>

### f111r.7,+P0

EVA: sshedy qokeey checthey oteedy lchedy chol sheeol qokeody raiin otedy otar aiin om

Direct Gloss (Procedural, Not a Real Translation):
- sshedy: tokens: s sh e p → connectors: s → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- checthey: tokens: ch e cth e → vowel_run: e (level 1; class e)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- chol: tokens: ch o l → connectors: l
- sheeol: tokens: sh ee o l → connectors: l → vowel_run: ee (level 2; class e)
- qokeody: tokens: qo k e o p → vowel_run: e (level 1; class e)
- raiin: tokens: r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- otar: tokens: o t a r → connectors: r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- om: tokens: o m → connectors: m

<a id="f111r-8-f111r-8-p0"></a>

### f111r.8,+P0

EVA: daiin o chedain daiin cheedy qokeey qokedy chckhhy otshedy lkeeol lkeey qotchdy

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- o: tokens: o
- chedain: tokens: ch e p a i n → connectors: n → vowel_run: e (level 1; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- cheedy: tokens: ch ee p → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- chckhhy: tokens: ch ckh h → unmodeled_tokens: h
- otshedy: tokens: o t sh e p → vowel_run: e (level 1; class e)
- lkeeol: tokens: l k ee o l → connectors: l l → vowel_run: ee (level 2; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- qotchdy: tokens: qo t ch p

<a id="f111r-9-f111r-9-p0"></a>

### f111r.9,+P0

EVA: ycheeodain okeeo olchedy lchedy qokeey okeeedy okain chedy chedyteey dal lam

Direct Gloss (Procedural, Not a Real Translation):
- ycheeodain: tokens: ch ee o p a i n → connectors: n → vowel_run: ee (level 2; class e)
- okeeo: tokens: o k ee o → vowel_run: ee (level 2; class e)
- olchedy: tokens: o l ch e p → connectors: l → vowel_run: e (level 1; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- okeeedy: tokens: o k eee p → vowel_run: eee (level 3; class e)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- chedyteey: tokens: ch e p t ee → vowel_run: e (level 1; class e)
- dal: tokens: p a l → connectors: l → vowel_run: a (level 1; class a)
- lam: tokens: l a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f111r-10-f111r-10-p0"></a>

### f111r.10,+P0

EVA: ykeedaiin shekain shedy qokear ochey reeey qokeey olaiiin chedy lkeeody oraiin oty

Direct Gloss (Procedural, Not a Real Translation):
- ykeedaiin: tokens: k ee p aiin → vowel_run: ee (level 2; class e) → suffix: aiin
- shekain: tokens: sh e k a i n → connectors: n → vowel_run: e (level 1; class e)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokear: tokens: qo k e a r → connectors: r → vowel_run: e (level 1; class e)
- ochey: tokens: o ch e → vowel_run: e (level 1; class e)
- reeey: tokens: r eee → connectors: r → vowel_run: eee (level 3; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- olaiiin: tokens: o l a iii n → connectors: l n → vowel_run: a (level 1; class a) → suffix: iin
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- lkeeody: tokens: l k ee o p → connectors: l → vowel_run: ee (level 2; class e)
- oraiin: tokens: o r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- oty: tokens: o t

<a id="f111r-11-f111r-11-p0"></a>

### f111r.11,+P0

EVA: dshey lkeedy lkeedy cheedy oteedaiin sheedy ar aikhy shkaiin chey daiin daram

Direct Gloss (Procedural, Not a Real Translation):
- dshey: tokens: p sh e → vowel_run: e (level 1; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- cheedy: tokens: ch ee p → vowel_run: ee (level 2; class e)
- oteedaiin: tokens: o t ee p aiin → vowel_run: ee (level 2; class e) → suffix: aiin
- sheedy: tokens: sh ee p → vowel_run: ee (level 2; class e)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- aikhy: tokens: a i k h → vowel_run: a (level 1; class a) → unmodeled_tokens: h
- shkaiin: tokens: sh k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- daram: tokens: p a r a m → connectors: r m → vowel_run: a (level 1; class a)

<a id="f111r-12-f111r-12-p0"></a>

### f111r.12,+P0

EVA: dsheeo qokeedy otchedey lshey lkeedy oteey qokeedy oteolair ar shedam cheam

Direct Gloss (Procedural, Not a Real Translation):
- dsheeo: tokens: p sh ee o → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- otchedey: tokens: o t ch e p e → vowel_run: e (level 1; class e)
- lshey: tokens: l sh e → connectors: l → vowel_run: e (level 1; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- oteolair: tokens: o t e o l a i r → connectors: l r → vowel_run: e (level 1; class e)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- shedam: tokens: sh e p a m → connectors: m → vowel_run: e (level 1; class e)
- cheam: tokens: ch e a m → connectors: m → vowel_run: e (level 1; class e)

<a id="f111r-13-f111r-13-p0"></a>

### f111r.13,+P0

EVA: qosheo lchdy lshedy olkeeedy lr al chr or dain shey arain chey teey cheodaiin

Direct Gloss (Procedural, Not a Real Translation):
- qosheo: tokens: qo sh e o → vowel_run: e (level 1; class e)
- lchdy: tokens: l ch p → connectors: l
- lshedy: tokens: l sh e p → connectors: l → vowel_run: e (level 1; class e)
- olkeeedy: tokens: o l k eee p → connectors: l → vowel_run: eee (level 3; class e)
- lr: tokens: l r → connectors: l r
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- chr: tokens: ch r → connectors: r
- or: tokens: o r → connectors: r
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- arain: tokens: a r a i n → connectors: r n → vowel_run: a (level 1; class a)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- teey: tokens: t ee → vowel_run: ee (level 2; class e)
- cheodaiin: tokens: ch e o p aiin → vowel_run: e (level 1; class e) → suffix: aiin

<a id="f111r-14-f111r-14-p0"></a>

### f111r.14,+P0

EVA: dchedar oteey lchey ykeeeos aiin shear oteedy tedam oteey pchedey chedy lchedy

Direct Gloss (Procedural, Not a Real Translation):
- dchedar: tokens: p ch e p a r → connectors: r → vowel_run: e (level 1; class e)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- lchey: tokens: l ch e → connectors: l → vowel_run: e (level 1; class e)
- ykeeeos: tokens: k eee o s → connectors: s → vowel_run: eee (level 3; class e)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- shear: tokens: sh e a r → connectors: r → vowel_run: e (level 1; class e)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- tedam: tokens: t e p a m → connectors: m → vowel_run: e (level 1; class e)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- pchedey: tokens: p ch e p e → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)

<a id="f111r-15-f111r-15-p0"></a>

### f111r.15,+P0

EVA: sheeodar aiin sheey shey oteody oteedy chey daiin oteol otedar tar pchdam

Direct Gloss (Procedural, Not a Real Translation):
- sheeodar: tokens: sh ee o p a r → connectors: r → vowel_run: ee (level 2; class e)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- sheey: tokens: sh ee → vowel_run: ee (level 2; class e)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- oteody: tokens: o t e o p → vowel_run: e (level 1; class e)
- oteedy: tokens: o t ee p → vowel_run: ee (level 2; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- oteol: tokens: o t e o l → connectors: l → vowel_run: e (level 1; class e)
- otedar: tokens: o t e p a r → connectors: r → vowel_run: e (level 1; class e)
- tar: tokens: t a r → connectors: r → vowel_run: a (level 1; class a)
- pchdam: tokens: p ch p a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f111r-16-f111r-16-p0"></a>

### f111r.16,+P0

EVA: dchedshey qokeedy okeodain shet sheota[?:im] oteal tey lkeey sheedy shtar keedy

Direct Gloss (Procedural, Not a Real Translation):
- dchedshey: tokens: p ch e p sh e → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- okeodain: tokens: o k e o p a i n → connectors: n → vowel_run: e (level 1; class e)
- shet: tokens: sh e t → vowel_run: e (level 1; class e)
- sheota: tokens: sh e o t a → vowel_run: e (level 1; class e)
- im: tokens: i m → connectors: m → vowel_run: i (level 1; class i)
- oteal: tokens: o t e a l → connectors: l → vowel_run: e (level 1; class e)
- tey: tokens: t e → vowel_run: e (level 1; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- sheedy: tokens: sh ee p → vowel_run: ee (level 2; class e)
- shtar: tokens: sh t a r → connectors: r → vowel_run: a (level 1; class a)
- keedy: tokens: k ee p → vowel_run: ee (level 2; class e)

<a id="f111r-17-f111r-17-p0"></a>

### f111r.17,+P0

EVA: otes lchey lshedy qoteedy olcheody qokeey chey kal chekeedy oteed keedy otam

Direct Gloss (Procedural, Not a Real Translation):
- otes: tokens: o t e s → connectors: s → vowel_run: e (level 1; class e)
- lchey: tokens: l ch e → connectors: l → vowel_run: e (level 1; class e)
- lshedy: tokens: l sh e p → connectors: l → vowel_run: e (level 1; class e)
- qoteedy: tokens: qo t ee p → vowel_run: ee (level 2; class e)
- olcheody: tokens: o l ch e o p → connectors: l → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- kal: tokens: k a l → connectors: l → vowel_run: a (level 1; class a)
- chekeedy: tokens: ch e k ee p → vowel_run: e (level 1; class e)
- oteed: tokens: o t ee p → vowel_run: ee (level 2; class e)
- keedy: tokens: k ee p → vowel_run: ee (level 2; class e)
- otam: tokens: o t a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f111r-18-f111r-18-p0"></a>

### f111r.18,+P0

EVA: daiin shecthey oteody qochey qokeey chey teey qokeedy qokeedy cheol lokeedy lched

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- shecthey: tokens: sh e cth e → vowel_run: e (level 1; class e)
- oteody: tokens: o t e o p → vowel_run: e (level 1; class e)
- qochey: tokens: qo ch e → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- teey: tokens: t ee → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- cheol: tokens: ch e o l → connectors: l → vowel_run: e (level 1; class e)
- lokeedy: tokens: l o k ee p → connectors: l → vowel_run: ee (level 2; class e)
- lched: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)

<a id="f111r-19-f111r-19-p0"></a>

### f111r.19,+P0

EVA: saiin okeeol qokeedy lchey lcheoekam oaiin cheky qokeeodar checthdy qoted l loty

Direct Gloss (Procedural, Not a Real Translation):
- saiin: tokens: s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin
- okeeol: tokens: o k ee o l → connectors: l → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- lchey: tokens: l ch e → connectors: l → vowel_run: e (level 1; class e)
- lcheoekam: tokens: l ch e o e k a m → connectors: l m → vowel_run: e (level 1; class e)
- oaiin: tokens: o aiin → vowel_run: a (level 1; class a) → suffix: aiin
- cheky: tokens: ch e k → vowel_run: e (level 1; class e)
- qokeeodar: tokens: qo k ee o p a r → connectors: r → vowel_run: ee (level 2; class e)
- checthdy: tokens: ch e cth p → vowel_run: e (level 1; class e)
- qoted: tokens: qo t e p → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- loty: tokens: l o t → connectors: l

<a id="f111r-20-f111r-20-p0"></a>

### f111r.20,+P0

EVA: qokeey qokeey l[ch:a]edy checthy chedey cheol teedy okeedy qokeedy okeey cho ol l kai@175;

Direct Gloss (Procedural, Not a Real Translation):
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- l: tokens: l → connectors: l
- ch: tokens: ch
- a: tokens: a → vowel_run: a (level 1; class a)
- edy: tokens: e p → vowel_run: e (level 1; class e)
- checthy: tokens: ch e cth → vowel_run: e (level 1; class e)
- chedey: tokens: ch e p e → vowel_run: e (level 1; class e)
- cheol: tokens: ch e o l → connectors: l → vowel_run: e (level 1; class e)
- teedy: tokens: t ee p → vowel_run: ee (level 2; class e)
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- cho: tokens: ch o
- ol: tokens: o l → connectors: l
- l: tokens: l → connectors: l
- kai: tokens: k a i → vowel_run: a (level 1; class a)

<a id="f111r-21-f111r-21-p0"></a>

### f111r.21,+P0

EVA: daiin okar ain teey oteey ox o r shey chey ky deedy cheo dain sheet ar al l cheotydy

Direct Gloss (Procedural, Not a Real Translation):
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okar: tokens: o k a r → connectors: r → vowel_run: a (level 1; class a)
- ain: tokens: a i n → connectors: n → vowel_run: a (level 1; class a)
- teey: tokens: t ee → vowel_run: ee (level 2; class e)
- oteey: tokens: o t ee → vowel_run: ee (level 2; class e)
- ox: tokens: o x
- o: tokens: o
- r: tokens: r → connectors: r
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- ky: tokens: k
- deedy: tokens: p ee p → vowel_run: ee (level 2; class e)
- cheo: tokens: ch e o → vowel_run: e (level 1; class e)
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- sheet: tokens: sh ee t → vowel_run: ee (level 2; class e)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- l: tokens: l → connectors: l
- cheotydy: tokens: ch e o t p → vowel_run: e (level 1; class e)

<a id="f111r-22-f111r-22-p0"></a>

### f111r.22,+P0

EVA: ssheo l keedy dal tched checkhey okey qo ain qo chedy qokeor okeey keo cthedy qokey

Direct Gloss (Procedural, Not a Real Translation):
- ssheo: tokens: s sh e o → connectors: s → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- keedy: tokens: k ee p → vowel_run: ee (level 2; class e)
- dal: tokens: p a l → connectors: l → vowel_run: a (level 1; class a)
- tched: tokens: t ch e p → vowel_run: e (level 1; class e)
- checkhey: tokens: ch e ckh e → vowel_run: e (level 1; class e)
- okey: tokens: o k e → vowel_run: e (level 1; class e)
- qo: tokens: qo
- ain: tokens: a i n → connectors: n → vowel_run: a (level 1; class a)
- qo: tokens: qo
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokeor: tokens: qo k e o r → connectors: r → vowel_run: e (level 1; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- keo: tokens: k e o → vowel_run: e (level 1; class e)
- cthedy: tokens: cth e p → vowel_run: e (level 1; class e)
- qokey: tokens: qo k e → vowel_run: e (level 1; class e)

<a id="f111r-23-f111r-23-p0"></a>

### f111r.23,+P0

EVA: dshey sheeckhy qokeshey keeshody qokeey qokedy okain shekal okechedy qokechedy

Direct Gloss (Procedural, Not a Real Translation):
- dshey: tokens: p sh e → vowel_run: e (level 1; class e)
- sheeckhy: tokens: sh ee ckh → vowel_run: ee (level 2; class e)
- qokeshey: tokens: qo k e sh e → vowel_run: e (level 1; class e)
- keeshody: tokens: k ee sh o p → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a)
- shekal: tokens: sh e k a l → connectors: l → vowel_run: e (level 1; class e)
- okechedy: tokens: o k e ch e p → vowel_run: e (level 1; class e)
- qokechedy: tokens: qo k e ch e p → vowel_run: e (level 1; class e)

<a id="f111r-24-f111r-24-p0"></a>

### f111r.24,+P0

EVA: sain chckhey cheos lchal lkechey okeey lkechedy oteodain char lky lchr ainai@175;

Direct Gloss (Procedural, Not a Real Translation):
- sain: tokens: s a i n → connectors: s n → vowel_run: a (level 1; class a)
- chckhey: tokens: ch ckh e → vowel_run: e (level 1; class e)
- cheos: tokens: ch e o s → connectors: s → vowel_run: e (level 1; class e)
- lchal: tokens: l ch a l → connectors: l l → vowel_run: a (level 1; class a)
- lkechey: tokens: l k e ch e → connectors: l → vowel_run: e (level 1; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- lkechedy: tokens: l k e ch e p → connectors: l → vowel_run: e (level 1; class e)
- oteodain: tokens: o t e o p a i n → connectors: n → vowel_run: e (level 1; class e)
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- lky: tokens: l k → connectors: l
- lchr: tokens: l ch r → connectors: l r
- ainai: tokens: a i n a i → connectors: n → vowel_run: a (level 1; class a)

<a id="f111r-25-f111r-25-p0"></a>

### f111r.25,+P0

EVA: so sheor okeedy oteeey qokeey qokeeo s chedar alal oikhy oxar aiin odaiin chody

Direct Gloss (Procedural, Not a Real Translation):
- so: tokens: s o → connectors: s
- sheor: tokens: sh e o r → connectors: r → vowel_run: e (level 1; class e)
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- oteeey: tokens: o t eee → vowel_run: eee (level 3; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokeeo: tokens: qo k ee o → vowel_run: ee (level 2; class e)
- s: tokens: s → connectors: s
- chedar: tokens: ch e p a r → connectors: r → vowel_run: e (level 1; class e)
- alal: tokens: a l a l → connectors: l l → vowel_run: a (level 1; class a)
- oikhy: tokens: o i k h → vowel_run: i (level 1; class i) → unmodeled_tokens: h
- oxar: tokens: o x a r → connectors: r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- odaiin: tokens: o p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chody: tokens: ch o p

<a id="f111r-26-f111r-26-p0"></a>

### f111r.26,+P0

EVA: soaiin shecthy oteodaiin chedy otedar air ykeey shedain l chedy qokeedy qoteedaiin

Direct Gloss (Procedural, Not a Real Translation):
- soaiin: tokens: s o aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin
- shecthy: tokens: sh e cth → vowel_run: e (level 1; class e)
- oteodaiin: tokens: o t e o p aiin → vowel_run: e (level 1; class e) → suffix: aiin
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- otedar: tokens: o t e p a r → connectors: r → vowel_run: e (level 1; class e)
- air: tokens: a i r → connectors: r → vowel_run: a (level 1; class a)
- ykeey: tokens: k ee → vowel_run: ee (level 2; class e)
- shedain: tokens: sh e p a i n → connectors: n → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qoteedaiin: tokens: qo t ee p aiin → vowel_run: ee (level 2; class e) → suffix: aiin

<a id="f111r-27-f111r-27-p0"></a>

### f111r.27,+P0

EVA: dar sheod qokeey qokeody otey lkeedy chl lkeey daiin chedar chedyteedain olkal

Direct Gloss (Procedural, Not a Real Translation):
- dar: tokens: p a r → connectors: r → vowel_run: a (level 1; class a)
- sheod: tokens: sh e o p → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokeody: tokens: qo k e o p → vowel_run: e (level 1; class e)
- otey: tokens: o t e → vowel_run: e (level 1; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- chl: tokens: ch l → connectors: l
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chedar: tokens: ch e p a r → connectors: r → vowel_run: e (level 1; class e)
- chedyteedain: tokens: ch e p t ee p a i n → connectors: n → vowel_run: e (level 1; class e)
- olkal: tokens: o l k a l → connectors: l l → vowel_run: a (level 1; class a)

<a id="f111r-28-f111r-28-p0"></a>

### f111r.28,+P0

EVA: qokeeol shedy qokeod cheokeey sheokar shekeey qotees al dain chedy qotain alam

Direct Gloss (Procedural, Not a Real Translation):
- qokeeol: tokens: qo k ee o l → connectors: l → vowel_run: ee (level 2; class e)
- shedy: tokens: sh e p → vowel_run: e (level 1; class e)
- qokeod: tokens: qo k e o p → vowel_run: e (level 1; class e)
- cheokeey: tokens: ch e o k ee → vowel_run: e (level 1; class e)
- sheokar: tokens: sh e o k a r → connectors: r → vowel_run: e (level 1; class e)
- shekeey: tokens: sh e k ee → vowel_run: e (level 1; class e)
- qotees: tokens: qo t ee s → connectors: s → vowel_run: ee (level 2; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qotain: tokens: qo t a i n → connectors: n → vowel_run: a (level 1; class a)
- alam: tokens: a l a m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f111r-29-f111r-29-p0"></a>

### f111r.29,+P0

EVA: salkeedy lkeedy chckhy rky chey okeeo chr al ar qokeedy qokedy qoteedy qod

Direct Gloss (Procedural, Not a Real Translation):
- salkeedy: tokens: s a l k ee p → connectors: s l → vowel_run: a (level 1; class a)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- chckhy: tokens: ch ckh
- rky: tokens: r k → connectors: r
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- okeeo: tokens: o k ee o → vowel_run: ee (level 2; class e)
- chr: tokens: ch r → connectors: r
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- qokedy: tokens: qo k e p → vowel_run: e (level 1; class e)
- qoteedy: tokens: qo t ee p → vowel_run: ee (level 2; class e)
- qod: tokens: qo p

<a id="f111r-30-f111r-30-p0"></a>

### f111r.30,+P0

EVA: qockhedy qokeechy qotchey chtar ar chkeey lkar aiin chedy qokeey q[o:?]irain

Direct Gloss (Procedural, Not a Real Translation):
- qockhedy: tokens: qo ckh e p → vowel_run: e (level 1; class e)
- qokeechy: tokens: qo k ee ch → vowel_run: ee (level 2; class e)
- qotchey: tokens: qo t ch e → vowel_run: e (level 1; class e)
- chtar: tokens: ch t a r → connectors: r → vowel_run: a (level 1; class a)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- chkeey: tokens: ch k ee → vowel_run: ee (level 2; class e)
- lkar: tokens: l k a r → connectors: l r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- q: tokens: q
- o: tokens: o
- irain: tokens: i r a i n → connectors: r n → vowel_run: i (level 1; class i)

<a id="f111r-31-f111r-31-p0"></a>

### f111r.31,+P0

EVA: sheotchedy shedain shkeyqokeaiin shkeody qokeey qokeedy chkal checthy

Direct Gloss (Procedural, Not a Real Translation):
- sheotchedy: tokens: sh e o t ch e p → vowel_run: e (level 1; class e)
- shedain: tokens: sh e p a i n → connectors: n → vowel_run: e (level 1; class e)
- shkeyqokeaiin: tokens: sh k e qo k e aiin → vowel_run: e (level 1; class e) → suffix: aiin
- shkeody: tokens: sh k e o p → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- chkal: tokens: ch k a l → connectors: l → vowel_run: a (level 1; class a)
- checthy: tokens: ch e cth → vowel_run: e (level 1; class e)

<a id="f111r-32-f111r-32-p0"></a>

### f111r.32,+P0

EVA: dsheeoteey qokeeo l keey tear qoteey qotain cho lkeeedy lchechdy qokechy

Direct Gloss (Procedural, Not a Real Translation):
- dsheeoteey: tokens: p sh ee o t ee → vowel_run: ee (level 2; class e)
- qokeeo: tokens: qo k ee o → vowel_run: ee (level 2; class e)
- l: tokens: l → connectors: l
- keey: tokens: k ee → vowel_run: ee (level 2; class e)
- tear: tokens: t e a r → connectors: r → vowel_run: e (level 1; class e)
- qoteey: tokens: qo t ee → vowel_run: ee (level 2; class e)
- qotain: tokens: qo t a i n → connectors: n → vowel_run: a (level 1; class a)
- cho: tokens: ch o
- lkeeedy: tokens: l k eee p → connectors: l → vowel_run: eee (level 3; class e)
- lchechdy: tokens: l ch e ch p → connectors: l → vowel_run: e (level 1; class e)
- qokechy: tokens: qo k e ch → vowel_run: e (level 1; class e)

<a id="f111r-33-f111r-33-p0"></a>

### f111r.33,+P0

EVA: qoeeeo [s:d] ain ar ol keey lkair cheda chkal keeodal eees chody qokeedy dl

Direct Gloss (Procedural, Not a Real Translation):
- qoeeeo: tokens: qo eee o → vowel_run: eee (level 3; class e)
- s: tokens: s → connectors: s
- d: tokens: p
- ain: tokens: a i n → connectors: n → vowel_run: a (level 1; class a)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- ol: tokens: o l → connectors: l
- keey: tokens: k ee → vowel_run: ee (level 2; class e)
- lkair: tokens: l k a i r → connectors: l r → vowel_run: a (level 1; class a)
- cheda: tokens: ch e p a → vowel_run: e (level 1; class e)
- chkal: tokens: ch k a l → connectors: l → vowel_run: a (level 1; class a)
- keeodal: tokens: k ee o p a l → connectors: l → vowel_run: ee (level 2; class e)
- eees: tokens: eee s → connectors: s → vowel_run: eee (level 3; class e)
- chody: tokens: ch o p
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- dl: tokens: p l → connectors: l

<a id="f111r-34-f111r-34-p0"></a>

### f111r.34,+P0

EVA: teo olkeeo al lkeeey olain ar akeom qokain al qokeeod lkedlkey teedar

Direct Gloss (Procedural, Not a Real Translation):
- teo: tokens: t e o → vowel_run: e (level 1; class e)
- olkeeo: tokens: o l k ee o → connectors: l → vowel_run: ee (level 2; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- lkeeey: tokens: l k eee → connectors: l → vowel_run: eee (level 3; class e)
- olain: tokens: o l a i n → connectors: l n → vowel_run: a (level 1; class a)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- akeom: tokens: a k e o m → connectors: m → vowel_run: a (level 1; class a)
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- qokeeod: tokens: qo k ee o p → vowel_run: ee (level 2; class e)
- lkedlkey: tokens: l k e p l k e → connectors: l l → vowel_run: e (level 1; class e)
- teedar: tokens: t ee p a r → connectors: r → vowel_run: ee (level 2; class e)

<a id="f111r-35-f111r-35-p0"></a>

### f111r.35,+P0

EVA: y chedl ar aiin aiy dam

Direct Gloss (Procedural, Not a Real Translation):
- y: [unparsed]
- chedl: tokens: ch e p l → connectors: l → vowel_run: e (level 1; class e)
- ar: tokens: a r → connectors: r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- aiy: tokens: a i → vowel_run: a (level 1; class a)
- dam: tokens: p a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f111r-36-f111r-36-p0"></a>

### f111r.36,+P0

EVA: polal shedal pchedy qofchedy pykeor cheefy qopchey opchedal lfchedy pchal

Direct Gloss (Procedural, Not a Real Translation):
- polal: tokens: p o l a l → connectors: l l → vowel_run: a (level 1; class a)
- shedal: tokens: sh e p a l → connectors: l → vowel_run: e (level 1; class e)
- pchedy: tokens: p ch e p → vowel_run: e (level 1; class e)
- qofchedy: tokens: qo f ch e p → vowel_run: e (level 1; class e)
- pykeor: tokens: p k e o r → connectors: r → vowel_run: e (level 1; class e)
- cheefy: tokens: ch ee f → vowel_run: ee (level 2; class e)
- qopchey: tokens: qo p ch e → vowel_run: e (level 1; class e)
- opchedal: tokens: o p ch e p a l → connectors: l → vowel_run: e (level 1; class e)
- lfchedy: tokens: l f ch e p → connectors: l → vowel_run: e (level 1; class e)
- pchal: tokens: p ch a l → connectors: l → vowel_run: a (level 1; class a)

<a id="f111r-37-f111r-37-p0"></a>

### f111r.37,+P0

EVA: dain chol l cheeol keeol lkeeedy qokeey l keedy l kedy okeal kain chckhy ltedy

Direct Gloss (Procedural, Not a Real Translation):
- dain: tokens: p a i n → connectors: n → vowel_run: a (level 1; class a)
- chol: tokens: ch o l → connectors: l
- l: tokens: l → connectors: l
- cheeol: tokens: ch ee o l → connectors: l → vowel_run: ee (level 2; class e)
- keeol: tokens: k ee o l → connectors: l → vowel_run: ee (level 2; class e)
- lkeeedy: tokens: l k eee p → connectors: l → vowel_run: eee (level 3; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- l: tokens: l → connectors: l
- keedy: tokens: k ee p → vowel_run: ee (level 2; class e)
- l: tokens: l → connectors: l
- kedy: tokens: k e p → vowel_run: e (level 1; class e)
- okeal: tokens: o k e a l → connectors: l → vowel_run: e (level 1; class e)
- kain: tokens: k a i n → connectors: n → vowel_run: a (level 1; class a)
- chckhy: tokens: ch ckh
- ltedy: tokens: l t e p → connectors: l → vowel_run: e (level 1; class e)

<a id="f111r-38-f111r-38-p0"></a>

### f111r.38,+P0

EVA: dched daiin al okeey kechedy chcthedy chedain otedy qokeedy key l ltain

Direct Gloss (Procedural, Not a Real Translation):
- dched: tokens: p ch e p → vowel_run: e (level 1; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- kechedy: tokens: k e ch e p → vowel_run: e (level 1; class e)
- chcthedy: tokens: ch cth e p → vowel_run: e (level 1; class e)
- chedain: tokens: ch e p a i n → connectors: n → vowel_run: e (level 1; class e)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- key: tokens: k e → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- ltain: tokens: l t a i n → connectors: l n → vowel_run: a (level 1; class a)

<a id="f111r-39-f111r-39-p0"></a>

### f111r.39,+P0

EVA: saiin chckhedy okain oteain shok qokechdy okechedy chedy qokeey cheamar

Direct Gloss (Procedural, Not a Real Translation):
- saiin: tokens: s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin
- chckhedy: tokens: ch ckh e p → vowel_run: e (level 1; class e)
- okain: tokens: o k a i n → connectors: n → vowel_run: a (level 1; class a)
- oteain: tokens: o t e a i n → connectors: n → vowel_run: e (level 1; class e)
- shok: tokens: sh o k
- qokechdy: tokens: qo k e ch p → vowel_run: e (level 1; class e)
- okechedy: tokens: o k e ch e p → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- cheamar: tokens: ch e a m a r → connectors: m r → vowel_run: e (level 1; class e)

<a id="f111r-40-f111r-40-p0"></a>

### f111r.40,+P0

EVA: saiin al keey okedain otedy qokeokedy qokeedy oteeolkeey okeeo l lainal

Direct Gloss (Procedural, Not a Real Translation):
- saiin: tokens: s aiin → connectors: s → vowel_run: a (level 1; class a) → suffix: aiin
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- keey: tokens: k ee → vowel_run: ee (level 2; class e)
- okedain: tokens: o k e p a i n → connectors: n → vowel_run: e (level 1; class e)
- otedy: tokens: o t e p → vowel_run: e (level 1; class e)
- qokeokedy: tokens: qo k e o k e p → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- oteeolkeey: tokens: o t ee o l k ee → connectors: l → vowel_run: ee (level 2; class e)
- okeeo: tokens: o k ee o → vowel_run: ee (level 2; class e)
- l: tokens: l → connectors: l
- lainal: tokens: l a i n a l → connectors: l n l → vowel_run: a (level 1; class a)

<a id="f111r-41-f111r-41-p0"></a>

### f111r.41,+P0

EVA: ycheey raiin cheody qokechy qokeey okedy lchedy qokeeal daiin otaral

Direct Gloss (Procedural, Not a Real Translation):
- ycheey: tokens: ch ee → vowel_run: ee (level 2; class e)
- raiin: tokens: r aiin → connectors: r → vowel_run: a (level 1; class a) → suffix: aiin
- cheody: tokens: ch e o p → vowel_run: e (level 1; class e)
- qokechy: tokens: qo k e ch → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- okedy: tokens: o k e p → vowel_run: e (level 1; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- qokeeal: tokens: qo k ee a l → connectors: l → vowel_run: ee (level 2; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin
- otaral: tokens: o t a r a l → connectors: r l → vowel_run: a (level 1; class a)

<a id="f111r-42-f111r-42-p0"></a>

### f111r.42,+P0

EVA: qokeol qok[a:ee]l qokeeol cho lkedy l chedy chedy chedyteokain chedy l kag am

Direct Gloss (Procedural, Not a Real Translation):
- qokeol: tokens: qo k e o l → connectors: l → vowel_run: e (level 1; class e)
- qok: tokens: qo k
- a: tokens: a → vowel_run: a (level 1; class a)
- ee: tokens: ee → vowel_run: ee (level 2; class e)
- l: tokens: l → connectors: l
- qokeeol: tokens: qo k ee o l → connectors: l → vowel_run: ee (level 2; class e)
- cho: tokens: ch o
- lkedy: tokens: l k e p → connectors: l → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- chedyteokain: tokens: ch e p t e o k a i n → connectors: n → vowel_run: e (level 1; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- kag: tokens: k a g → vowel_run: a (level 1; class a)
- am: tokens: a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f111r-43-f111r-43-p0"></a>

### f111r.43,+P0

EVA: okeedy lkeedy lkedar okeedy chedy

Direct Gloss (Procedural, Not a Real Translation):
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- lkedar: tokens: l k e p a r → connectors: l r → vowel_run: e (level 1; class e)
- okeedy: tokens: o k ee p → vowel_run: ee (level 2; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)

<a id="f111r-44-f111r-44-p0"></a>

### f111r.44,+P0

EVA: pchedar oteeol lkeedain okeam sheo teolkedain shal keey lteeal cheal oty

Direct Gloss (Procedural, Not a Real Translation):
- pchedar: tokens: p ch e p a r → connectors: r → vowel_run: e (level 1; class e)
- oteeol: tokens: o t ee o l → connectors: l → vowel_run: ee (level 2; class e)
- lkeedain: tokens: l k ee p a i n → connectors: l n → vowel_run: ee (level 2; class e)
- okeam: tokens: o k e a m → connectors: m → vowel_run: e (level 1; class e)
- sheo: tokens: sh e o → vowel_run: e (level 1; class e)
- teolkedain: tokens: t e o l k e p a i n → connectors: l n → vowel_run: e (level 1; class e)
- shal: tokens: sh a l → connectors: l → vowel_run: a (level 1; class a)
- keey: tokens: k ee → vowel_run: ee (level 2; class e)
- lteeal: tokens: l t ee a l → connectors: l l → vowel_run: ee (level 2; class e)
- cheal: tokens: ch e a l → connectors: l → vowel_run: e (level 1; class e)
- oty: tokens: o t

<a id="f111r-45-f111r-45-p0"></a>

### f111r.45,+P0

EVA: oain ain keeey teed checkhed iir okeey lkeeey okeeo lkeey lkeey lkeedy qoky

Direct Gloss (Procedural, Not a Real Translation):
- oain: tokens: o a i n → connectors: n → vowel_run: a (level 1; class a)
- ain: tokens: a i n → connectors: n → vowel_run: a (level 1; class a)
- keeey: tokens: k eee → vowel_run: eee (level 3; class e)
- teed: tokens: t ee p → vowel_run: ee (level 2; class e)
- checkhed: tokens: ch e ckh e p → vowel_run: e (level 1; class e)
- iir: tokens: ii r → connectors: r → vowel_run: ii (level 2; class i)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- lkeeey: tokens: l k eee → connectors: l → vowel_run: eee (level 3; class e)
- okeeo: tokens: o k ee o → vowel_run: ee (level 2; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- qoky: tokens: qo k

<a id="f111r-46-f111r-46-p0"></a>

### f111r.46,+P0

EVA: y cheeo lkeey oteed lkeedy okeeshey chey lkar al olchal or aiin che[o:?]m

Direct Gloss (Procedural, Not a Real Translation):
- y: [unparsed]
- cheeo: tokens: ch ee o → vowel_run: ee (level 2; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- oteed: tokens: o t ee p → vowel_run: ee (level 2; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- okeeshey: tokens: o k ee sh e → vowel_run: ee (level 2; class e)
- chey: tokens: ch e → vowel_run: e (level 1; class e)
- lkar: tokens: l k a r → connectors: l r → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- olchal: tokens: o l ch a l → connectors: l l → vowel_run: a (level 1; class a)
- or: tokens: o r → connectors: r
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- che: tokens: ch e → vowel_run: e (level 1; class e)
- o: tokens: o
- m: tokens: m → connectors: m

<a id="f111r-47-f111r-47-p0"></a>

### f111r.47,+P0

EVA: sheedy qokeey sheey qoteedy qeear al chedy okeey chedy

Direct Gloss (Procedural, Not a Real Translation):
- sheedy: tokens: sh ee p → vowel_run: ee (level 2; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- sheey: tokens: sh ee → vowel_run: ee (level 2; class e)
- qoteedy: tokens: qo t ee p → vowel_run: ee (level 2; class e)
- qeear: tokens: q ee a r → connectors: r → vowel_run: ee (level 2; class e)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)

<a id="f111r-48-f111r-48-p0"></a>

### f111r.48,+P0

EVA: poeokeey lkeedy tedain shecthy qokeedy chckhy lfchy cheal chepcham daim

Direct Gloss (Procedural, Not a Real Translation):
- poeokeey: tokens: p o e o k ee → vowel_run: e (level 1; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- tedain: tokens: t e p a i n → connectors: n → vowel_run: e (level 1; class e)
- shecthy: tokens: sh e cth → vowel_run: e (level 1; class e)
- qokeedy: tokens: qo k ee p → vowel_run: ee (level 2; class e)
- chckhy: tokens: ch ckh
- lfchy: tokens: l f ch → connectors: l
- cheal: tokens: ch e a l → connectors: l → vowel_run: e (level 1; class e)
- chepcham: tokens: ch e p ch a m → connectors: m → vowel_run: e (level 1; class e)
- daim: tokens: p a i m → connectors: m → vowel_run: a (level 1; class a)

<a id="f111r-49-f111r-49-p0"></a>

### f111r.49,+P0

EVA: dcheey keeey qockhey qokeey lchedy keeor okeey lkeedy lkeey qokaiin ol

Direct Gloss (Procedural, Not a Real Translation):
- dcheey: tokens: p ch ee → vowel_run: ee (level 2; class e)
- keeey: tokens: k eee → vowel_run: eee (level 3; class e)
- qockhey: tokens: qo ckh e → vowel_run: e (level 1; class e)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- lchedy: tokens: l ch e p → connectors: l → vowel_run: e (level 1; class e)
- keeor: tokens: k ee o r → connectors: r → vowel_run: ee (level 2; class e)
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- lkeey: tokens: l k ee → connectors: l → vowel_run: ee (level 2; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- ol: tokens: o l → connectors: l

<a id="f111r-50-f111r-50-p0"></a>

### f111r.50,+P0

EVA: okeey l chedy cheekain qokeeokain chckhy cheal k[ee:ei]l qokain chckhal lkal

Direct Gloss (Procedural, Not a Real Translation):
- okeey: tokens: o k ee → vowel_run: ee (level 2; class e)
- l: tokens: l → connectors: l
- chedy: tokens: ch e p → vowel_run: e (level 1; class e)
- cheekain: tokens: ch ee k a i n → connectors: n → vowel_run: ee (level 2; class e)
- qokeeokain: tokens: qo k ee o k a i n → connectors: n → vowel_run: ee (level 2; class e)
- chckhy: tokens: ch ckh
- cheal: tokens: ch e a l → connectors: l → vowel_run: e (level 1; class e)
- k: tokens: k
- ee: tokens: ee → vowel_run: ee (level 2; class e)
- ei: tokens: e i → vowel_run: e (level 1; class e)
- l: tokens: l → connectors: l
- qokain: tokens: qo k a i n → connectors: n → vowel_run: a (level 1; class a)
- chckhal: tokens: ch ckh a l → connectors: l → vowel_run: a (level 1; class a)
- lkal: tokens: l k a l → connectors: l l → vowel_run: a (level 1; class a)

<a id="f111r-51-f111r-51-p0"></a>

### f111r.51,+P0

EVA: polkeeo shey cheokeain chl kar r aiin char ?ain al lkeedy qokal okchy

Direct Gloss (Procedural, Not a Real Translation):
- polkeeo: tokens: p o l k ee o → connectors: l → vowel_run: ee (level 2; class e)
- shey: tokens: sh e → vowel_run: e (level 1; class e)
- cheokeain: tokens: ch e o k e a i n → connectors: n → vowel_run: e (level 1; class e)
- chl: tokens: ch l → connectors: l
- kar: tokens: k a r → connectors: r → vowel_run: a (level 1; class a)
- r: tokens: r → connectors: r
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- ain: tokens: a i n → connectors: n → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- lkeedy: tokens: l k ee p → connectors: l → vowel_run: ee (level 2; class e)
- qokal: tokens: qo k a l → connectors: l → vowel_run: a (level 1; class a)
- okchy: tokens: o k ch

<a id="f111r-52-f111r-52-p0"></a>

### f111r.52,+P0

EVA: dair al qokeey qokaiin sheal qokeain shckhy sain chckhy char aiin alom

Direct Gloss (Procedural, Not a Real Translation):
- dair: tokens: p a i r → connectors: r → vowel_run: a (level 1; class a)
- al: tokens: a l → connectors: l → vowel_run: a (level 1; class a)
- qokeey: tokens: qo k ee → vowel_run: ee (level 2; class e)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- sheal: tokens: sh e a l → connectors: l → vowel_run: e (level 1; class e)
- qokeain: tokens: qo k e a i n → connectors: n → vowel_run: e (level 1; class e)
- shckhy: tokens: sh ckh
- sain: tokens: s a i n → connectors: s n → vowel_run: a (level 1; class a)
- chckhy: tokens: ch ckh
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- alom: tokens: a l o m → connectors: l m → vowel_run: a (level 1; class a)

<a id="f111r-53-f111r-53-p0"></a>

### f111r.53,+P0

EVA: yshe aiin okshdy shkeey kain chaiin alolshey qokaiin chcthydain

Direct Gloss (Procedural, Not a Real Translation):
- yshe: tokens: sh e → vowel_run: e (level 1; class e)
- aiin: tokens: aiin → vowel_run: a (level 1; class a) → suffix: aiin
- okshdy: tokens: o k sh p
- shkeey: tokens: sh k ee → vowel_run: ee (level 2; class e)
- kain: tokens: k a i n → connectors: n → vowel_run: a (level 1; class a)
- chaiin: tokens: ch aiin → vowel_run: a (level 1; class a) → suffix: aiin
- alolshey: tokens: a l o l sh e → connectors: l l → vowel_run: a (level 1; class a)
- qokaiin: tokens: qo k aiin → vowel_run: a (level 1; class a) → suffix: aiin
- chcthydain: tokens: ch cth p a i n → connectors: n → vowel_run: a (level 1; class a)

<a id="f111r-54-f111r-54-p0"></a>

### f111r.54,+P0

EVA: tair chckhaiin dair qokal otain okal

Direct Gloss (Procedural, Not a Real Translation):
- tair: tokens: t a i r → connectors: r → vowel_run: a (level 1; class a)
- chckhaiin: tokens: ch ckh aiin → vowel_run: a (level 1; class a) → suffix: aiin
- dair: tokens: p a i r → connectors: r → vowel_run: a (level 1; class a)
- qokal: tokens: qo k a l → connectors: l → vowel_run: a (level 1; class a)
- otain: tokens: o t a i n → connectors: n → vowel_run: a (level 1; class a)
- okal: tokens: o k a l → connectors: l → vowel_run: a (level 1; class a)
