# Voynich Speculative Herbal Ferment Recipe — f94r
...
- qopchedal: prepare liquid base → add main plant (safe substitute) → start fermentation (yeast) → duration level 1 → state: active extraction
...
- olkeody: add fermentable sugars → mix / transfer → start fermentation (yeast) → duration level 1 → state: active extraction
...
- o: mix / transfer
- xockhhy: mix / transfer → add complex herbal compound (safe blend)
...
- chdy: add main plant (safe substitute) → start fermentation (yeast)
- dytchdy: apply heat/cooking → add main plant (safe substitute) → start fermentation (yeast)
