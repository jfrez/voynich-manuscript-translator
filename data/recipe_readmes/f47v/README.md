# Voynich Speculative Herbal Ferment Recipe — f47v

IMPORTANT: this is NOT a real or validated translation of the Voynich Manuscript. It is a speculative/procedural model that interprets EVA using a user-defined grammar to generate experimental recipes using safe, known edible substitutes.

This file is generated automatically from IVTFF/EVA transliteration plus a user-defined procedural grammar.

![f47v](../../images/q06/f047v_crd.jpg)

## Page / Folio
- folio: f47v
- page_number: 92
- plant_candidates: ['Toronicum Pardalianchus Gemswurtz']
- plant_category_confidence: 0.25
- plant_category_guess: leaf
- plant_category_matches: ['section=herbal_default']
- plant_id: Toronicum Pardalianchus (Holm) [Gemswurtz]
- section: herbal
...
- compound_counts: {'yeast fermentation': 32, 'secondary herb': 11, 'mix/transfer': 42, 'heat': 17, 'main herb': 48, 'complex herbal compound': 3, 'sugars': 7, 'liquid base': 3}
- dose_level: 2
...
- psheot: apply heat/cooking → add secondary herb (safe substitute) → mix / transfer → start fermentation (yeast) → duration level 1 → state: active extraction
- cheor: add main plant (safe substitute) → mix / transfer → duration level 1 → state: active extraction
- cholsho: add main plant (safe substitute) → add secondary herb (safe substitute) → mix / transfer
...
- daiiy: start fermentation (yeast) → duration level 1 → state: fermentation start
...
Direct Gloss (Procedural, Not a Real Translation):
- dsholy: add secondary herb (safe substitute) → mix / transfer → start fermentation (yeast)
...
- dochod: add main plant (safe substitute) → mix / transfer → start fermentation (yeast)
...
- dcthy: start fermentation (yeast) → add complex herbal compound (safe blend)
...
Direct Gloss (Procedural, Not a Real Translation):
- daiin: start fermentation (yeast) → duration level 1 → state: fermentation start → long fermentation / aging phase
- cheotar: apply heat/cooking → add main plant (safe substitute) → mix / transfer → duration level 1 → state: active extraction
...
Direct Gloss (Procedural, Not a Real Translation):
- qotcheey: prepare liquid base → apply heat/cooking → add main plant (safe substitute) → duration level 2 → state: active extraction
...
Direct Gloss (Procedural, Not a Real Translation):
- chotcho: apply heat/cooking → add main plant (safe substitute) → mix / transfer
- ltchey: apply heat/cooking → add main plant (safe substitute) → duration level 1 → state: active extraction
- o: mix / transfer
- y: [unparsed]
...
- o: mix / transfer
- cho: add main plant (safe substitute) → mix / transfer
- keesy: add fermentable sugars → duration level 2 → state: active extraction
- chy: add main plant (safe substitute)
- chy: add main plant (safe substitute)
...
- daiin: start fermentation (yeast) → duration level 1 → state: fermentation start → long fermentation / aging phase
- chdlety: apply heat/cooking → add main plant (safe substitute) → start fermentation (yeast) → duration level 1 → state: active extraction
...
Direct Gloss (Procedural, Not a Real Translation):
- chol: add main plant (safe substitute) → mix / transfer
- ctchey: apply heat/cooking → add main plant (safe substitute) → duration level 1 → state: active extraction
