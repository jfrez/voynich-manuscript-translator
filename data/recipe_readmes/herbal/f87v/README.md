# Voynich Speculative Procedural Protocol — f87v

IMPORTANT: this is NOT a real or validated translation of the Voynich Manuscript. It is a speculative/procedural model that interprets EVA using a user-defined grammar to generate experimental recipes using safe, known edible substitutes.

This file is generated automatically from IVTFF/EVA transliteration plus a user-defined procedural grammar.

![f87v](../../../images/q15/f087v_crd.jpg)

## Page / Folio
- currier: A
- folio: f87v
- page_number: 180
- section: herbal

## EVA Text (Transliteration)
```text
@180;cheey daiin cpheodar shody dcheody todor oeeepchody oldar
schey ctheey qocthsy s dcheoky s cth
y cheockhy okey cthy yteeor ctos
dcheecthey cthosy ykeeor oky
ycheey keeo salckhy eoiin scheom
dcheos shey solchiyd oteos cthey
qotechoep cheos cthey qokeod qokeog
shoeey cthey qo cthodol yteodaiin sheol
techol qoscheody otol
opcheey cthhy cpheckhy cheo r chepcheol
kchor ol cheeol qok odal lseeey keodam
ytcheoq[o:y] chos ckheos cheoy s cheol sal dain
ksheol qoar cfhs ykaiin saral aiim am
san ainy keol dair old ain atolg
yksho qos arolor or al daraiing
saiin olcthr
```

## Domain Context (Heuristic; Not a Translation)

This section summarizes recurring **basewords** in this IVTFF domain and shows simple substring evidence that the token markers used by the procedural grammar occur inside frequent words.

Any Italian anagram / English gloss is a best-effort lexicon match, not a decipherment.



### Associated basewords (non-generic; top by frequency in this domain)
- `daiin` (count=461) → Italian anagram `piani`; English: plans (arrangements)
- `okaiin` (count=59) → Italian anagram `coniai`; English: [n/a]
- `chaiin` (count=39) → Italian anagram `acini`; English: [n/a]
- `saiin` (count=37) → Italian anagram `asini`; English: [n/a]
- `qokaiin` (count=34) → Italian anagram `ciancio`; English: [n/a]
- `qokar` (count=29) → Italian anagram `carco`; English: [n/a]
- `odaiin` (count=27) → Italian anagram `inopia`; English: poverty
- `otchol` (count=25) → Italian anagram `colto`; English: cultivated
- `kaiin` (count=24) → Italian anagram `acini`; English: [n/a]
- `chodaiin` (count=24) → Italian anagram `apocini`; English: [n/a]
- `qotol` (count=20) → Italian anagram `colto`; English: cultivated
- `okain` (count=19) → Italian anagram `acino`; English: a berry
- `qotor` (count=18) → Italian anagram `corto`; English: short
- `ykaiin` (count=16) → Italian anagram `acini`; English: [n/a]
- `qodaiin` (count=15) → Italian anagram `apocini`; English: [n/a]

### Marker evidence (substring in frequent basewords)
- `qo`: 57 basewords; examples: `qotchy`, `qokchy`, `qokedy`, `qokaiin`, `qoky`, `qokol`
- `q`: 58 basewords; examples: `qotchy`, `qokchy`, `qokedy`, `qokaiin`, `qoky`, `qokol`
- `o`: 252 basewords; examples: `chol`, `o`, `chor`, `or`, `shol`, `ol`
- `k`: 142 basewords; examples: `okaiin`, `oky`, `chckhy`, `qokchy`, `qokedy`, `okal`
- `t`: 102 basewords; examples: `cthy`, `oty`, `qotchy`, `cthol`, `cthor`, `otaiin`
- `p`: 15 basewords; examples: `cphy`, `ypchedy`, `opchy`, `opchey`, `pchor`, `qopchy`
- `ch`: 138 basewords; examples: `chol`, `chor`, `chy`, `chey`, `chedy`, `chdy`
- `sh`: 46 basewords; examples: `shol`, `sho`, `shy`, `shor`, `shey`, `shedy`
- `f`: 1 basewords; examples: `f`
- `cth`: 17 basewords; examples: `cthy`, `cthol`, `cthor`, `cthey`, `chcthy`, `ctho`
- `ckh`: 15 basewords; examples: `chckhy`, `ckhy`, `ckhol`, `ckhey`, `checkhy`, `shckhy`
- `cph`: 2 basewords; examples: `cphy`, `cphol`
- `dy`: 78 basewords; examples: `dy`, `chedy`, `chdy`, `chody`, `qokedy`, `shedy`
- `iin`: 39 basewords; examples: `daiin`, `aiin`, `okaiin`, `chaiin`, `saiin`, `qokaiin`
- `aiin`: 32 basewords; examples: `daiin`, `aiin`, `okaiin`, `chaiin`, `saiin`, `qokaiin`

## Recipes Index (This Page)
- [f87v.1,@P0](#f87v-1-f87v-1-p0)
- [f87v.2,+P0](#f87v-2-f87v-2-p0)
- [f87v.3,+P0](#f87v-3-f87v-3-p0)
- [f87v.4,+P0](#f87v-4-f87v-4-p0)
- [f87v.5,+P0](#f87v-5-f87v-5-p0)
- [f87v.6,+P0](#f87v-6-f87v-6-p0)
- [f87v.7,+P0](#f87v-7-f87v-7-p0)
- [f87v.8,+P0](#f87v-8-f87v-8-p0)
- [f87v.9,+P0](#f87v-9-f87v-9-p0)
- [f87v.10,+P0](#f87v-10-f87v-10-p0)
- [f87v.11,+P0](#f87v-11-f87v-11-p0)
- [f87v.12,+P0](#f87v-12-f87v-12-p0)
- [f87v.13,+P0](#f87v-13-f87v-13-p0)
- [f87v.14,+P0](#f87v-14-f87v-14-p0)
- [f87v.15,+P0](#f87v-15-f87v-15-p0)
- [f87v.16,+P0](#f87v-16-f87v-16-p0)

## Line Glosses (Procedural Gloss Only; Not a Translation)

<a id="f87v-1-f87v-1-p0"></a>

### f87v.1,@P0

EVA: @180;cheey daiin cpheodar shody dcheody todor oeeepchody oldar

Direct Gloss (Procedural, Not a Real Translation):
- cheey: add main plant (safe substitute) → duration level 2 → state: active extraction
- daiin: add starter / activate → duration level 1 → state: phase transition/start → long phase
- cpheodar: mix / transfer → add starter / activate → add complex herbal compound (safe blend) → duration level 1 → state: active extraction
- shody: add secondary herb (safe substitute) → mix / transfer → add starter / activate
- dcheody: add main plant (safe substitute) → mix / transfer → add starter / activate → duration level 1 → state: active extraction
- todor: apply heat/cooking → mix / transfer → add starter / activate
- oeeepchody: add main plant (safe substitute) → mix / transfer → add starter / activate → duration level 3 → state: active extraction
- oldar: mix / transfer → add starter / activate → duration level 1 → state: phase transition/start

<a id="f87v-2-f87v-2-p0"></a>

### f87v.2,+P0

EVA: schey ctheey qocthsy s dcheoky s cth

Direct Gloss (Procedural, Not a Real Translation):
- schey: add main plant (safe substitute) → duration level 1 → state: active extraction
- ctheey: add complex herbal compound (safe blend) → duration level 2 → state: active extraction
- qocthsy: prepare liquid base → add complex herbal compound (safe blend)
- s: [unparsed]
- dcheoky: add fermentable sugars → add main plant (safe substitute) → mix / transfer → add starter / activate → duration level 1 → state: active extraction
- s: [unparsed]
- cth: add complex herbal compound (safe blend)

<a id="f87v-3-f87v-3-p0"></a>

### f87v.3,+P0

EVA: y cheockhy okey cthy yteeor ctos

Direct Gloss (Procedural, Not a Real Translation):
- y: [unparsed]
- cheockhy: add main plant (safe substitute) → mix / transfer → add complex herbal compound (safe blend) → duration level 1 → state: active extraction
- okey: add fermentable sugars → mix / transfer → duration level 1 → state: active extraction
- cthy: add complex herbal compound (safe blend)
- yteeor: apply heat/cooking → mix / transfer → duration level 2 → state: active extraction
- ctos: apply heat/cooking → mix / transfer

<a id="f87v-4-f87v-4-p0"></a>

### f87v.4,+P0

EVA: dcheecthey cthosy ykeeor oky

Direct Gloss (Procedural, Not a Real Translation):
- dcheecthey: add main plant (safe substitute) → add starter / activate → add complex herbal compound (safe blend) → duration level 2 → state: active extraction
- cthosy: mix / transfer → add complex herbal compound (safe blend)
- ykeeor: add fermentable sugars → mix / transfer → duration level 2 → state: active extraction
- oky: add fermentable sugars → mix / transfer

<a id="f87v-5-f87v-5-p0"></a>

### f87v.5,+P0

EVA: ycheey keeo salckhy eoiin scheom

Direct Gloss (Procedural, Not a Real Translation):
- ycheey: add main plant (safe substitute) → duration level 2 → state: active extraction
- keeo: add fermentable sugars → mix / transfer → duration level 2 → state: active extraction
- salckhy: add complex herbal compound (safe blend) → duration level 1 → state: phase transition/start
- eoiin: mix / transfer → duration level 1 → state: active extraction → medium phase
- scheom: add main plant (safe substitute) → mix / transfer → duration level 1 → state: active extraction

<a id="f87v-6-f87v-6-p0"></a>

### f87v.6,+P0

EVA: dcheos shey solchiyd oteos cthey

Direct Gloss (Procedural, Not a Real Translation):
- dcheos: add main plant (safe substitute) → mix / transfer → add starter / activate → duration level 1 → state: active extraction
- shey: add secondary herb (safe substitute) → duration level 1 → state: active extraction
- solchiyd: add main plant (safe substitute) → mix / transfer → add starter / activate → duration level 1 → state: cooling/rest
- oteos: apply heat/cooking → mix / transfer → duration level 1 → state: active extraction
- cthey: add complex herbal compound (safe blend) → duration level 1 → state: active extraction

<a id="f87v-7-f87v-7-p0"></a>

### f87v.7,+P0

EVA: qotechoep cheos cthey qokeod qokeog

Direct Gloss (Procedural, Not a Real Translation):
- qotechoep: prepare liquid base → apply heat/cooking → add main plant (safe substitute) → mix / transfer → add starter / activate → duration level 1 → state: active extraction
- cheos: add main plant (safe substitute) → mix / transfer → duration level 1 → state: active extraction
- cthey: add complex herbal compound (safe blend) → duration level 1 → state: active extraction
- qokeod: prepare liquid base → add fermentable sugars → mix / transfer → add starter / activate → duration level 1 → state: active extraction
- qokeog: prepare liquid base → add fermentable sugars → mix / transfer → duration level 1 → state: active extraction

<a id="f87v-8-f87v-8-p0"></a>

### f87v.8,+P0

EVA: shoeey cthey qo cthodol yteodaiin sheol

Direct Gloss (Procedural, Not a Real Translation):
- shoeey: add secondary herb (safe substitute) → mix / transfer → duration level 2 → state: active extraction
- cthey: add complex herbal compound (safe blend) → duration level 1 → state: active extraction
- qo: prepare liquid base
- cthodol: mix / transfer → add starter / activate → add complex herbal compound (safe blend)
- yteodaiin: apply heat/cooking → mix / transfer → add starter / activate → duration level 1 → state: active extraction → long phase
- sheol: add secondary herb (safe substitute) → mix / transfer → duration level 1 → state: active extraction

<a id="f87v-9-f87v-9-p0"></a>

### f87v.9,+P0

EVA: techol qoscheody otol

Direct Gloss (Procedural, Not a Real Translation):
- techol: apply heat/cooking → add main plant (safe substitute) → mix / transfer → duration level 1 → state: active extraction
- qoscheody: prepare liquid base → add main plant (safe substitute) → mix / transfer → add starter / activate → duration level 1 → state: active extraction
- otol: apply heat/cooking → mix / transfer

<a id="f87v-10-f87v-10-p0"></a>

### f87v.10,+P0

EVA: opcheey cthhy cpheckhy cheo r chepcheol

Direct Gloss (Procedural, Not a Real Translation):
- opcheey: add main plant (safe substitute) → mix / transfer → add starter / activate → duration level 2 → state: active extraction
- cthhy: add complex herbal compound (safe blend) → unmodeled token(s) present: h
- cpheckhy: add complex herbal compound (safe blend) → duration level 1 → state: active extraction
- cheo: add main plant (safe substitute) → mix / transfer → duration level 1 → state: active extraction
- r: [unparsed]
- chepcheol: add main plant (safe substitute) → mix / transfer → add starter / activate → duration level 1 → state: active extraction

<a id="f87v-11-f87v-11-p0"></a>

### f87v.11,+P0

EVA: kchor ol cheeol qok odal lseeey keodam

Direct Gloss (Procedural, Not a Real Translation):
- kchor: add fermentable sugars → add main plant (safe substitute) → mix / transfer
- ol: mix / transfer
- cheeol: add main plant (safe substitute) → mix / transfer → duration level 2 → state: active extraction
- qok: prepare liquid base → add fermentable sugars
- odal: mix / transfer → add starter / activate → duration level 1 → state: phase transition/start
- lseeey: duration level 3 → state: active extraction
- keodam: add fermentable sugars → mix / transfer → add starter / activate → duration level 1 → state: active extraction

<a id="f87v-12-f87v-12-p0"></a>

### f87v.12,+P0

EVA: ytcheoq[o:y] chos ckheos cheoy s cheol sal dain

Direct Gloss (Procedural, Not a Real Translation):
- ytcheoq: prepare base (generic) → apply heat/cooking → add main plant (safe substitute) → mix / transfer → duration level 1 → state: active extraction
- o: mix / transfer
- y: [unparsed]
- chos: add main plant (safe substitute) → mix / transfer
- ckheos: mix / transfer → add complex herbal compound (safe blend) → duration level 1 → state: active extraction
- cheoy: add main plant (safe substitute) → mix / transfer → duration level 1 → state: active extraction
- s: [unparsed]
- cheol: add main plant (safe substitute) → mix / transfer → duration level 1 → state: active extraction
- sal: duration level 1 → state: phase transition/start
- dain: add starter / activate → duration level 1 → state: phase transition/start

<a id="f87v-13-f87v-13-p0"></a>

### f87v.13,+P0

EVA: ksheol qoar cfhs ykaiin saral aiim am

Direct Gloss (Procedural, Not a Real Translation):
- ksheol: add fermentable sugars → add secondary herb (safe substitute) → mix / transfer → duration level 1 → state: active extraction
- qoar: prepare liquid base → duration level 1 → state: phase transition/start
- cfhs: add complex herbal compound (safe blend)
- ykaiin: add fermentable sugars → duration level 1 → state: phase transition/start → long phase
- saral: duration level 1 → state: phase transition/start
- aiim: duration level 1 → state: phase transition/start
- am: duration level 1 → state: phase transition/start

<a id="f87v-14-f87v-14-p0"></a>

### f87v.14,+P0

EVA: san ainy keol dair old ain atolg

Direct Gloss (Procedural, Not a Real Translation):
- san: duration level 1 → state: phase transition/start
- ainy: duration level 1 → state: phase transition/start
- keol: add fermentable sugars → mix / transfer → duration level 1 → state: active extraction
- dair: add starter / activate → duration level 1 → state: phase transition/start
- old: mix / transfer → add starter / activate
- ain: duration level 1 → state: phase transition/start
- atolg: apply heat/cooking → mix / transfer → duration level 1 → state: phase transition/start

<a id="f87v-15-f87v-15-p0"></a>

### f87v.15,+P0

EVA: yksho qos arolor or al daraiing

Direct Gloss (Procedural, Not a Real Translation):
- yksho: add fermentable sugars → add secondary herb (safe substitute) → mix / transfer
- qos: prepare liquid base
- arolor: mix / transfer → duration level 1 → state: phase transition/start
- or: mix / transfer
- al: duration level 1 → state: phase transition/start
- daraiing: add starter / activate → duration level 1 → state: phase transition/start → long phase

<a id="f87v-16-f87v-16-p0"></a>

### f87v.16,+P0

EVA: saiin olcthr

Direct Gloss (Procedural, Not a Real Translation):
- saiin: duration level 1 → state: phase transition/start → long phase
- olcthr: mix / transfer → add complex herbal compound (safe blend)
