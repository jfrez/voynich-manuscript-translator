# Voynich Speculative Procedural Protocol — f16v

IMPORTANT: this is NOT a real or validated translation of the Voynich Manuscript. It is a speculative/procedural model that interprets EVA using a user-defined grammar to generate experimental recipes using safe, known edible substitutes.

This file is generated automatically from IVTFF/EVA transliteration plus a user-defined procedural grammar.

![f16v](../../../images/q02/f016v_crd.jpg)

## Page / Folio
- currier: A
- folio: f16v
- page_number: 30
- section: herbal

## EVA Text (Transliteration)
```text
pchroiin otchor chpchol chpchey s pchocty
ytchor y ky chokchy qokchocthor shory
ykchy dy choy qoty chy kchy koshet
dchol chcthody cphod chotol dal
ytchy chyty chor chol ytchy dan
sor ch k[a:o]r oty chkar chol dairin
pchocthy chypchy qotchy chcfhhy sy
daiin chol y daiin chcthy qot char chor sholo
dshy okaiin okaiin chol chor cthor ty chody
qokchy chy dy ykchy chckhy otain cthor [cth:ith]y
okytaiin chkchy saiin
daiin yky otor chody
sokar oaorar
```

## Domain Context (Heuristic; Not a Translation)

This section summarizes recurring **basewords** in this IVTFF domain and shows simple substring evidence that the token markers used by the procedural grammar occur inside frequent words.

Any Italian anagram / English gloss is a best-effort lexicon match, not a decipherment.



### Associated basewords (non-generic; top by frequency in this domain)
- `daiin` (count=461) → Italian anagram `piani`; English: plans (arrangements)
- `okaiin` (count=59) → Italian anagram `coniai`; English: [n/a]
- `chaiin` (count=39) → Italian anagram `acini`; English: [n/a]
- `saiin` (count=37) → Italian anagram `asini`; English: [n/a]
- `qokaiin` (count=34) → Italian anagram `ciancio`; English: [n/a]
- `qokar` (count=29) → Italian anagram `carco`; English: [n/a]
- `odaiin` (count=27) → Italian anagram `inopia`; English: poverty
- `otchol` (count=25) → Italian anagram `colto`; English: cultivated
- `kaiin` (count=24) → Italian anagram `acini`; English: [n/a]
- `chodaiin` (count=24) → Italian anagram `apocini`; English: [n/a]
- `qotol` (count=20) → Italian anagram `colto`; English: cultivated
- `okain` (count=19) → Italian anagram `acino`; English: a berry
- `qotor` (count=18) → Italian anagram `corto`; English: short
- `ykaiin` (count=16) → Italian anagram `acini`; English: [n/a]
- `qodaiin` (count=15) → Italian anagram `apocini`; English: [n/a]

### Marker evidence (substring in frequent basewords)
- `qo`: 57 basewords; examples: `qotchy`, `qokchy`, `qokedy`, `qokaiin`, `qoky`, `qokol`
- `q`: 58 basewords; examples: `qotchy`, `qokchy`, `qokedy`, `qokaiin`, `qoky`, `qokol`
- `o`: 252 basewords; examples: `chol`, `o`, `chor`, `or`, `shol`, `ol`
- `k`: 142 basewords; examples: `okaiin`, `oky`, `chckhy`, `qokchy`, `qokedy`, `okal`
- `t`: 102 basewords; examples: `cthy`, `oty`, `qotchy`, `cthol`, `cthor`, `otaiin`
- `p`: 15 basewords; examples: `cphy`, `ypchedy`, `opchy`, `opchey`, `pchor`, `qopchy`
- `ch`: 138 basewords; examples: `chol`, `chor`, `chy`, `chey`, `chedy`, `chdy`
- `sh`: 46 basewords; examples: `shol`, `sho`, `shy`, `shor`, `shey`, `shedy`
- `f`: 1 basewords; examples: `f`
- `cth`: 17 basewords; examples: `cthy`, `cthol`, `cthor`, `cthey`, `chcthy`, `ctho`
- `ckh`: 15 basewords; examples: `chckhy`, `ckhy`, `ckhol`, `ckhey`, `checkhy`, `shckhy`
- `cph`: 2 basewords; examples: `cphy`, `cphol`
- `dy`: 78 basewords; examples: `dy`, `chedy`, `chdy`, `chody`, `qokedy`, `shedy`
- `iin`: 39 basewords; examples: `daiin`, `aiin`, `okaiin`, `chaiin`, `saiin`, `qokaiin`
- `aiin`: 32 basewords; examples: `daiin`, `aiin`, `okaiin`, `chaiin`, `saiin`, `qokaiin`

## Recipes Index (This Page)
- [f16v.1,@P0](#f16v-1-f16v-1-p0)
- [f16v.2,+P0](#f16v-2-f16v-2-p0)
- [f16v.3,+P0](#f16v-3-f16v-3-p0)
- [f16v.4,+P0](#f16v-4-f16v-4-p0)
- [f16v.5,+P0](#f16v-5-f16v-5-p0)
- [f16v.6,+P0](#f16v-6-f16v-6-p0)
- [f16v.7,+P0](#f16v-7-f16v-7-p0)
- [f16v.8,+P0](#f16v-8-f16v-8-p0)
- [f16v.9,+P0](#f16v-9-f16v-9-p0)
- [f16v.10,+P0](#f16v-10-f16v-10-p0)
- [f16v.11,+P0](#f16v-11-f16v-11-p0)
- [f16v.12,+P0](#f16v-12-f16v-12-p0)
- [f16v.13,+P0](#f16v-13-f16v-13-p0)

## Line Glosses (Procedural Gloss Only; Not a Translation)

<a id="f16v-1-f16v-1-p0"></a>

### f16v.1,@P0

EVA: pchroiin otchor chpchol chpchey s pchocty

Direct Gloss (Procedural, Not a Real Translation):
- pchroiin: add main plant (safe substitute) → mix / transfer → add starter / activate → duration level 2 → state: cooling/rest → medium phase
- otchor: apply heat/cooking → add main plant (safe substitute) → mix / transfer
- chpchol: add main plant (safe substitute) → mix / transfer → add starter / activate
- chpchey: add main plant (safe substitute) → add starter / activate → duration level 1 → state: active extraction
- s: [unparsed]
- pchocty: apply heat/cooking → add main plant (safe substitute) → mix / transfer → add starter / activate

<a id="f16v-2-f16v-2-p0"></a>

### f16v.2,+P0

EVA: ytchor y ky chokchy qokchocthor shory

Direct Gloss (Procedural, Not a Real Translation):
- ytchor: apply heat/cooking → add main plant (safe substitute) → mix / transfer
- y: [unparsed]
- ky: add fermentable sugars
- chokchy: add fermentable sugars → add main plant (safe substitute) → mix / transfer
- qokchocthor: prepare liquid base → add fermentable sugars → add main plant (safe substitute) → mix / transfer → add complex herbal compound (safe blend)
- shory: add secondary herb (safe substitute) → mix / transfer

<a id="f16v-3-f16v-3-p0"></a>

### f16v.3,+P0

EVA: ykchy dy choy qoty chy kchy koshet

Direct Gloss (Procedural, Not a Real Translation):
- ykchy: add fermentable sugars → add main plant (safe substitute)
- dy: add starter / activate
- choy: add main plant (safe substitute) → mix / transfer
- qoty: prepare liquid base → apply heat/cooking
- chy: add main plant (safe substitute)
- kchy: add fermentable sugars → add main plant (safe substitute)
- koshet: add fermentable sugars → apply heat/cooking → add secondary herb (safe substitute) → mix / transfer → duration level 1 → state: active extraction

<a id="f16v-4-f16v-4-p0"></a>

### f16v.4,+P0

EVA: dchol chcthody cphod chotol dal

Direct Gloss (Procedural, Not a Real Translation):
- dchol: add main plant (safe substitute) → mix / transfer → add starter / activate
- chcthody: add main plant (safe substitute) → mix / transfer → add starter / activate → add complex herbal compound (safe blend)
- cphod: mix / transfer → add starter / activate → add complex herbal compound (safe blend)
- chotol: apply heat/cooking → add main plant (safe substitute) → mix / transfer
- dal: add starter / activate → duration level 1 → state: phase transition/start

<a id="f16v-5-f16v-5-p0"></a>

### f16v.5,+P0

EVA: ytchy chyty chor chol ytchy dan

Direct Gloss (Procedural, Not a Real Translation):
- ytchy: apply heat/cooking → add main plant (safe substitute)
- chyty: apply heat/cooking → add main plant (safe substitute)
- chor: add main plant (safe substitute) → mix / transfer
- chol: add main plant (safe substitute) → mix / transfer
- ytchy: apply heat/cooking → add main plant (safe substitute)
- dan: add starter / activate → duration level 1 → state: phase transition/start

<a id="f16v-6-f16v-6-p0"></a>

### f16v.6,+P0

EVA: sor ch k[a:o]r oty chkar chol dairin

Direct Gloss (Procedural, Not a Real Translation):
- sor: mix / transfer
- ch: add main plant (safe substitute)
- k: add fermentable sugars
- a: duration level 1 → state: phase transition/start
- o: mix / transfer
- r: [unparsed]
- oty: apply heat/cooking → mix / transfer
- chkar: add fermentable sugars → add main plant (safe substitute) → duration level 1 → state: phase transition/start
- chol: add main plant (safe substitute) → mix / transfer
- dairin: add starter / activate → duration level 1 → state: phase transition/start

<a id="f16v-7-f16v-7-p0"></a>

### f16v.7,+P0

EVA: pchocthy chypchy qotchy chcfhhy sy

Direct Gloss (Procedural, Not a Real Translation):
- pchocthy: add main plant (safe substitute) → mix / transfer → add starter / activate → add complex herbal compound (safe blend)
- chypchy: add main plant (safe substitute) → add starter / activate
- qotchy: prepare liquid base → apply heat/cooking → add main plant (safe substitute)
- chcfhhy: add main plant (safe substitute) → add complex herbal compound (safe blend) → unmodeled token(s) present: h
- sy: [unparsed]

<a id="f16v-8-f16v-8-p0"></a>

### f16v.8,+P0

EVA: daiin chol y daiin chcthy qot char chor sholo

Direct Gloss (Procedural, Not a Real Translation):
- daiin: add starter / activate → duration level 1 → state: phase transition/start → long phase
- chol: add main plant (safe substitute) → mix / transfer
- y: [unparsed]
- daiin: add starter / activate → duration level 1 → state: phase transition/start → long phase
- chcthy: add main plant (safe substitute) → add complex herbal compound (safe blend)
- qot: prepare liquid base → apply heat/cooking
- char: add main plant (safe substitute) → duration level 1 → state: phase transition/start
- chor: add main plant (safe substitute) → mix / transfer
- sholo: add secondary herb (safe substitute) → mix / transfer

<a id="f16v-9-f16v-9-p0"></a>

### f16v.9,+P0

EVA: dshy okaiin okaiin chol chor cthor ty chody

Direct Gloss (Procedural, Not a Real Translation):
- dshy: add secondary herb (safe substitute) → add starter / activate
- okaiin: add fermentable sugars → mix / transfer → duration level 1 → state: phase transition/start → long phase
- okaiin: add fermentable sugars → mix / transfer → duration level 1 → state: phase transition/start → long phase
- chol: add main plant (safe substitute) → mix / transfer
- chor: add main plant (safe substitute) → mix / transfer
- cthor: mix / transfer → add complex herbal compound (safe blend)
- ty: apply heat/cooking
- chody: add main plant (safe substitute) → mix / transfer → add starter / activate

<a id="f16v-10-f16v-10-p0"></a>

### f16v.10,+P0

EVA: qokchy chy dy ykchy chckhy otain cthor [cth:ith]y

Direct Gloss (Procedural, Not a Real Translation):
- qokchy: prepare liquid base → add fermentable sugars → add main plant (safe substitute)
- chy: add main plant (safe substitute)
- dy: add starter / activate
- ykchy: add fermentable sugars → add main plant (safe substitute)
- chckhy: add main plant (safe substitute) → add complex herbal compound (safe blend)
- otain: apply heat/cooking → mix / transfer → duration level 1 → state: phase transition/start
- cthor: mix / transfer → add complex herbal compound (safe blend)
- cth: add complex herbal compound (safe blend)
- ith: apply heat/cooking → duration level 1 → state: cooling/rest → unmodeled token(s) present: h
- y: [unparsed]

<a id="f16v-11-f16v-11-p0"></a>

### f16v.11,+P0

EVA: okytaiin chkchy saiin

Direct Gloss (Procedural, Not a Real Translation):
- okytaiin: add fermentable sugars → apply heat/cooking → mix / transfer → duration level 1 → state: phase transition/start → long phase
- chkchy: add fermentable sugars → add main plant (safe substitute)
- saiin: duration level 1 → state: phase transition/start → long phase

<a id="f16v-12-f16v-12-p0"></a>

### f16v.12,+P0

EVA: daiin yky otor chody

Direct Gloss (Procedural, Not a Real Translation):
- daiin: add starter / activate → duration level 1 → state: phase transition/start → long phase
- yky: add fermentable sugars
- otor: apply heat/cooking → mix / transfer
- chody: add main plant (safe substitute) → mix / transfer → add starter / activate

<a id="f16v-13-f16v-13-p0"></a>

### f16v.13,+P0

EVA: sokar oaorar

Direct Gloss (Procedural, Not a Real Translation):
- sokar: add fermentable sugars → mix / transfer → duration level 1 → state: phase transition/start
- oaorar: mix / transfer → duration level 1 → state: phase transition/start
