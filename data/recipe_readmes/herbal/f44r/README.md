# Voynich Speculative Procedural Protocol — f44r

IMPORTANT: this is NOT a real or validated translation of the Voynich Manuscript. It is a speculative/procedural model that interprets EVA using a user-defined grammar to generate experimental recipes using safe, known edible substitutes.

This file is generated automatically from IVTFF/EVA transliteration plus a user-defined procedural grammar.

![f44r](../../../images/q06/f044r_crd.jpg)

## Page / Folio
- folio: f44r
- page_number: 85
- section: herbal

## EVA Text (Transliteration)
```text
tshodpy oraiphy koees ypsholy shy otoloaiin
ydsh dyeees y ty oky okchey qykchey dchy dy
oair ekokeey kshotol otal daiin ototaykal
ychor ykchey ykchy chody dam
toy shysho qoteey char cheeky sheey ytey daiin
dshor y tchol shy o @190;cheekchy qotchy otchy dar
qotch' o[q:y]d shol qotshy oyty chom
pshy opchey qopchy ofchey roiry sholos ykchy
otchol ol dchckhy qoky qotchy qokchy qokyd
qokchor okchy qoto ykol choky choky chol dam
ytsho qockhy okchody
```

## Domain Context (Heuristic; Not a Translation)

This section summarizes recurring **basewords** in this IVTFF domain and shows simple substring evidence that the token markers used by the procedural grammar occur inside frequent words.

Any Italian anagram / English gloss is a best-effort lexicon match, not a decipherment.



### Associated basewords (non-generic; top by frequency in this domain)
- `paiin` (count=477) → Italian anagram `piani`; English: plans (arrangements)
- `okaiin` (count=59) → Italian anagram `coniai`; English: [n/a]
- `qokep` (count=41) → Italian anagram `pecco`; English: [n/a]
- `saiin` (count=40) → Italian anagram `asini`; English: [n/a]
- `kaiin` (count=40) → Italian anagram `acini`; English: [n/a]
- `chaiin` (count=39) → Italian anagram `acini`; English: [n/a]
- `qokaiin` (count=34) → Italian anagram `ciancio`; English: [n/a]
- `qokar` (count=29) → Italian anagram `carco`; English: [n/a]
- `opaiin` (count=29) → Italian anagram `inopia`; English: poverty
- `otchol` (count=25) → Italian anagram `colto`; English: cultivated
- `chopaiin` (count=24) → Italian anagram `apocini`; English: [n/a]
- `qotol` (count=20) → Italian anagram `colto`; English: cultivated
- `okain` (count=19) → Italian anagram `acino`; English: a berry
- `qotor` (count=18) → Italian anagram `corto`; English: short
- `qopaiin` (count=15) → Italian anagram `apocini`; English: [n/a]

### Marker evidence (substring in frequent basewords)
- `qo`: 58 basewords; examples: `qotch`, `qok`, `qot`, `qokch`, `qokep`, `qokaiin`
- `q`: 59 basewords; examples: `qotch`, `qok`, `qot`, `qokch`, `qokep`, `qokaiin`
- `o`: 274 basewords; examples: `chol`, `o`, `chor`, `or`, `shol`, `ol`
- `k`: 146 basewords; examples: `ok`, `k`, `okaiin`, `kch`, `chckh`, `qok`
- `t`: 101 basewords; examples: `cth`, `ot`, `t`, `qotch`, `cthol`, `qot`
- `p`: 152 basewords; examples: `paiin`, `p`, `par`, `pain`, `pal`, `chep`
- `ch`: 145 basewords; examples: `chol`, `chor`, `ch`, `che`, `chep`, `cho`
- `sh`: 51 basewords; examples: `shol`, `sh`, `sho`, `shor`, `she`, `shep`
- `f`: 2 basewords; examples: `fchep`, `f`
- `cth`: 18 basewords; examples: `cth`, `cthol`, `cthor`, `cthe`, `chcth`, `ctho`
- `ckh`: 18 basewords; examples: `chckh`, `ckh`, `ckhe`, `ckhol`, `shckh`, `checkh`
- `cph`: 3 basewords; examples: `cph`, `cphol`, `cphe`
- `iin`: 39 basewords; examples: `paiin`, `aiin`, `okaiin`, `saiin`, `kaiin`, `chaiin`
- `aiin`: 31 basewords; examples: `paiin`, `aiin`, `okaiin`, `saiin`, `kaiin`, `chaiin`

## Recipes Index (This Page)
- [f44r.1,@P0](#f44r-1-f44r-1-p0)
- [f44r.2,+P0](#f44r-2-f44r-2-p0)
- [f44r.3,+P0](#f44r-3-f44r-3-p0)
- [f44r.4,+P0](#f44r-4-f44r-4-p0)
- [f44r.5,+P0](#f44r-5-f44r-5-p0)
- [f44r.6,+P0](#f44r-6-f44r-6-p0)
- [f44r.7,+P0](#f44r-7-f44r-7-p0)
- [f44r.8,+P0](#f44r-8-f44r-8-p0)
- [f44r.9,+P0](#f44r-9-f44r-9-p0)
- [f44r.10,+P0](#f44r-10-f44r-10-p0)
- [f44r.11,+P0](#f44r-11-f44r-11-p0)

## Line Glosses (Procedural Gloss Only; Not a Translation)

<a id="f44r-1-f44r-1-p0"></a>

### f44r.1,@P0

EVA: tshodpy oraiphy koees ypsholy shy otoloaiin

Direct Gloss (Procedural, Not a Real Translation):
- tshodpy: tokens: t sh o p p
- oraiphy: tokens: o r a i p h → connectors: r → vowel_run: a (level 1; class a) → unmodeled_tokens: h
- koees: tokens: k o ee s → connectors: s → vowel_run: ee (level 2; class e)
- ypsholy: tokens: p sh o l → connectors: l
- shy: tokens: sh
- otoloaiin: tokens: o t o l o aiin → connectors: l → vowel_run: a (level 1; class a) → suffix: aiin

<a id="f44r-2-f44r-2-p0"></a>

### f44r.2,+P0

EVA: ydsh dyeees y ty oky okchey qykchey dchy dy

Direct Gloss (Procedural, Not a Real Translation):
- ydsh: tokens: p sh
- dyeees: tokens: p eee s → connectors: s → vowel_run: eee (level 3; class e)
- y: [unparsed]
- ty: tokens: t
- oky: tokens: o k
- okchey: tokens: o k ch e → vowel_run: e (level 1; class e)
- qykchey: tokens: q k ch e → vowel_run: e (level 1; class e)
- dchy: tokens: p ch
- dy: tokens: p

<a id="f44r-3-f44r-3-p0"></a>

### f44r.3,+P0

EVA: oair ekokeey kshotol otal daiin ototaykal

Direct Gloss (Procedural, Not a Real Translation):
- oair: tokens: o a i r → connectors: r → vowel_run: a (level 1; class a)
- ekokeey: tokens: e k o k ee → vowel_run: e (level 1; class e)
- kshotol: tokens: k sh o t o l → connectors: l
- otal: tokens: o t a l → connectors: l → vowel_run: a (level 1; class a)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))
- ototaykal: tokens: o t o t a k a l → connectors: l → vowel_run: a (level 1; class a)

<a id="f44r-4-f44r-4-p0"></a>

### f44r.4,+P0

EVA: ychor ykchey ykchy chody dam

Direct Gloss (Procedural, Not a Real Translation):
- ychor: tokens: ch o r → connectors: r
- ykchey: tokens: k ch e → vowel_run: e (level 1; class e)
- ykchy: tokens: k ch
- chody: tokens: ch o p
- dam: tokens: p a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f44r-5-f44r-5-p0"></a>

### f44r.5,+P0

EVA: toy shysho qoteey char cheeky sheey ytey daiin

Direct Gloss (Procedural, Not a Real Translation):
- toy: tokens: t o
- shysho: tokens: sh sh o
- qoteey: tokens: qo t ee → vowel_run: ee (level 2; class e)
- char: tokens: ch a r → connectors: r → vowel_run: a (level 1; class a)
- cheeky: tokens: ch ee k → vowel_run: ee (level 2; class e)
- sheey: tokens: sh ee → vowel_run: ee (level 2; class e)
- ytey: tokens: t e → vowel_run: e (level 1; class e)
- daiin: tokens: p aiin → vowel_run: a (level 1; class a) → suffix: aiin (lexicon-context: `paiin` → `piani`; plans (arrangements))

<a id="f44r-6-f44r-6-p0"></a>

### f44r.6,+P0

EVA: dshor y tchol shy o @190;cheekchy qotchy otchy dar

Direct Gloss (Procedural, Not a Real Translation):
- dshor: tokens: p sh o r → connectors: r
- y: [unparsed]
- tchol: tokens: t ch o l → connectors: l
- shy: tokens: sh
- o: tokens: o
- cheekchy: tokens: ch ee k ch → vowel_run: ee (level 2; class e)
- qotchy: tokens: qo t ch
- otchy: tokens: o t ch
- dar: tokens: p a r → connectors: r → vowel_run: a (level 1; class a)

<a id="f44r-7-f44r-7-p0"></a>

### f44r.7,+P0

EVA: qotch' o[q:y]d shol qotshy oyty chom

Direct Gloss (Procedural, Not a Real Translation):
- qotch: tokens: qo t ch
- o: tokens: o
- q: tokens: q
- y: [unparsed]
- d: tokens: p
- shol: tokens: sh o l → connectors: l
- qotshy: tokens: qo t sh
- oyty: tokens: o t
- chom: tokens: ch o m → connectors: m

<a id="f44r-8-f44r-8-p0"></a>

### f44r.8,+P0

EVA: pshy opchey qopchy ofchey roiry sholos ykchy

Direct Gloss (Procedural, Not a Real Translation):
- pshy: tokens: p sh
- opchey: tokens: o p ch e → vowel_run: e (level 1; class e)
- qopchy: tokens: qo p ch
- ofchey: tokens: o f ch e → vowel_run: e (level 1; class e)
- roiry: tokens: r o i r → connectors: r r → vowel_run: i (level 1; class i)
- sholos: tokens: sh o l o s → connectors: l s
- ykchy: tokens: k ch

<a id="f44r-9-f44r-9-p0"></a>

### f44r.9,+P0

EVA: otchol ol dchckhy qoky qotchy qokchy qokyd

Direct Gloss (Procedural, Not a Real Translation):
- otchol: tokens: o t ch o l → connectors: l (lexicon-context: `otchol` → `colto`; cultivated)
- ol: tokens: o l → connectors: l
- dchckhy: tokens: p ch ckh
- qoky: tokens: qo k
- qotchy: tokens: qo t ch
- qokchy: tokens: qo k ch
- qokyd: tokens: qo k p

<a id="f44r-10-f44r-10-p0"></a>

### f44r.10,+P0

EVA: qokchor okchy qoto ykol choky choky chol dam

Direct Gloss (Procedural, Not a Real Translation):
- qokchor: tokens: qo k ch o r → connectors: r
- okchy: tokens: o k ch
- qoto: tokens: qo t o
- ykol: tokens: k o l → connectors: l
- choky: tokens: ch o k
- choky: tokens: ch o k
- chol: tokens: ch o l → connectors: l
- dam: tokens: p a m → connectors: m → vowel_run: a (level 1; class a)

<a id="f44r-11-f44r-11-p0"></a>

### f44r.11,+P0

EVA: ytsho qockhy okchody

Direct Gloss (Procedural, Not a Real Translation):
- ytsho: tokens: t sh o
- qockhy: tokens: qo ckh
- okchody: tokens: o k ch o p
